...
| `context_current_state_clause/0` | "CONTEXT.md documents the **current state** of the code — what is TRUE NOW…" — the shared "current state, not history" paragraph, extended with active-maintenance guidance (keep CONTEXT.md concise by pruning when adding; write findings at the best-fit child/descendant node with a routing-table entry at the current level; extract oversized sections into skills) | Manager, Architect, Investigator, ContextExtractor |
