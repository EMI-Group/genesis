...
- **Architect foreign-repo role rule (in `architect.ex` "Foreign Repository Integration")**: the Architect prompt requires determining what each foreign repo is FOR before designing — tests / reference implementation / dependency / docs — and treats foreign-repo test suites as "given test suites": design so they pass (100% pass-rate target) and propagate them into Phase 2 implementation objectives. Do not remove this when editing the prompt; it fixes the real-world failure mode where tests were implicitly supplied as foreign repos and the Architect ignored them.
