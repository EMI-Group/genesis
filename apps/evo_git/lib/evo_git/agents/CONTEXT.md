...
| `EvoGit.Agents.SelfReflective` | `self_reflective.ex` | Repo-less self-reflective agent (chatbot-like, no worktree, no repository of its own) — read-only over the Genesis source (its `repo_path` IS the Genesis source root); controls tasks, shows user guides, and reports system info on the user's behalf via the single `run_command` shell tool (`task.list/get/start/cancel/force_kill/delete/investigate`, `guide.show`, `project.list`, `system.info`; `task.investigate` is a v1 placeholder) | `:read` | None | None (read-only) |
