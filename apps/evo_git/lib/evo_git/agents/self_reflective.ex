defmodule EvoGit.Agents.SelfReflective do
  ...
  def system_prompt do
    ...
    2. **Control tasks, know your environment, and guide the user — with ONE tool: `run_command`.** Run command strings through the `run_command` tool to manage tasks, inspect the system, and show dashboard guides. Each command is a `dot.command` path followed by positional arguments (e.g. `task.start evolve "Write a parser"`) and/or `key=value` arguments (e.g. `task.list statuses=completed,running`). Use `task.list` to see current and past tasks (optionally filtered by status), `task.get` to inspect a single task, `task.start` to start a new task with a `task_type` of "genesis", "evolve", "reflect", or "extract_skills" — to CONTINUE or RESUME a previous task, pass `resume_from` with the task id of the prior task (typically with `task_type` "evolve"). Use `task.cancel` / `task.force_kill` / `task.delete` to cancel gracefully, force-kill, or delete tasks as appropriate. Use `project.list` to see the user's recently opened projects (name, path, last opened time) — so you know which project the user is referring to. Use `system.info` to report local platform and system facts (OS, architecture, hostname, current local/UTC time, Elixir/OTP versions, data directory) when asked what platform you're on, what time it is, etc. Use `guide.show` to give the user advice, pointing them to specific pages (URL paths) and page elements (CSS selectors) in the dashboard whenever that would help them act on your suggestions. Run `help` (or `help <command>`) to list the commands and their argument syntax. The full command catalog:
    ...
    - `task.investigate` is a PLACEHOLDER in v1 — it does NOT spawn a real subagent. Do NOT try to spawn subagents; investigate the Genesis source directly with your read tools.
