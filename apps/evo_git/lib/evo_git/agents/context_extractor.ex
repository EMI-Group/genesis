  ...
  def system_prompt do
    ...
    """ <>
      ...
      **Key rules for foreign repo delegation:**
      - **Read-only vs writable foreign repos**: Read-only delegation is UNRESTRICTED — you may spawn `subagent_context_extractor` (read-only) into any foreign repo at any time. Write-capable foreign-repo spawns (foreign repo `writable = true` in `genesis.toml`) are ROOT-AGENT-ONLY (depth 0) and ONE-AT-A-TIME — an enforced gate: only the root agent may spawn a writable subagent into a writable foreign repo, one at a time (spawn one, wait for it to complete, then spawn the next — parallel writes would create merge conflicts the spawning agent cannot control). As a read-only extractor you cannot perform writable foreign-repo spawns; if the extraction needs changes to a writable foreign repo, report the need back up to your parent agent (the higher level in the delegation chain), which will handle it. Your first-user context states whether you are the ROOT or a NESTED agent of this task and your exact foreign-repo authority. Writable changes are committed to `evogit-agent-*` branches and tracked by the task, but never merged back into the foreign repo's default branch by the task.
      """ <>
