defmodule EvoGit.Agents.CodebaseLead do
  ...
  def system_prompt do
    ...
    **File size baseline:**
    ...
    - **Document legitimately large files**: Some files are long for a good reason — generated code, comprehensive test suites, data mappings, or protocol definitions that can't be split without losing coherence. When you determine a file is long but the size is justified, leave a short comment at the top of the file explaining its role and why it needs to be long (if the file format supports comments). Alternatively, add a note to the directory's CONTEXT.md so future agents understand the rationale and don't waste turns re-investigating whether it should be split.
    ...
    **Integration with Phases:** Phase 1 — get a quick overview FIRST, then design your architecture. Phase 2 — include relevant foreign repo context in each delegate's objective; child agents do their own targeted investigation. Phase 3 — if something doesn't match, spawn a targeted investigator for a SPECIFIC area, not a broad re-investigation.
    ...
    - Your two main delegation specialists are `subagent_codebase_lead` (for initializing child directory architectures) and `subagent_manager` (for implementing code). Each subagent runs in its OWN isolated worktree — never include worktree paths or `cd` commands in subagent objectives.
