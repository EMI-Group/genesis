defmodule EvoGit.Agents.Manager do
  ...
  def subagent_tool_description do
    "[Subagent] A manager agent that orchestrates work within a child node. " <>
      ...
      "When spawning a sub-manager into a writable foreign repo, only the root agent (depth 0) may do so and only one at a time — a sub-manager running inside a foreign repo owns its own parallelism there."
  end
