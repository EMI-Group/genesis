...
| `EvoGit.Runtime.WorktreeInitScript` | `worktree_init_script.ex` | `generate/2`, `build_system_prompt/0`, `build_user_prompt/1`, `extract_script/1` | Genesis Mode B one-shot LLM query that generates a "Worktree Init Script" and writes it to `genesis.toml` (`[worktree].script`) so the existing per-worktree init-script infrastructure copies deps/build cache into new worktrees. Runs before any agent is spawned; NON-FATAL on failure. |
...
### Spurious `:failed` Hazard — ETS Ownership & Crash Cascade (KNOWN BUG)
A task can be spuriously marked `:failed` while still running. The root cause is that the three scheduler ETS tables (`:evogit_agent_state`, `:evogit_sched_meta`, `:evogit_archive_records`) are created in `AgentScheduler.init/1` (`agent_scheduler.ex:353-355`) and therefore **owned by the GenServer process**. They have **no `:heir`** (grep for `:heir` returns nothing). If the `AgentScheduler` GenServer crashes and its supervisor restarts it, the tables are **destroyed and recreated empty**, triggering this cascade:
1. `AgentScheduler` crashes → ETS tables destroyed.
2. The EvoDash wrapper process's blocking `GenServer.call(__MODULE__, {:run_agent, spec}, :infinity)` (`agent_scheduler.ex:76-78`) raises (caller died) → wrapper process crashes.
3. EvoDash `TaskRegistry`'s `:DOWN` handler (`task_registry.ex:907`) checks `sched_meta_has_active_agents?(task_id)` against the now-empty/missing ETS table → returns `false`.
4. Task prematurely marked `:failed` (`task_registry.ex:939`) even though the agent processes are still alive under `EvoGit.TaskSupervisor` and will eventually commit their work.

This explains the symptom where `:failed` is set BEFORE the runtime's `notify_finalizing`/`merge_and_report` run (which log "Agent produced changes" / "Created branch"): the `:failed` comes from the EvoDash monitor cascade, not from the runtime return value. The `sched_meta_has_active_agents?/1` guard (`task_registry.ex:555-572`) returns `false` both when the table is `:undefined` and when the matching `task_id` entries were already cleaned up during agent teardown (`lifecycle.ex` calls `Store.delete_sched_meta/1`).

**The fix exists but is unmerged**: commit `c7126f59` ("move ETS table creation to Application to survive AgentScheduler crashes", on branch `evogit/evolve_b1695fe4`, NOT in `HEAD` history) moves table creation to `EvoGit.Application.start/2` via an idempotent `ensure_ets_table/2` helper, so the tables survive scheduler crashes. A companion EvoDash commit `9a7acd8a` ("Fix TaskRegistry terminal guards to allow status recovery") also exists on the same branch.
