...
Every message in an agent's session memory (the `%ReqLLM.Message{}` list inside `%ReqLLM.Context{}.messages`, held in `AgentState.context` / `:evogit_agent_state` ETS) is stamped with `metadata[:timestamp]` — **Unix seconds** (`System.system_time(:second)`), an atom-keyed integer consistent with the existing `:turn` metadata convention. Stamping is **guarded and idempotent**: it happens ONLY at the single choke point `EvoGit.AgentScheduler.Store.update_agent_context/2` (`store.ex:353`), and only for messages that do NOT already carry a `:timestamp` — so already-stamped messages keep their original time across repeated context syncs, and re-syncs never overwrite. Because it runs on every context sync, it covers ALL message types (assistant, tool results, nudge, compression summaries, user-injected via `send_user_message`) with one code path. The metadata flows through `RemoteAPI.get_agent_history/1` unchanged (raw structs), so consumers read `msg.metadata[:timestamp]` directly. `DateTime` was deliberately avoided (struct — heavier, and Jason-encodes to ISO-8601 strings elsewhere in the codebase); an integer is timezone-agnostic and trivial for the dashboard to convert to local display time. Covered by `test/evo_git/agent_scheduler/store_test.exs` (7 tests). Dashboard display consumption is a separate task in `./apps/evo_dash/`.
