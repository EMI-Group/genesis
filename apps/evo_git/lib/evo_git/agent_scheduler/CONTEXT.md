...
|------|-----------|----------|---------|
| LLM slots | `llm_holders`, `llm_waiting`, `llm_backoff_until` | **per-model** `model_concurrency[model_id]` (from `[[llm.models]]` profile `concurrency`, default 3); fallback `max_concurrency` (3) for unknown model_ids via `State.concurrency_for/2` | **per-model** 60s cooldown on `:rate_limit` errors (NOT global — one model's rate limit doesn't block others) |
| Tool slots | `tool_holders`, `tool_waiting` | `max_tool_concurrency` (2) | None |

### `scheduler.max_concurrency` precedence (verified 2026-08, do not "simplify")

`scheduler.max_concurrency` is **not dead config, but its live effect is narrow**:
- **Effective LLM limit for an agent = `model_concurrency[model_id]` if present, else `state.max_concurrency`** (`State.concurrency_for/2`, state.ex:205-207). Both slot enforcers read it: `Slots.do_handle_request_llm_slot/4` (slots.ex:103) and `Slots.grant_llm_from_queue/2` (slots.ex:399).
...
- **Legacy (no `[[llm.models]]`) path**: `scheduler.max_concurrency` IS live — it becomes the synthesized "default" profile's `concurrency` via `Schema.LLM.build_legacy_default_profile/1` (schema/llm.ex:123), then flows into `model_concurrency["default"]`. This happens at config-load time (init or reload), NOT at runtime.
- **When `[[llm.models]]` profiles exist**: each profile's own `concurrency` wins for its pool; `scheduler.max_concurrency` does NOT cap or override it — it only remains as (a) the fallback for agents whose `model_id` is not in `model_concurrency` (state.ex:206) and (b) the value surfaced in `get_config/0` (agent_scheduler.ex:656).
- **⚠️ CLI `-c`/`--concurrency` override is effectively inert against live pools**: `CLI.Parser.configure_scheduler/1` maps `-c` → `update_config(max_concurrency: N)` (parser.ex:67) → `State.do_update_config` `maybe_update(:max_concurrency, opts)` (state.ex:332), which changes ONLY `state.max_concurrency`. It never touches `model_concurrency`, and since the default profile id is always present in `model_concurrency`, the fallback is never consulted in normal operation — so `-c` has no visible effect on LLM concurrency unless an agent is bound to a model_id absent from the profiles map. (`--tool-concurrency` → `update_config(max_tool_concurrency: N)` DOES take effect — tool slots read `state.max_tool_concurrency` directly, slots.ex:219.) The TOML-file path (`scheduler.max_concurrency` + reload) works as described above; the runtime-override path does not.
