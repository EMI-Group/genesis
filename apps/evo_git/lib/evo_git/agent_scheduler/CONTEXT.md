...
Every message in an agent's session memory (the `%ReqLLM.Message{}` list inside `%ReqLLM.Context{}.messages`, held in `AgentState.context` / `:evogit_agent_state` ETS) is stamped with `metadata[:timestamp]` — **Unix seconds** (`System.system_time(:second)`), an atom-keyed integer consistent with the existing `:turn` metadata convention. Stamping is **guarded and idempotent** (`stamp_missing_timestamps/1`, store.ex): only messages that do NOT already carry a `:timestamp` get stamped, so already-stamped messages keep their original time across repeated context syncs and re-syncs never overwrite. `metadata: nil` is tolerated (stamped into a fresh map), and other metadata keys like `:turn` are preserved alongside `:timestamp`.

**Single choke point — `batch_update_agent/2` (store.ex:324)**: every ETS context write goes through it, and it stamps a `:context` field value (when a `%ReqLLM.Context{}`) before the `:ets.insert` (`stamp_context_field/1`). This covers BOTH the `update_agent_context/2` path AND the agent loop's post-LLM-response batch write (tool_dispatch.ex:411), so the stored history NEVER has a no-timestamp window mid-turn — and the final assistant message on the `{:complete, ...}` path (tool_dispatch.ex:423-424) is stamped too, because it is already in the context at the :411 write. `update_agent_context/2` is a thin wrapper that returns the STAMPED `%ReqLLM.Context{}` — the exact value written to ETS — so callers can adopt it into their in-memory state; this is what keeps the idempotence guard engaged across turns (an in-memory copy carrying stamps means a later sync only stamps genuinely new messages). `ContextBuilder.sync_context_to_ets/2` (agent/context_builder.ex:88-90) passes the stamped context through unchanged. Non-context values (`context: nil` on crash-retry clears, lifecycle.ex:171) and keyword lists without `:context` pass through untouched.

**Write inventory (all stamped):** runner.ex:134 (initial sync, turn 0), runner.ex:233 (user-message injection), runner.ex:253 (top-of-loop after compression/warnings), tool_dispatch.ex:411 (after every LLM response — stamped via `batch_update_agent`), tool_dispatch.ex:465 (end of each `{:continue, ...}` turn after tool responses). The metadata flows through `RemoteAPI.get_agent_history/1` unchanged (raw structs), so consumers read `msg.metadata[:timestamp]` directly. `DateTime` was deliberately avoided (struct — heavier, and Jason-encodes to ISO-8601 strings elsewhere in the codebase); an integer is timezone-agnostic and trivial for the dashboard to convert to local display time. **The dashboard interprets integer timestamps as Unix SECONDS** (`EvoDashWeb.Helpers.format_history_timestamp/1`) — do NOT switch granularity without updating evo_dash. Second-granularity means messages produced within the same second share a stamp — acceptable by design. Compression (`context_compression.ex:85-86`) rebuilds context as `[system, initial_user, summary]` — the fresh summary gets one timestamp at the next sync (history collapse is by design, not a bug).

### ⚠️ KNOWN ISSUE — timestamp stamping is defeated by the live agent-loop write path (as of HEAD)
The stamping guarantees above hold ONLY for direct callers of `update_agent_context/2`. In the real agent loop the stamped copy is **discarded** and a **bypass write** clobbers ETS, so the dashboard sees EITHER no timestamps OR one shared timestamp for the entire history. Verified by code reading + in-memory simulation (`mix run -e`, clock-controlled). Root cause — two interacting defects:

1. **Bypass write (tool_dispatch.ex:411):** after every LLM response, `process_llm_response/5` calls `AgentScheduler.batch_update_agent(agent_id, context: state.context, turn:, usage:, total_tokens:)` — `batch_update_agent/2` (store.ex:324-337) is a direct ETS insert that does NOT stamp. The context written here is the agent's **in-memory** `state.context`, which **never** carries `:timestamp` keys (see #2). This clobbers whatever stamped version was in ETS — so during tool execution (between :411 and the end-of-turn sync at :465; can be many seconds for subagent spawns / slow tools) the ETS history has **NO timestamps at all** → dashboard "no timestamp" symptom.
2. **Stamped copy is discarded (store.ex:354 + tool_dispatch.ex:465):** `update_agent_context/2` → `batch_update_agent(context: stamp_message_timestamps(context))` writes the stamped context to ETS but returns `:ok`; `ContextBuilder.sync_context_to_ets/2` (context_builder.ex:88-90) and `ToolDispatch` (tool_dispatch.ex:465) discard the result. The agent loop keeps its **unstamped** in-memory context, so the idempotence guard in `stamp_missing_timestamps/1` (store.ex:365-378) **never fires in the real flow** — `get_agent_context/1` has zero callers in lib (grep-verified), nothing reads the stamped ETS copy back into the loop. Consequence: every `:465` sync re-stamps the **ENTIRE history** (all messages lack `:timestamp` in memory) with a single `System.system_time(:second)` value → **ALL messages share one timestamp** → dashboard "same timestamp for all messages" symptom. The 7 `store_test.exs` stamping tests pass because they feed freshly-stamped/ETS-read contexts straight back into `update_agent_context/2` (where the guard works); no test exercises the loop's in-memory context (tool_dispatch_test.exs has zero timestamp/context-sync coverage).

**Full context-write inventory (agent state ETS table `:evogit_agent_state`):** stamped via `update_agent_context/2` at runner.ex:134 (initial sync, turn 0), runner.ex:233 (user-message injection), runner.ex:253 (top-of-loop after compression/warnings), tool_dispatch.ex:465 (end of each `{:continue, ...}` turn after tool responses); **UNSTAMPED** via `batch_update_agent(context:)` at tool_dispatch.ex:411 (after every LLM response). Context cleared: lifecycle.ex:171 (`context: nil` on crash-retry; fresh context rebuilt on re-dispatch). Completed agents: `recycle_agent/2` deletes both ETS entries, so `get_agent_history/1` returns `[]` for finished agents; the final assistant message of a completed agent is never stamped (completion path `{:complete, ...}` at tool_dispatch.ex:423-424 skips the :465 sync) — only observable in the brief window before recycle. Compression (`context_compression.ex:85-86`) rebuilds context as `[system, initial_user, summary]` — the summary is a fresh unstamped message that gets one timestamp at the next sync (history collapse is by design, not the bug).

**Fix directions** (either/both): (a) make the `:411` write go through stamping — e.g. stamp inside `batch_update_agent/2` whenever `:context` is among the fields (single choke point, no caller changes), and/or (b) return the stamped context from `update_agent_context/2`/`sync_context_to_ets/2` and feed it back into `state.context` so the in-memory copy carries timestamps and the idempotence guard actually engages across turns. Also consider `System.system_time(:millisecond)` if per-turn granularity matters (1s resolution means fast consecutive turns share a stamp even after the fix). Add an integration test that runs the loop write sequence (:411-style batch write then :465-style sync across 2 turns) and asserts per-message monotonic timestamps.
