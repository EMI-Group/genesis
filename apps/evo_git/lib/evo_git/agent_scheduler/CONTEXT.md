...
### ✅ FIXED — timestamp stamping defeated by the live agent-loop write path (store side DONE; loop-side adoption + test updates PENDING at parent level)
The original issue (documented commit 9184ebc8) had two root causes: (1) the post-LLM-response batch write at tool_dispatch.ex:411 bypassed stamping → no-timestamp window in ETS; (2) `update_agent_context/2` returned `:ok` so the loop's in-memory context stayed unstamped and every sync re-stamped the ENTIRE history with one `now` → all messages shared one timestamp.

**Store-side fix (DONE — commits 942bbb72 / 4e91a68e, in this directory's `store.ex`):** (a) `batch_update_agent/2` now stamps a `:context` field before the ETS insert — closes the :411 no-timestamp window AND the `{:complete, ...}` final-assistant-message case (that message is already in the context at the :411 write, so it gets stamped with no extra sync); (b) `update_agent_context/2` now returns the stamped `%ReqLLM.Context{}` (was `:ok`).

**Remaining cross-node work (PENDING — requires write access to sibling/parent nodes; this node's executor was framework-rejected from editing them):**
- `./apps/evo_git/lib/evo_git/agent/` — adopt the returned stamped context into in-memory `%LoopState{}.context` at the four sync sites: runner.ex:134 (initial sync), runner.ex:233 (user-message injection), runner.ex:253 (top-of-loop sync), tool_dispatch.ex:465 (end-of-turn sync, in the `{:continue, ...}` branch). Without this, the in-memory copy still carries no stamps and the "same timestamp for all messages" symptom persists (the guard never engages in the real loop). `context_builder.ex:88-90` (`sync_context_to_ets/2`) already passes the new return through — no change needed there.
- `./apps/evo_git/test/evo_git/agent_scheduler/store_test.exs` — update the 8 `:ok = Store.update_agent_context(...)` pattern matches (lines 85/105/116/126/142/154/166/176) to the new return value, and add the two-turn integration test (batch write → sync → batch write → sync) asserting: (1) ETS messages carry integer timestamps IMMEDIATELY after the batch write (no no-timestamp window); (2) pre-stamped messages keep their EXACT original timestamp across batch writes and syncs (guard engaged); (3) later-turn messages get fresh timestamps ≥ earlier ones — NOT all sharing the last sync's timestamp (use a distinctive past `old_ts` like `1_600_000_000` for determinism). Store-side behavior was verified with a standalone script before the test file could be updated (16/16 checks passed).
- `./apps/evo_git/lib/evo_git/agent_scheduler.ex:852` — stale `@spec update_agent_context(pos_integer(), ReqLLM.Context.t()) :: :ok` → `:: ReqLLM.Context.t()` (the delegate passes the new return through automatically; the annotation is now wrong).

**Expected test state until the test file is updated:** 7 `store_test.exs` failures, all `** (MatchError)` on `:ok = Store.update_agent_context(...)` — the direct, mechanical consequence of the API change. All other agent_scheduler tests pass.
