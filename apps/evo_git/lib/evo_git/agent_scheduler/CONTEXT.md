...
- **⚠️ CLI `-c`/`--concurrency` override is effectively inert against live pools**: `CLI.Parser.configure_scheduler/1` maps `-c` → `update_config(default_llm_max_concurrency: N)` (parser.ex:67) → `State.do_update_config` `maybe_update(:default_llm_max_concurrency, opts)` (state.ex:332), which changes ONLY `state.default_llm_max_concurrency`. It never touches `model_concurrency`, and since the default profile id is always present in `model_concurrency`, the fallback is never consulted in normal operation — so `-c` has no visible effect on LLM concurrency unless an agent is bound to a model_id absent from the profiles map. (`--tool-concurrency` → `update_config(max_tool_concurrency: N)` DOES take effect — tool slots read `state.max_tool_concurrency` directly, slots.ex:219.) The TOML-file path (`scheduler.default_llm_max_concurrency` + reload) works as described above; the runtime-override path does not.
