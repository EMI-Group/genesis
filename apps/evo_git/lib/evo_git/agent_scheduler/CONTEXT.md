...
3. `run_init_script/3` — Runs optional init script from `genesis.toml` (primary repo only). Accepts `opts` keyword list with `:source_worktree_path` (parent agent's worktree or repo root for top-level). Sets env vars: `SOURCE_REPO_PATH`, `TARGET_WORKTREE_PATH`, `SOURCE_WORKTREE_PATH`. Windows init scripts run via `Platform.shell_args/1` → PowerShell `-EncodedCommand` invocation (single source of truth: `EvoGit.Powershell` in lib/evo_git/powershell.ex; fixes the `-Command` re-parsing empty-output bug — script is base64-encoded so no quoting issues, output forced UTF-8).
