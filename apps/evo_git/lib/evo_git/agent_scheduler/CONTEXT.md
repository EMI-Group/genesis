...
### `scheduler.default_llm_max_concurrency` precedence (verified 2026-08, do not "simplify")
...
### Slot-Management Bug Fixes — profile-update wipe, invisible stale pools, inert default override

Three coordinated bugs were fixed in `state.ex` + `slots.ex` (verified root causes; do not re-introduce):

- **✅ Profile-update wipe (deadlock + over-grant)**: `State.do_update_config/2`'s `:model_profiles` branch previously rebuilt state via `from_model_profiles(profiles)` (no opts) and merged back ONLY the non-LLM fields — the four `llm_*` maps became the fresh empty ones. Every queued waiter's `GenServer.from` was dropped (agents block in `GenServer.call(..., :infinity)` forever — permanent hang with capacity free) and all current holders were forgotten (over-grant beyond capacity). Now the branch calls `from_model_profiles(profiles, opts)` (opts pass-through so a `:default_llm_max_concurrency` override in the same update is honored instead of being clobbered by the first profile's concurrency) and merges the OLD state's `llm_holders`/`llm_waiting`/`llm_backoff_until`/`llm_last_granted` maps OVER the fresh ones (old entries win for ids present in both; brand-new profile ids keep fresh empty entries; stale pool ids keep live entries), then `prune_empty_pools/1` drops entries with empty values (empty MapSet / empty queue / nil backoff / empty last_granted — safe because all accessors default gracefully). The trailing `Slots.grant_pending_on_resume(state)` (state.ex:399) grants under the new capacity after the merge.
- **✅ Invisible stale pools (deadlock + ghost-holder leak)**: `State.all_model_ids/1` returned only `Map.keys(model_concurrency)`, so pools for model_ids absent from `model_concurrency` (agents whose ETS `model_id` became stale after a profiles update removed their profile; agents requesting a model not in profiles) were invisible to `grant_pending_llm_slots` (both Enum.reduce sweeps), `release_agent_slots`'s LLM cleanup, and `purge_llm_waiting_queues` → queued stale-model waiters never granted; dead holders in stale pools never released (ghost-holder leak → pool fills with ghosts → all stale-model agents block). Now `all_model_ids/1` returns the UNION of keys of `model_concurrency`, `llm_holders`, `llm_waiting`, `llm_backoff_until`. Capacity for a stale pool = `concurrency_for`'s fallback (`default_llm_max_concurrency`) — correct and safe. All call sites are inside this node (slots.ex Enum.reduce sweeps).
- **✅ Race-proof LLM release**: `Slots.handle_release_llm_slot/2` resolved the model via ETS at cast-processing time (`resolve_model_id`) and deleted from ONE pool — the wrong pool when the ETS entry is gone (stale `model_id` after a profiles update). Now it scans ALL pool ids (`State.all_model_ids`), deletes the agent from every pool's holder set (idempotent — an agent holds at most one LLM slot), cleans `llm_last_granted` for the pool it was removed from (mirroring `release_agent_slots`), then runs the existing `grant_pending_llm_slots` sweep.
- **✅ Inert `default_llm_max_concurrency` override**: the generic `maybe_update(:default_llm_max_concurrency, opts)` changed only `state.default_llm_max_concurrency` — never `model_concurrency`, and since the "default" pool id is always present in `model_concurrency`, `concurrency_for/2`'s fallback was never consulted in normal operation. Replaced with a dedicated pipeline step (`maybe_apply_default_llm_concurrency/2` → public `State.apply_default_llm_concurrency_override/2`): sets the default AND applies a FLOOR to every `model_concurrency` entry (`max(c, N)`) — raising the default never lowers an explicit profile concurrency; applies whether or not `:model_profiles` is in the same update (CLI `-c` and the dashboard default-concurrency slider now raise the effective capacity of ALL live pools immediately, with AND without `[[llm.models]]` profiles).
...
There is no global `state.repo_root` fallback — repo root resolution is always per-agent. Repo-init tracking lives in `WorktreeManager` (lazy per-repo init on first create request); `%State{}` no longer carries `initialized` / `initialized_repos` fields.
