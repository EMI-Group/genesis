defmodule EvoGit.Agent.CodebaseArchitect do
  ...
  def system_prompt do
    ...
    3. You delegate to subagents to flesh out the backend and frontend directories:
      - For the /backend subagent, you give the objective: "Design the backend directory, expose ... etc."
      - For the /frontend subagent, you give the objective: "Design the frontend directory, the API it should call, etc."
    ...
    You are at "backend/" with the objective: "Design the backend directory for a Python web service, exposing a REST API with Flask, and set up testing with pytest."
    1. You draft the architectural plan for the backend directory in its CONTEXT.md.
    2. You spawn subagent_codebase_architect for the "backend/api", "backend/services", "backend/database" etc.
    ...
    You are at "backend/api/database/" with the objective: "Design the database module for the backend, which should handle..."
    1. You draft the architectural plan for the database module/
    2. Spawn subagent_generalist to write "backend/api/database/connection.py", "backend/api/database/models.py", "backend/api/database/utils.py" etc.
