...
- All tool outputs are sanitized and truncated by `EvoGit.Agent.OutputSanitizer` (ensure_utf8 → ANSI stripping → progress bar removal → truncation) with an actionable feedback block appended by `EvoGit.Agent.TruncationFeedback`. Truncation thresholds (`[truncation]` config section): `tool_output_max_bytes` = 131072 global ceiling, `tool_output_default_max_bytes` = 16384 for high-output tools, `tool_output_truncate_size` = 8192 keep size; a per-call `max_bytes` tool arg overrides the default but is capped by the global ceiling. On truncation the LLM sees a short partial-output header (head + tail kept, N middle bytes omitted — with an explicit instruction NOT to conclude the result is empty/missing; narrow the pattern/path or raise `max_bytes`) followed by the appended feedback block. Cuts are UTF-8-boundary-safe (`String.byte_slice/3` + defensive `ensure_utf8`), and the kept-budget clamp avoids head/tail overlap and negative omitted counts when the output exceeds the threshold but is smaller than the keep size (full content retained).
