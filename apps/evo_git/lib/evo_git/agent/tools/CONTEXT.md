...
- New tools must: follow module pattern (schema + execute), be aliased in `tools.ex`, have a dispatch clause, and be added to `schemas/0`. **Exception**: the self-reflective/task-control tools (`list_tasks`, `get_task`, `start_task`, `cancel_task`, `force_kill_task`, `delete_task`, `guide_user`, `subagent_investigator`, `list_recent_projects`, `system_info`) are dispatch-registered ONLY — they must NOT be added to `schemas/0` (see "Task Management & Guide Tools" below).
