defmodule EvoGit.Agent.Tools.Bash do
  ...
  On Linux and macOS, this uses bash. On Windows, this uses PowerShell.
  The tool description and prompts adapt automatically to the current platform.
  """
  ...
  defp powershell_schema do
    ReqLLM.tool(
      ...
      - Edit files: Use edit_file (not string replacement)
      - Write files: Use write_file (not Set-Content/Out-File)
      - Communication: Output directly (not Write-Host/Write-Output)
      Aside from these exceptions, you can use PowerShell to run tools for file operations:
      - Use Get-ChildItem with args for listing files with specific needs (e.g., `Get-ChildItem -Force` for detailed listing).
      - Use project level package managers (e.g., uv, npm, mix, cargo) for managing dependencies.
      ...
      - Double-quote all paths containing spaces.
      - Verify parent directories exist before creating files/folders.
      - Always use $env:TEMP for temporary files.
