...
**Touched-path collection**: `skill_add`/`skill_edit` stage the absolute path the skills API returned, made relative via `Path.relative_to(file_path, repo_path)`. `skill_enable`/`skill_disable` stage `Path.join(<node_path>, "CONTEXT.md")`. `skill_remove` collects its paths **before** mutating anything — the skill file (resolved through the same exact-then-case-insensitive filename lookup the deletion itself uses: `EvoGit.Skills.CRUD.find_skill_file/2` against `Path.join(repo_path, EvoGit.Skills.skills_dir())`, made relative via `Path.relative_to/2`, falling back to `EvoGit.Skills.skills_dir()/<name>.md` when it returns nil) plus every `CONTEXT.md` that currently enables the skill (`Enum.map(EvoGit.Skills.where_enabled(name, repo_path), &Path.join(&1, "CONTEXT.md"))`) — then stages them all in ONE commit after `remove_skill/2` + `remove_skill_from_all_contexts/2` have run. The tool layer duplicates no scan logic: whatever `where_enabled/2` enumerates is what the commit stages.
