...
**⚠️ Slot is HELD for the entire retry sequence:** `AgentScheduler.with_llm_slot/2` (agent_scheduler.ex:322-332) acquires via blocking `GenServer.call` (`request_llm_slot`, `timeout: :infinity`, agent_scheduler.ex:262-265) and releases via `after` → `release_llm_slot/1` cast (agent_scheduler.ex:277-280). The whole retry stream — outer exponential-backoff sleeps AND inner ReqLLM Retry-After sleeps — runs INSIDE the callback, so the agent never returns its slot between attempts. Worst case for a persistently rate-limited model: ~10 min slot-hold (16 outer attempts × up to 60s sleeps + inner sleeps ≈ 603s+). Only after the stream is exhausted (and the error returns / raise propagates) is the slot released. `ContextCompression` also uses `with_llm_slot` (context_compression.ex:79-103) but WITHOUT the outer retry loop — single `stream_text` (inner 3-retry only).
