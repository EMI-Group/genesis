...
- Audit of other call sites (as of this change): `tag/3`/`delete_tag/2` have **zero callers** in lib/test (dead API surface; git itself validates tag names as ref names) — left unchanged. `create_branch/3`/`add_worktree/4` branch names come from `Runtime.Helpers.generate_branch_name/1` (`genesis/agent_<hex>`, safe alphabet) or fixed test literals — safe, left unchanged. `create_pull_request/5` title/body are content-bearing but go to the `gh` CLI (a native Go binary parsing via CommandLineToArgvW, NOT MSYS2 — Erlang's Windows argv quoting round-trips embedded quotes correctly for it) — different bug class, left unchanged. Out-of-scope note: `agent/tools/context.ex` (write_context/edit_context commit path) still passes `"Update CONTEXT.md for #{dir_path}#{trailer}"` via `-m` through `EvoGit.sandbox_run` — same bug class if `dir_path` ever contains `"`; lives in a different node.
