...
CoW (copy-on-write) optimized worktree creation: create an EMPTY linked worktree (`git worktree add --no-checkout`), copy the unchanged ("shared") tracked files from a source working tree via `cp` (Linux reflink attempt, macOS clonefile), then `git checkout <commit> -- .` restores only the differing files. **Optimization with graceful fallback** — any failure calls `disable/0` (persistent-term) and returns `{:fallback, reason}` so the caller (`AgentScheduler.Worktrees.create_worktree/6`, the ONLY production call site — `worktrees.ex:110-117`) falls back to `Git.add_worktree/4` (`git worktree add -b <branch> <path> <base_sha>` = FULL checkout). **Shape-agnostic w.r.t. the Git adapter contract** — its tagged `with` patterns and catch-all `error` arms are contract-agnostic; its `{:error, code, output}` comes from its OWN internal `cp` invocations (`copy_files_linux/3` cow_worktree.ex:279, `copy_files_macos/3` :311/:325), NOT the Git adapter.

Public API: `enabled?/0` (gate — reads `[:git, :cow_worktree_creation]` config `:auto`/`:enabled`/`:disabled`, default `:auto`, schema `config/schema/definitions.ex:563`; `:auto` auto-detects ONCE as non-Windows + `cp` on PATH, caches via persistent-term), `create_worktree/5` (`(repo_root, worktree_path, target_commit, branch_name, source_path)` → `:ok` | `{:fallback, reason}`), `flag/0`/`enable/0`/`disable/0` (persistent-term flag `:evogit_cow_worktree_enabled`). `source_path` = source working tree: top-level agents copy from repo root; subagents copy from the parent agent's worktree (`Worktrees.resolve_source_path/2`).

**Algorithm** (`create_worktree/5`, cow_worktree.ex:99-179): `rev_parse` source worktree → `status --porcelain` dirty files → `diff_name_only` changed-between-commits → `ls_tree_names` all target-tree files → shared = `target ∖ changed ∖ dirty` (113-116) → `worktree add --no-checkout -b <branch> <path> <target_commit>` (185-209) → `copy_shared_files/3` then `checkout_target/2`. File lists come from `git ls-tree -r`, so **gitlink/submodule paths are excluded** (submodules arrive as empty placeholders) and dirty submodules are excluded like any file (`parse_dirty_files/1`, 359-373). Copies are batched `@batch_size 1000` — Linux: one `cp --reflink=auto --parents <files…> <wt>/` per batch (`cd: source_path`, 267-282); macOS: one `cp -c <files…> <dest>` per distinct parent dir, dirs pre-created via `File.mkdir_p` (284-337); other platforms → `{:error, -1, "unsupported platform for CoW copy"}` → fallback. The copy is SEQUENTIAL within a batch (one `cp` process); parallelism comes only from many agents each running their own `cp` concurrently.

**⚠️ No filesystem-capability probe — the "CoW" benefit is FS-dependent and silently degrades.** `enabled?/0` checks only non-Windows + `cp` on PATH; it never probes reflink/clonefile support. `cp --reflink=auto` (and macOS `cp -c`) fall back to a FULL byte copy with no error when the FS cannot reflink — notably **ZFS before OpenZFS 2.2, and ZFS 2.2+ with block cloning disabled** (the `zfs_bclone_enabled` module parameter defaults to 0 since 2.2.1; reflink only works when it is `1`), and **ext4 without the reflink patches**. On such an FS the CoW path still copies every shared tracked file byte-for-byte — with many agents fanning out this saturates the disk. btrfs/XFS (reflink) and APFS (clonefile) already give true metadata-only clones through the SAME code path, so no code change is needed there. `{:fallback, reason}` (including `:disabled`) always lands on `Git.add_worktree` (full checkout), so a filesystem-level snapshot/clone primitive can reuse the identical fallback contract.
...
**I/O profile / concurrency (how many processes per creation)**: `create_worktree/5` (cow_worktree.ex:99 — arity is 5, NOT 6) issues per creation, on Linux, ~8–10 blocking `System.cmd` OS processes plus `ceil(shared_files / 1000)` `cp` processes: (1) `git rev-parse HEAD` in `source_path` (`Git.rev_parse`, :101), (2) `git status --porcelain` in `source_path` (`Git.status`, :103 — O(working-tree) stat + hash of modified files), (3) `git diff --name-only <source_sha> <target_commit>` in `repo_root` (`Git.diff_name_only`, :105), (4) `git ls-tree -r <target_commit>` in `repo_root` (`Git.ls_tree_names`, :107 — O(total tracked files) full listing), (5) optional `git branch -D` if the branch exists (:187), (6) `git worktree add --no-checkout -b <branch> <path> <commit>` (:191 — registration only, no file writes), (7) `cp --reflink=auto --parents <batch…> <dest>/` in 1000-file chunks (`copy_files_linux/3`, :267-282 — one process per chunk, all shared files), (8) `git checkout <target_commit> -- .` (`checkout_target/2`, :237 — a FULL-tree index/stat pass; writes only the diff/dirty files). The caller (`EvoGit.AgentScheduler.Worktrees.assign_and_prepare_worktree/3`, worktrees.ex:315-317) then adds `git clean -fd` + `git checkout <branch_name>`. macOS is WORSE for spawn count: `copy_files_macos/3` (:284-328) issues ONE `cp -c` per distinct parent directory per batch (up to ~1000 spawns per batch) plus `File.mkdir_p` per dir.
...
**No serialization, no lock, no rate-limit in the adapter** — `create_worktree/5` is a pure per-call function with zero shared state. The `EvoGit.AgentScheduler.WorktreeManager` GenServer does NOT serialize creations either: `start_create/8` (worktree_manager.ex:385-421) spawns an independent `Task.start` per agent and replies `{:noreply, …}` immediately, so N concurrent agents drive N concurrent full-tree I/O pipelines. The GenServer only tracks bookkeeping/monitors.
**A single `{:fallback, reason}` disables CoW VM-wide**: every fallback arm calls `disable()` → `:persistent_term` `{EvoGit.Adapters.CowWorktree, :evogit_cow_worktree_enabled}` = `:disabled` (cow_worktree.ex:34-40, called at :140,:146,:152,:157,:166,:175), and `enabled?/0` then returns false forever until VM restart — so one transient failure (e.g. one `cp` failure) moves ALL subsequent worktrees to `Git.add_worktree` (full materialization from the object DB: reads+decompresses every blob and writes every file — far more I/O than reflink). Fallback reasons: `:no_source_head`, `:no_source_status`, `:no_changed_files`, `:no_target_tree`, `:worktree_add_failed`, `:cp_failed`, `:checkout_failed`, unsupported platform → `:cp_failed`, plus `{:fallback, :disabled}` injected by worktrees.ex:119 when CoW is off.
