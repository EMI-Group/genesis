...
CoW (copy-on-write) optimized worktree creation: create an EMPTY linked worktree (`git worktree add --no-checkout`), copy the unchanged ("shared") tracked files from a source working tree via `cp` (Linux reflink attempt, macOS clonefile), then `git checkout <commit> -- .` restores only the differing files. **Optimization with graceful fallback** — a failure returns `{:fallback, reason}` so the caller (`AgentScheduler.Worktrees.create_worktree/6`, the ONLY production call site — `worktrees.ex:110-117`) falls back to `Git.add_worktree/4` (`git worktree add -b <branch> <path> <base_sha>` = FULL checkout) for THAT creation; all fallback returns are routed through `handle_fallback/2` (see below), which disables CoW persistently ONLY for a permanent reason. **Shape-agnostic w.r.t. the Git adapter contract** — its tagged `with` patterns and catch-all `error` arms are contract-agnostic; its `{:error, code, output}` comes from its OWN internal `cp` invocations (`copy_files_linux/3` cow_worktree.ex:279, `copy_files_macos/3` :311/:325), NOT the Git adapter.
