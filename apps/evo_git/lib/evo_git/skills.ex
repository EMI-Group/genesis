defmodule EvoGit.Skills do
  ...
  def parse_yaml_simple(yaml_str) do
    lines = String.split(yaml_str, "\n") |> Enum.map(&String.trim_trailing/1)
    {result, _rest} = parse_yaml_lines(lines, %{}, :top)
    {:ok, result}
  rescue
    e in RuntimeError ->
      {:error, "Failed to parse frontmatter: #{Exception.message(e)}"}
  end

  # Top-level parsing
  defp parse_yaml_lines([], acc, _state), do: {acc, []}

  defp parse_yaml_lines([line | rest], acc, :top) do
    cond do
      line == "" or String.trim(line) == "" ->
        parse_yaml_lines(rest, acc, :top)

      String.starts_with?(line, "parameters:") ->
        parse_yaml_lines(rest, acc, :parameters)

      match = Regex.run(~r/^(\w[\w_-]*):\s*(.*)/, line) ->
        [_, key, value] = match
        parse_yaml_lines(rest, Map.put(acc, key, String.trim(value)), :top)
  ...
  defp parse_yaml_lines([line | rest], acc, :parameters) do
    cond do
      ...
      Regex.match?(~r/^-\s+name:/, line) ->
        # Start of a new parameter entry
        {param, remaining} = parse_param_entry([line | rest])
        existing = Map.get(acc, "parameters", [])
        parse_yaml_lines(remaining, Map.put(acc, "parameters", existing ++ [param]), :parameters)

      Regex.match?(~r/^\w/, line) ->
        # A non-indented line — back to top-level
        parse_yaml_lines([line | rest], acc, :top)
  ...
  # Parse a single parameter entry (lines starting with "- name:" and subsequent indented lines)
  defp parse_param_entry([first | rest]) do
    # Extract name from the first line: "  - name: param_name"
    [_, name] = Regex.run(~r/-\s+name:\s*(\S+)/, first)
    param = %{"name" => name}

    # Consume indented lines for this parameter
  ...
  defp consume_param_props([line | rest], param) do
    case Regex.run(~r/^\s+(\w[\w_-]*):\s*(.*)/, line) do
      [_, key, value] ->
        normalized =
          case key do
            "required" -> to_boolean(String.trim(value))
            "default" -> parse_yaml_value(String.trim(value))
            _ -> String.trim(value)
          end

        consume_param_props(rest, Map.put(param, key, normalized))

      nil ->
        # Not an indented property line — stop consuming this param
        {param, [line | rest]}
    end
  end

  defp consume_param_props([], param), do: {param, []}
