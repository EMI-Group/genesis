...
- **`provider_options_for_model/1` is native-OpenAI-only by design** (`[store: false]` for a native `:openai` endpoint, `[]` otherwise — see the `API Surface` entry above for what makes an endpoint non-native). Passing `provider_options: [store: false]` to ANY non-OpenAI provider raises at request time — req_llm validates `provider_options` against the provider schema (`zai`/`zai_coder`/`zai_coding_plan` accept `[:thinking]` only → `NimbleOptions.ValidationError: unknown options [:store]`). Verified empirically against `deps/req_llm` 1.24.0.
