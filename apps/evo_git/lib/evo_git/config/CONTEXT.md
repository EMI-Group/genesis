...
The `[:tmp, :*]` keys carry `category: :data` (NOT a new `:tmp` category): the dashboard Settings page hardcodes its category set (`EvoDashWeb.SettingsComponents.CategoryMetadata`), so a new category would crash its sidebar render. `:data` renders them generically with no platform filtering; the `[tmp]` TOML table itself is unaffected by the display grouping.
...
- **`provider_options` / `store: false`**: `Schema.LLM.provider_options_for_model/1` returns `[store: false]` (from `default_provider_options/0`) for a NATIVE OpenAI endpoint only — provider `:openai` AND no custom endpoint override. A model-spec MAP carrying a non-nil `:base_url` or `:extra` is an OpenAI-**compatible** third-party endpoint (presence check, so even an empty `extra` counts as an override), so e.g. an `:openai_compatible` catalog profile (`provider_atoms: [:openai]`, `requires_base_url: true` — llm_catalog.ex:189-198 — OpenRouter et al.) gets `[]`.
- `store` is a Responses-API-only field: ReqLLM materializes it only in the OpenAI Responses driver body — the shared Chat-Completions encoder (`ReqLLM.Provider.Defaults.encode_chat_body/1`) reads `provider_options` ONLY for `[:response_format]`. A profile's explicit `provider_options` override always wins over the provider-aware default (an empty override map yields no `:provider_options` key at all), and passing a `provider_options` key the provider does not declare RAISES at request time (req_llm validates against the provider schema — `zai` accepts `[:thinking]` only).
- **Generation parameters are provider/model-aware** at the single choke point `Schema.LLM.profile_generation_params/1`, which pipes its assembled keyword list through `Schema.LLMConstraints.filter/3` (declarative Z.AI/GLM constraint table + llm_db model metadata; full contract in `schema/CONTEXT.md`). The dashboard persists NO generation params unless the user typed them (an untouched profile is `id`/`model`/`concurrency` only), so the filter only ever removes parameters the user explicitly set.
- **Not every extracted key reaches the wire**: `top_k` and `reasoning_effort` are accepted ReqLLM options but are NOT encoded by the Z.AI family (`Zai.Shared.add_basic_options/2` emits only `temperature`, `top_p`, `frequency_penalty`, `presence_penalty`, `user`, `seed`, `stop`; `reasoning_effort` is telemetry-only there), whereas the OpenAI chat-completions path DOES emit `reasoning_effort` (`providers/openai/chat_api/request.ex`). `stream_options: %{include_usage: true}` is emitted unconditionally on EVERY streamed chat call by req_llm's encoders (`provider/defaults.ex`, `providers/zai/shared.ex`, `providers/openrouter.ex`, `providers/openai/chat_api/request.ex`, azure, zenmux) — it is not an accepted ReqLLM option (passing it raises `NimbleOptions.ValidationError`) and streaming cannot be turned off, so it is NOT suppressible from the config layer.
