...
**Boot-ordering note (data-dir override key feasibility)**: config.toml IS parsed before the Store child spec's data-dir computation — `EvoGit.Config.resolve()` runs unconditionally at `config/runtime.exs:75` (release boot, before any app starts, in every release incl. `genesis_remote`) and again inside `EvoGit.Application.start/2` via `EvoGit.Distribution.maybe_enable/0` (application.ex:59 → distribution.ex:34, which calls `Config.resolve()`).
Both run before the Store child spec at application.ex:80-85 evaluates `Application.get_env(:evo_git, :data_dir, EvoGit.Platform.data_dir())` (the data-dir read at application.ex:83), so a future config.toml key consulted inside `Platform.data_dir/0` or at that call site would be readable at Store-init time with no ordering problem.
The config file's own location never depends on the data dir — `Config.config_path/0` → `Config.config_dir/0` (config.ex:821-823) → `EvoGit.Platform.config_dir("genesis")` (a separate `base_dir` call keyed on `XDG_CONFIG_HOME`, platform.ex:159-161) — so a data-dir override key would create no chicken-and-egg.
`EvoGit.Platform.data_dir/0` (platform.ex:129) does NOT consult `EvoGit.Config` — it is pure OS + env via `base_dir("genesis", "XDG_DATA_HOME", ".local/share")` (platform.ex:138-140); any new TOML override must be added explicitly there.
Platform→Config runtime calls are an established, safe pattern (no compile cycle — the calls live in function bodies): `Platform.shell/0` (platform.ex:59) already calls `Config.resolve([:tools, :shell])`.
```toml
[scheduler]
