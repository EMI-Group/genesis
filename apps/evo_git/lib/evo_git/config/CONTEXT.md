...
`config_status/0` (config.ex:619-663) reports `missing: [:llm_model]` + the warning "LLM model is not configured. Set [llm] model in config.toml." (config.ex:623) when NO model profile carries a usable model. The predicate (config.ex:628-634) runs over `Schema.model_profiles(resolved)` (ALL profiles, not just the first): a profile counts only when `Map.get(profile, :model)` `is_binary` AND `!= ""` — **nil and empty-string models both fail the gate**. Because `config_status/0` calls `resolve()` first (config.ex:620) in its own process, the returned `:missing`/`:warnings`/`:validation_errors` always reflect a fresh stat-validated disk read in the calling process (never another process's `Process.get(:evo_git_config_validation_errors, [])`, config.ex:661 — that dict is set/deleted only by `resolve/0` at config.ex:170/173, and every `config_status` call re-runs `resolve` in the same process).

**Validation asymmetry (bug-relevant)**: per-profile `:model` validation (schema.ex:483-494) treats `nil` as an error ("profile must have a 'model' field", schema.ex:486) but accepts ANY binary — **`""` (empty string) passes `Schema.validate`** (schema.ex:488-489), yet still fails the `config_status` has_model gate. So a profile whose model is the empty string yields `validation_errors: []` but `missing: [:llm_model]`. The flat `[:llm, :model]` `:model_spec` validation is equally lax for strings (schema.ex:338-339: any binary accepted, no non-empty rule). `config_status`'s own check is the ONLY place empty strings are rejected. A missing `:llm` section / `llm.models = []` / flat model nil produce NO validation error (nil and empty list always pass, schema.ex:189-202, 403-410) — the `:llm_model` missing warning is the sole signal, which is by design.

**Map-model profiles also fail the gate**: `config_status`'s predicate requires a BINARY `:model` (config.ex:630-631), so a profile whose `:model` is a MAP with override keys (e.g. `%{provider: :openai, id: "x", base_url: "..."}` — kept as a map by `atomize_enum_values`'s `normalize_model_map` at config.ex:357-388 whenever `base_url`/other override keys are present; only bare `provider+id` maps collapse to a `"provider:id"` string) is fully functional for ReqLLM and the dashboard connection test (`SystemCheck.llm_test/2` accepts maps; settings_live.ex:1554 passes the raw profile `:model`) yet FAILS the has_model gate → the "LLM model is not configured" banner persists despite a working model. Pinned by config_test.exs:982-1017 (map models with overrides survive as maps).
