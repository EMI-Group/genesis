...
- **`provider_options` / `store: false` gotcha**: `Schema.LLM.provider_options_for_model/1` (llm.ex:200-203) returns `[store: false]` for ANY model whose provider atom resolves to `:openai`, but `provider_from_model/1` (llm.ex:171-191) inspects ONLY the `:provider` key — it IGNORES `:base_url`/`:extra`.
- So an `:openai_compatible` catalog profile (`provider_atoms: [:openai]`, `requires_base_url: true` — llm_catalog.ex:189-198) with a custom `base_url` (e.g. an OpenAI-compatible proxy such as OpenRouter) still inherits this OpenAI-only option.
- `store` is a Responses-API-only field: ReqLLM only materializes it in the OpenAI Responses driver body — the shared Chat-Completions encoder (`ReqLLM.Provider.Defaults.encode_chat_body/1`) reads `provider_options` ONLY for `[:response_format]` and never emits `store`; the `:openrouter` provider module (openrouter.ex) has no `store` handling at all.
- Net effect: the mis-injected option is HARMLESS on every Chat-Completions path (silently dropped) — it only reaches the wire if a profile resolves to the OpenAI Responses API (gpt-5*/gpt-4.1*/gpt-4o*/o-series ids). Fixing it (only attach `store: false` to a true api.openai.com endpoint) is a latent-correctness cleanup, not a known request-failure cause.
