...
- **Windows bash resolution is PATH-only** (`executor.ex:201-204` `System.find_executable("bash")`): the vendored MinGit that ships in a desktop release lives at `<priv>/vendor/windows-x64/mingit/bin/bash.exe` and is NOT put on PATH (nor is it resolved through `EvoGit.Executable.resolve/1`, which only knows `git`/`rg` — `executable.ex:26-34`), so on a Windows desktop install every bash skill returns the "Install Git for Windows" error even though bash is bundled; the branch degrades gracefully (no raise).
