...
## Path Resolution & Commit Ownership (the "uncommitted skill file" gap)

- `EvoGit.Skills.CRUD` performs **no git operation at all** — only `File.mkdir_p!`/`File.write`/`File.rm` (`crud.ex:64`, `:99`, `:125`, `:197`); committing is left to the caller/agent.
- The `repo_root` argument of `add_skill/4`, `edit_skill/3`, `remove_skill/2`, `list_skills/1`, `read_skill/2` and of `ContextIntegration.remove_skill_from_all_contexts/2` is the **original repo checkout, never the agent's transient worktree**: the skill tools receive the dispatch's `repo_root` (= `Process.get(:genesis_repo_root)`, set at `agent/runner.ex:212`, threaded from `agent/tool_dispatch.ex:941`), while EVERY git commit runs in the worktree (`Process.get(:repo_path)` = `<repo_root>/.genesis/workers/worker_T*`, `agent/runner.ex:256`; auto-commit fallback `git add --all` + commit in `agent_scheduler/dispatch.ex:286-298`). `Dispatch.resolve_agent_repo_root/2` (`agent_scheduler/dispatch.ex:522-557`) resolves the root by stripping the worktree suffix.
- **Known issue**: a skill file created/edited/deleted through the `skill_*` tools lands in `<repo_root>/.agents/skills/` — OUTSIDE the worktree — so no agent commit, no auto-commit fallback, no review diff and no branch merge can ever contain it; it lingers as an untracked file in the user's working tree ("skill files somehow not committed"). Not pinned by any test.
- Skills written by the agent's own tools (`write_file`, `edit_file`, `run_bash`) INTO the worktree ARE committed normally — the gap is specific to the `skill_*` tool path.
- `enable_skill/3` / `disable_skill/3` take `repo_path` (the WORKTREE) and rewrite `<repo_path>/<node_path>/CONTEXT.md` — those edits ARE committed by the agent's worktree commit; but `SkillEnable`'s existence pre-check (`agent/tools/skill/skill_enable.ex:52-55`) and `SkillRemove`'s cleanup read/write `repo_root` instead, so the tool layer mixes both roots.
- `load_skills/1` reads ONLY `<repo_root>/.agents/skills/` (`skills.ex:76-77`) — there is no global/config-dir skills path, so the skills read and write roots agree; but skills are read from the main checkout while enablement (`hierarchical_skill_names/2`) is read from the worktree.
- `.agents/` appears in NO `.gitignore` (root, `apps/evo_dash`, `desktop/src-tauri`, and the auto-written `.gitignore` of genesis-created repos which contains only `/.genesis` — `runtime.ex:29-31`), and no git-add/commit caller uses an excluding pathspec (`git add --all`; `Git.add/2` → `git add .`). A user repo whose own `.gitignore` ignores `.agents/` would additionally hide skill files from `git add --all`.
- Other writers: `EvoGit.Agents.SkillExtractor` (write scope stated in its prompt only) reached via `EvoGit.Runtime.SkillExtraction.run/1` ← task type `:extract_skills` (dashboard Review → "Extract Skills", `apps/evo_dash/.../review_live.ex:926`) writes through the same `skill_add` tools → the same gap; `merge_and_report/4` then reports "No changes detected" because the worktree stayed clean. `apps/evo_dash` contains no skill-file writer.
...
- `remove_skill_from_all_contexts/2` is the ONLY batch entry point (`context_integration.ex:232-252`): it rewrites every matching CONTEXT.md under the passed root; `skill_remove` passes `repo_root` (the main checkout), so it edits unreviewed files outside any task branch. `find_all_context_files/1` skips `.genesis/.git/_build/deps/node_modules` but relies on `Path.wildcard` without `match_dot: true`, so CONTEXT.md files under hidden directories (e.g. `.github/`) are never found or cleaned.
