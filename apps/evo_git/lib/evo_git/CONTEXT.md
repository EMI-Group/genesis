...
- **Timestamp normalization**: `EvoGit.Store.init/1` runs `Schema.normalize_timestamps(conn)` after `Schema.migrate_schema(conn)` — normalizes legacy timestamp formats in the tasks/projects tables on startup.
- **Extended summary API**: `EvoGit.Store.select_tasks_summary(store \\ __MODULE__, statuses \\ [])` and `select_tasks_summary_by_path(store \\ __MODULE__, project_path, statuses \\ [])` — `statuses` is a list of status atoms, `[]` = all statuses; the status filter is pushed down to SQL; the SELECT projection adds scalar columns `branch_name, model_id, agent_count, base_sha, commit_sha, lease_expires_at` (heavy columns logs/usage/archive_metadata stay out). Summary map keys: id, status, review_status, result, started_at, finished_at, type, project_path, opts, branch_name, model_id, agent_count, base_sha, commit_sha, lease_expires_at. Pass-throughs: `TaskRegistry.list_tasks_summary/1` + `list_tasks_summary_by_path/2`, `AgentScheduler.RemoteAPI.list_tasks_summary/1` + `list_tasks_summary_by_path/2` (no-arg forms kept for old RPC callers), `EvoGit.RemoteNode.list_tasks_summary/2` + `list_tasks_summary_by_path/3` over `:erpc`.
