...
**⚠️ Test file pending at sibling node (parent action):** `apps/evo_git/test/evo_git/powershell_test.exs` (Linux-runnable pure tests for the module above, content validated 19/19) could NOT be created from this node — the test dir is a sibling node (write-scope contract). Parent must create it; the sandbox/CONTEXT.md testing note carries a PENDING flag until then.
