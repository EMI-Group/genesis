...
- **Justified:** `safe_select_all_tasks` for `list_tasks` dashboard "show all" (API contract, offloaded to Task process); `safe_select_all_projects` (≤10 rows); the remaining `task_get` full decodes in `{:task_status}`/`{:recheck_task}`/`:DOWN` diagnostics/lease_sweep detail reads.
