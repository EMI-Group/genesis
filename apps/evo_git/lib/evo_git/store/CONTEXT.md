...
4. **`select_tasks_summary` (store.ex:607-630) decodes `result` for every row on every dashboard poll.** Consumers only need `branch_name` from result (`show_review_button?` at evo_dash projects_live/assigns.ex), and `branch_name` is ALREADY a denormalized column (codec.ex:85,100-106; store.ex:544-548). Adding `branch_name` to the summary SELECT + dropping `result` from it (dashboard change) skips `decode_result` per row per poll — result JSON can be huge (embeds archive_records). Note decode runs INLINE in the Store GenServer (Task-offload now exists for the summary handlers at task_registry.ex:416-458 — since 1140cc58 — plus the two `safe_select_*` calls at task_registry.ex:273-291, but the decode still executes in THIS Store process), so narrower columns also cut Store GenServer heap churn.
