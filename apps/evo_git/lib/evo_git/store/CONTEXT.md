...
- Classifier: `EvoGit.Store.Errors.disk_full_error?/1` (public, pure — testable). Matches `{:sqlite_failure, code, _, _}` with `code in [8, 10, 13]`, `{:read_only_database, _, _}`, PLUS a message-text fallback — `String.contains?(String.downcase(msg), "database or disk is full")` on the `:sqlite_failure` message — covering synthetic/triggered errors with primary code 1 (the deterministic test trigger below). Message reword/localization downgrades to `false` (graceful — crash as before), never a misclassification.
...
- **✅ Deterministic trigger that works as root AND in CI**: open a second xqlite connection to the same DB file, create `CREATE TRIGGER ... BEFORE INSERT ON tasks BEGIN SELECT RAISE(FAIL, 'database or disk is full'); END`, close the second connection; the Store's next `put_task` fires the trigger → `{:error, {:sqlite_failure, 1, 1, "database or disk is full"}}` → classified `:disk_full` via the message fallback. `RAISE(FAIL)` rolls back the statement, so the table stays intact and reads keep working. (Trigger on `tasks` covers `put_task`; an analogous trigger could target `projects`/DELETE/UPDATE for the other writers, though the classifier + one integration path is the recommended coverage.)
- **Recommended test layout** (test agent to create `apps/evo_git/test/evo_git/store_disk_full_test.exs`):
  1. Pure classifier unit tests — feed synthetic shapes to the PUBLIC `EvoGit.Store.Errors.disk_full_error?/1`: `{:error, {:sqlite_failure, 13, 13, msg}}` → true, code 10 → true, code 8 → true, `{:error, {:read_only_database, 8, msg}}` → true, `{:error, {:sqlite_failure, 1, 1, "database or disk is full"}}` → true (message fallback), `{:ok, _}` → false, `{:error, {:sqlite_failure, 19, 19, "constraint failed"}}` → false, nil message → false.
  2. Integration: trigger-based test asserting `EvoGit.Store.put_task` returns `{:error, :disk_full}`, the Store process stays alive (`Process.alive?`), a subsequent read (`get_task`/`select_tasks_summary`) still works, and a disk-full warning was logged (`ExUnit.CaptureLog`).
  3. TaskRegistry degradation: with the trigger armed, `TaskRegistry.start_task` (or the Store-level put_task path) does not crash the registry.
  4. Non-disk-full errors still crash: feed a synthetic non-disk-full error by... (not easily injectable without a seam — the classifier unit tests cover the mapping; the crash path is `raise MatchError` in `execute_write/4`, unreachable without a real non-disk-full NIF error, which no test currently triggers).
