...
- This enables future `json_valid`-guarded `json_extract` SQL filters over the whole column. ⚠️ DBs that have NOT run `mix migrate.store` may still contain legacy raw strings — any SQL-side JSON access must remain `json_valid`-guarded (g2 resolved for migrated DBs: `mix migrate.store` rewrites pre-existing rows).
...
- **(g2) RESOLVED for migrated DBs — `result` column is uniformly valid JSON after `mix migrate.store`.** `encode_result/1` always wraps plain strings (crash fallbacks) with a `"string"` `__result_tag__`; the migration task rewrites pre-existing rows (`json_valid(result) = 0` → string-tag wrap; scalar-valid-JSON rows → tag wrap; untagged objects/arrays left for the defensive decode branch). ⚠️ DBs that have NOT run `mix migrate.store` may still contain legacy raw strings — `decode_result/1`'s defensive branches handle them, but any SQL-side `json_extract` on `result` must remain guarded by `json_valid(result)`. JSON1 verified available at runtime (bundled SQLite 3.53.2).
