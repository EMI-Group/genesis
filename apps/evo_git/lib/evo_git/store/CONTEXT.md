...
3. **`select_cleanup_info` (store.ex:588-601) reads ALL rows; `Cleanup` (cleanup.ex:39-53) filters/sorts in Elixir.** `WHERE finished_at IS NOT NULL` is trivially safe (cleanup only ever deletes `finished_at != nil` rows). Age-cutoff and count-trim pushdown (`finished_at < ?cutoff`, `ORDER BY finished_at DESC LIMIT -1 OFFSET ?max_tasks`) are BLOCKED by the ISO-8601 mixed-precision string caveat (below) — do those only after timestamp normalization. Also `delete_tasks/2` (store.ex:434-439) issues N individual DELETEs — batch as `DELETE ... WHERE id IN (...)` with chunking (SQLite max variables ~32766; rusqlite default).
...
- **(g1) ISO-8601 strings with mixed precision break lexicographic datetime comparison.** `DateTime.to_iso8601/1` `:auto` precision emits `"…00Z"` for whole seconds but `"…00.123456Z"` with microseconds — `'Z'` (0x5A) > `'.'` (0x2E), so `"…00Z"` sorts BEFORE `"…00.5Z"` lexicographically while being chronologically AFTER. This blocks SQL-side datetime filtering/ordering pushdowns (cleanup cutoff, project recency) AND is a latent correctness caveat in the EXISTING `ORDER BY started_at DESC` pagination (same-second tasks can misorder; unstable page boundaries). Fix if ever needed: store epoch-second INTEGER columns (like `lease_expires_at` already is — SQL-safe) or `to_iso8601(precision: :microsecond)` fixed-width strings.
...
- **(g4) Quarantine/integrity is a deliberate whole-table decode** (quarantine.ex:227-253, `TaskRegistry.init` task_registry.ex:184) — corruption detection IS the decode; cannot be SQL-lowered without changing the safety model.
- **(g5) Single GenServer connection serializes all SQLite I/O** — SQL pushdown reduces bytes/decode/GC but NOT contention; the 30s `@call_timeout` (store.ex:55) exists for NFS-slow disks, where fewer+smaller statements help directly. No transactions anywhere (each execute is autocommit; quarantine INSERT+DELETE pairs are two autocommits — crash between leaves a copy in both tables, recoverable).
...
- **Justified:** `integrity_check` init scrub (deliberate; cold, per-start); `safe_select_all_tasks` for `list_tasks` dashboard "show all" (API contract, offloaded to Task process); `safe_select_all_projects` (≤10 rows); `recover_quarantine` (manual mix task).
