...
- **`store_test.exs`** — `EvoGit.StoreTest` (`async: false`, ~1300 lines): `EvoGit.Store` GenServer behavior — encode/decode round-trips, insert/update/select/delete paths, lightweight narrow reads, and the **skip-and-log semantics** for undecodable rows (`safe_select_all_tasks/1`, `safe_select_all_projects/1`: bad rows SKIPPED + `Logger.warning`, retained untouched in the live table, and the legacy `*_quarantine` tables are never created — asserted via `sqlite_master`). Note: genuinely-undecodable rows require non-TEXT-affinity values (e.g. `opts = "[1,2]"` for tasks; INTEGER `last_opened_at` in a pre-created INTEGER-affinity `projects` table) — plain bad JSON decodes to nil and is NOT skipped.
- **`store_schema_migration_test.exs`** — `EvoGit.StoreSchemaMigrationTest` (`async: false`): `Schema.normalize_timestamps/1` migration — 6-digit fractional timestamps → 3-digit (`.123456Z` → `.123Z`), whole-second → `.000Z`, already-normalized rows untouched, garbage/NULL untouched, idempotent on second run; also `Codec.encode_datetime/1` fixed millisecond precision (note: `DateTime.truncate(:millisecond)` returns precision-0 datetimes unchanged, so `~U[2024-01-01 12:00:00Z]` encodes as 20-char — the `.000Z` form comes from the migration, which is the safety net), and `Store.init/1` wiring (init runs the migration before serving reads).
- **`store_summary_test.exs`** — `EvoGit.StoreSummaryTest` (`async: false`): `Store.select_tasks_summary/0,1` + `select_tasks_summary_by_path/2,3` — exact 15-key map contract (id, status, review_status, result, started_at, finished_at, type, project_path, opts, branch_name, model_id, agent_count, base_sha, commit_sha, lease_expires_at), `[]` = all, status-atom SQL pushdown, path+status combos, scalar round-trips; chunked `delete_tasks/2` with >500 ids (1005 — exercises the 500/chunk boundary); narrow reads `select_task_logs/2` (decoded logs, `[]` for empty logs, nil for unknown id) + `select_task_update_info/2` (4-field map or nil); SQL pushdown in `select_running_lease_info/0` (running/finalizing only) + `select_cleanup_info/0` (non-nil finished_at only).
