defmodule EvoGit.Store.ErrorsTest do
  ...
  describe "disk_full_error?/1" do
    ...
    test "returns true for read_only_database error" do
      assert Errors.disk_full_error?({:error, {:read_only_database, 8, "attempt to write a readonly database"}})
    end
    ...
    test "returns true for sqlite_failure code 13 (SQLITE_FULL)" do
      assert Errors.disk_full_error?({:error, {:sqlite_failure, 13, 13, "database or disk is full"}})
    end
    ...
    test "returns true for non-disk-full code but canonical SQLITE_FULL message" do
      assert Errors.disk_full_error?({:error, {:sqlite_failure, 5, 5, "database or disk is full"}})
    end

    test "returns true for message fallback with nil extended_code" do
      assert Errors.disk_full_error?({:error, {:sqlite_failure, 5, nil, "database or disk is full"}})
    end
