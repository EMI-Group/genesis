...
- **Known lib gotcha (out of this directory's scope)**: `EvoGit.Agent.Tools.SkillWhere.execute/3` raises `ArgumentError` on its non-empty branch — it does `"..." <> Enum.map(nodes, ...)` (binary `<>` list) before `|> Enum.join("\n")` (`lib/evo_git/agent/tools/skill/skill_where.ex`). Only its "not enabled at any node" branch is exercised by tests; the worktree-visibility of skill enablement is covered through `EvoGit.Skills.where_enabled/2` instead. `where_enabled/2` renders a ROOT-level enablement as `"./."` (`Path.relative_to(base, base) == "."`), pinned by `test/evo_git/skills_hierarchical_test.exs`.
