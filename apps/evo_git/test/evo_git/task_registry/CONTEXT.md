...
1. **No restart-recovery test for a `:finalizing` or `:running` row**: **the `:finalizing` case is now COVERED** by the `describe "startup reconciliation of orphaned :finalizing tasks"` block in `persistence_test.exs:782-849` (test "restart marks an orphaned :finalizing row :failed but leaves a :running row untouched"): seeds a `:finalizing` row + a `:running` control row (both with a future lease), restarts the registry via the standard stop/start pattern, and asserts the `:finalizing` row is marked `:failed` (with `finished_at` set, `lease_expires_at` cleared, `result` a binary — pinned as the fix's init-time reconciliation via `Store.select_running_lease_info/1` + `handle_update_status/6`) while the `:running` row with a valid lease is left untouched (orphaned-`:running` sweeping remains the `:lease_sweep` path, tested in `lease_heartbeat_test.exs`). This test is EXPECTED-RED against HEAD without the fix. The restart tests in `persistence_test.exs` (`:214-251`, `:255-280`) and the backfill tests (`:104-184`, `:540-581`) only use `:completed` tasks. Closest analogues:
   ...
   - Remaining `:running`-row restart nuance (still untested): a fresh registry against a `:running` row with a valid lease leaves it untouched (pinned as the control row in the new startup-reconciliation test); expired-lease unowned `:running` rows get swept to `:failed` only by the one-shot sweep 150s after init (or `{:recheck_task, id}` resolution).
