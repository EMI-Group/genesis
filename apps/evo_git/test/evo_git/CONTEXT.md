...
Covers the whole `EvoGit.TaskTmpdir` surface: `mode/0` (all three modes + unknown value + missing config.toml → `:system`), `path_for/2` + `managed_root/1` (exact string math per mode, `to_string/1` of integer ids, no mkdir), `ensure/2` (create, idempotent, never raises on failure), `put_current/1` + `current/0` (pdict round-trip incl. the non-binary → `nil` case), `reclaim/2` (removal, idempotence, crafted `task_id` escaping the root is refused, nothing outside the root is touched) and `reclaim_stale/1` (integer `[2]` and string `["1"]` live-id forms, non-`task_*` entries kept, `:per_repo` no-op, missing root → `:ok`).
