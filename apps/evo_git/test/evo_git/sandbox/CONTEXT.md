...
- `helpers_test.exs` (`EvoGit.Sandbox.HelpersTest`) — `EvoGit.Sandbox.Helpers`: `shell_escape/1` (single-quote escaping, metacharacter safety), `truncate_output/2` (nil / under / exactly-at / over `max_bytes` but under the truncate window / full truncation with notice), `read_tempfile/2` (read-and-delete semantics, missing file, truncation), `task_tmpdir_path/0` (nil when no per-task tmpdir is installed on the calling process, else the installed path), `temp_env_vars/0` (`[]` when unset, else `[{"TMPDIR", dir}, {"TMP", dir}, {"TEMP", dir}]`), `port_env/1` (binary name/value → charlist tuples incl. a Windows-style path, the exact `temp_env_vars/0` output, already-charlist pass-through, atom/integer conversions, `[]` → `[]`, order preserved), and the `Port.open/2` charlist-env contract regression (a REAL raw port opened with `Helpers.port_env/1`-converted env succeeds with exit 0 and delivers `TMPDIR=...` to the child, plus the inverse pin that the unconverted binary env shape still raises `ArgumentError` — both reproduce on Linux, deliberately NOT `Platform.windows?/0`-gated; the pair lives behind a COMPILE-TIME `System.find_executable("bash")` guard that yields a genuine ExUnit `@tag :skip` on a host without bash), `system_cmd/2` (`{:ok, _}` / `{:error, _}` / command-not-found).
