...
### `evo_dash/`
- `store_test.exs` — `EvoDash.StoreTest` — Regression tests for the `decode_blob/1` atom-repair chokepoint. Three describe blocks: (1) DateTime round-trip (real `%DateTime{}` structs survive put/get, calendar is a real atom not `"Elixir.Calendar.ISO"`, `DateTime.compare/2` works); (2) repair of corrupted (stringified module) data — raw blobs with `"Elixir.DateTime"`/`"Elixir.Calendar.ISO"` strings written DIRECTLY into the SQLite `tasks` table via `Xqlite.open`/`XqliteNIF.execute` (bypassing the store's clean `term_to_binary`), then read back through the public API to confirm `decode_blob/1` repairs them into real atoms (via `get` and `safe_select_all`); (3) security — no arbitrary atom creation (`"Elixir." <> _` strings that are NOT real loaded modules stay as strings because repair uses `String.to_existing_atom/1` only; non-Elixir-prefixed strings are never touched). Uses an isolated TaskStore with a unique tmp SQLite path (`async: false`; terminates and restarts production children).
- `task_registry_test.exs` — `EvoDash.TaskRegistryTest` — Covers cleanup_expired_tasks (age/count limits, running/pending preservation), set_review_metadata, TaskInfo field backfill (normalize_tasks backfills base_sha/commit_sha as nil), persistence (get_task retrieves seeded tasks; tasks survive registry restart with the same store), recent projects round-trip persistence (project survives registry restart with same store), projects table schema migration (self-heals a stale `path`-PK schema to `id` via TaskStore init repair, migrating rows), TaskStore.integrity_check (healthy store → :ok; undecodable TASKS rows hard-deleted + repaired count; undecodable PROJECTS rows QUARANTINED — raw blob preserved in `projects_quarantine`, not destroyed), and GenServer resilience (registry stays alive and preserves task state across mutation operations that trigger cleanup). Uses an isolated TaskRegistry + TaskStore (unique temp root + SQLite database file; terminates production children and restarts them in `on_exit`, `async: false`).
...
- All task persistence in tests goes through `EvoDash.Store` (SQLite-backed, namespaced `{:task, id}` keys) — no DETS or CubDB is used.
