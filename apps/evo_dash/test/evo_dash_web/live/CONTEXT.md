...
- `./home_live_test.exs` → Home chat page (`GET /help`, `:reflect` self-reflective agent) — full suite notes in `../CONTEXT.md`; the "send message" describe includes a regression test asserting a REAL `render_submit` sets `chat_task_id` synchronously (non-nil, matching the persisted reflect row) and renders NO "Failed to start the task" bubble; the "model selector" describe (5 tests) covers the per-chat model `<select>` (`select_chat_model` event → `selected_model_id` assign, threaded as `"model_id"`/`"model_id_locked"` STRING-keyed opts — the Store codec does NOT whitelist them, see `projects_live_test.exs` "model selection auto/lock semantics"), Auto-threads-neither-key, and remount persistence. NOTE: 3 of the 5 (select renders, pinned threading, remount restore) depend on the `home_live.ex` wiring (event handler + header select) that landed AFTER these tests — they fail/error until then; the other 2 (hidden-when-no-profiles, Auto-threads-neither-key) pass against old wiring
