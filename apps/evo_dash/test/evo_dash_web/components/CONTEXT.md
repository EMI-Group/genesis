...
- `task_card_components_test.exs` → `EvoDashWeb.TaskCardComponentsTest` — task-card affordances incl. cancel/force-kill button visibility + the failed-task structured-error display (`describe "task_card/1 — failed-task structured error display"`): collapsed error strip (truncated `error.message`, kind-label fallback), expanded detail block (kind + "Source: …" caption chips via `Helpers.task_error_kind_label/1`/`task_error_source_label/1`, full message, last ≤8 stacktrace frames in a `<pre>`), legacy `{:error,_}` result box coexistence, `error: nil`/non-map/16-key-summary-map safety, and the `:failed`-never-renders-a-Review-button pin + the failed-task structured-error display (`describe "task_card/1 — failed-task structured error display"`): collapsed error strip (truncated `error.message`, kind-label fallback), expanded detail block (kind + "Source: …" caption chips via `Helpers.task_error_kind_label/1`/`task_error_source_label/1`, full message, last ≤8 stacktrace frames in a `<pre>`), legacy `{:error,_}` result box coexistence, `error: nil`/non-map/16-key-summary-map safety, and the `:failed`-never-renders-a-Review-button pin
