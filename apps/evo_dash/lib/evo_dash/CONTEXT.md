...
2. **`handle_info({:DOWN, ref, :process, pid, reason})` (line 899/913)** — the process-down handler for the wrapper. Guarded by `sched_meta_has_active_agents?(task_id)`. **This is the prime suspect for the mid-run `:failed` bug.** When `reason != :normal` AND `sched_meta_has_active_agents?(task_id)` returns **false**, the task is marked `:failed` (line 931).
   - ⚠️ **IMPORTANT (corrected)**: After an abnormal wrapper exit, NO `{ref, result}` message is ever delivered — `Task.Supervisor.async_nolink` only sends the result message when the task function returns *normally*; an abnormal exit delivers ONLY `:DOWN {reason}`. Therefore site #1 (`handle_info({ref, result})`) never fires in the abnormal-exit case, and there is no "late `{:ok,_}` that gets discarded". The task simply STAYS `:failed` permanently. (A prior version of this doc incorrectly claimed a `{:ok,_}` "then arrives and is rejected as stale" — that is impossible given the `async_nolink` contract.)
   - ⚠️ **Also note the cast guard does NOT block `:failed → :completed`**: even in the hypothetical case where a `:completed` cast DID arrive after `:failed`, the stale-terminal guard at `handle_cast` lines 318-321 ends with `status != :completed`; for an incoming `:completed` that clause is `false`, so the whole guard is `false` and the update would be ALLOWED (flipping `:failed → :completed`). The guard only blocks non-`:completed` terminal transitions (e.g. `:failed → :cancelled`).
...
**Likely root cause of the observed bug:** The wrapper `Task.Supervisor.async_nolink` process (started at line 142) exits **abnormally** (non-`:normal` reason). When this happens: (1) `Task.Supervisor.async_nolink` delivers ONLY a `:DOWN {reason}` message — never a `{ref, result}` message; (2) the `:DOWN` handler runs `sched_meta_has_active_agents?(task_id)`; if that returns **false** (race window — the agent's `SchedMeta` ETS entry was already torn down, OR the table doesn't exist), the task is marked `:failed` at line 931 and STAYS `:failed` (no later result can repair it). The bug is therefore: *abnormal wrapper exit + a false `sched_meta_has_active_agents?` check*. The critical question for the evo_git side is **WHY the wrapper `async_nolink` process exits abnormally mid-run** (e.g. linked process death, supervisor shutdown, memory limits, or the runtime itself exiting before returning a normal `{:ok,_}`).

**Why the bug is intermittent / "random":** it requires the coincidence of (a) an abnormal wrapper exit AND (b) `sched_meta_has_active_agents?` returning false at that exact instant. When agents are genuinely still active in ETS, the guard correctly keeps the task `:running`, so only certain timing/orderings produce the spurious `:failed`.
