...
**Remaining candidates (blockers in parens):** (5) remote-poll dirty-check (needs `updated_at` column via `migrate_schema`) — TasksLive `:remote_poll` (`tasks_live.ex:351-362`) intentionally still transfers 25 FULL TaskInfo structs + COUNT every 3s (expandable cards need archive_metadata/logs/usage — investigated, NOT switchable to summaries); AgentsLive `:remote_poll` re-transfers agent summaries; the sidebar has NO poll (broadcast/debounce-driven); (6) raw archive JSON export for `TaskExportController` (decoded then re-encoded; a raw-column read would serve the stored JSON directly); (8) dead `tasks` attr passed to `Layouts.app` (declared but unused — noted by live/ sub-manager). `get_unique_paths` re-fetch (`tasks_live.ex:629`) and recent-projects sort are negligible.
