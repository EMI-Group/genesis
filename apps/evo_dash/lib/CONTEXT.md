...
- `./evo_dash/` → Domain modules: `Application` (OTP supervisor), `NodeContext` (SSH remote-development thin client), `DirectoryPicker` (+ `Wx` seam), `UpdateStatus` (auto-update hub), `DesktopLifetime` (desktop shell watcher), `ConnectionDiagnostics` (connection peer-address logger metadata), `ChatHistory` (in-memory ETS-backed chat store), `ActiveTasks` (sidebar last-known-state hub — pure module over a boot-created ETS table, no process), `AttachedFile`, `MarkdownRender`, `SettingsUtils`
...
| Module | Purpose |
|--------|---------|
| `EvoDash.Application` | OTP supervisor tree (Telemetry → PubSub → TaskSupervisor → ChatHistory → DirectoryPicker → UpdateStatus → Endpoint; `DesktopLifetime` appended when `EVOGIT_DESKTOP=1` + `EVOGIT_LIFETIME_PORT`). Boots the `:evo_dash_active_tasks` ETS table (idempotent `ensure_ets_table/2`, owned by the application process, NOT a supervised child) before starting the supervisor. `start/2` also calls `EvoDash.ConnectionDiagnostics.attach/0` (non-fatal; not a supervised child, so the children list is unchanged). Store/Registry/TaskRegistry live in `EvoGit.Application`. |
...
| `EvoDash.ConnectionDiagnostics` | Connection peer-address diagnostics — attaches a `:telemetry` handler on `[:thousand_island, :connection, :start]` that copies the span's `remote_address`/`remote_port` onto the connection process's `Logger.metadata` (`:remote_ip`/`:remote_port`), so every later log line of that connection (incl. the `** (Bandit.HTTPError) Read timeout`) carries the peer. `attach/0` is idempotent + non-fatal; wired from `EvoDash.Application.start/2`. The keys appear in output only when the root formatter's `metadata:` list includes `:remote_ip`/`:remote_port` (`config/config.exs`). |
...
The core's ONLY persistence-injection hook is a `task_store:` **start opt** on `EvoGit.TaskRegistry` (default `EvoGit.Store`) — it swaps the backing Store but the registry process is still reached only via the global name (detail: `EvoGit.TaskRegistry.start_link/1` in `apps/evo_git/lib/evo_git/task_registry.ex`).
