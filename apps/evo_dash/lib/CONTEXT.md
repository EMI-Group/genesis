...
|----------|-------|---------|
| `ProjectsLive` | `GET /` | Projects page: project tabs, task form, task cards, project settings |
...
|-------|--------|-------------|
| `"tasks"` | `{:tasks_updated}` | TaskRegistry, ProjectsLive |
...
- **Sidebar "Active Tasks" is SQL-filtered + debounced + deduped + UNIFIED (was the biggest hotspot)**: a SINGLE implementation in `EvoDashWeb.LiveHooks.NodeAware` (`fetch_active_tasks/1` node-aware fetch with pending-remote guard → `partition_active_tasks/1` pure → `assign_active_tasks/1` assign; `load_running_and_pending_tasks/1` delegating entry) fetches ONLY `@active_statuses = [:running, :pending, :finalizing, :completed]` via `TaskRegistry.list_tasks_summary(statuses)` (local) / `NodeContext.list_tasks_summary(node, statuses)` (remote RPC) — the SQL statuses-IN WHERE clause (parallel evo_git work) skips finished tasks that can never appear in the sidebar. The old `Assigns.assign_running_and_pending_tasks/1` duplicate (local-only, logic-identical) was DELETED and its 8 ProjectsLive call sites were removed/rerouted. Broadcast bursts are coalesced by a 300ms trailing debounce (`:node_aware_reload_tasks` + `:tasks_reload_pending` flag) — one reload per burst per LiveView, not one per broadcast. `assign_node/2` skips the reload when the node context is unchanged (`:tasks_node_loaded` guard) — kills the mount double-fetch AND pagination/palette push_patch re-fetches. **Dead-render skip**: `on_mount/4` fires the sidebar load ONLY on the connected mount (dead HTTP render keeps empty-list assigns, no query). Note `list_tasks_summary` decodes on short-lived Task heaps (task_registry.ex:416-427, Task-delegated) — the LiveView heap retention driver (`result` decode) is gone since `show_review_button?/1` is column-based (`branch_name`) and the parallel evo_git work drops `result` from the summary projection.
- **ProjectsLive main list REMOVED (Tasks A-D)**: the dashboard's `@tasks` main list and `Assigns.current_tasks/1` are DELETED — no unfiltered task-list fetch remains in dashboard code (round 1 had made it summary-backed via `list_tasks_summary()` / `list_tasks_summary_by_path(path)`; `lightweight_task/1` and `Assigns.strip_heavy_fields/1` were REMOVED in round 1, fixing the latent bug where `lightweight_task` stripped `result` → dashboard Review buttons / full-result modal were broken). The `notified_task_ids` notification path is now a **minimal id-only projection**: `Assigns.build_notified_task_ids/1` uses `TaskRegistry.list_task_ids([:completed, :failed, :cancelled])` (id+status+updated_at only — no result/opts JSON decode; supersedes the round-1 `cf7d1fb7` `list_tasks_summary()` switch). Template audit: dashboard cards need ONLY summary-contract fields (`components/CONTEXT.md` audit table) — NO lazy `get_task(id)` detail load needed for rendering; `get_task/1` is used ONLY per newly-terminal id for browser notifications; archive UI remains Tasks/Review-page-only.
- **`Assigns.assign_running_and_pending_tasks/2` REMOVED (Tasks A-D)**: the 2-arity variant (which filtered a caller-passed `all_tasks` instead of re-calling `list_tasks_summary()`) is DELETED — the dashboard no longer holds an unfiltered `@tasks` list to filter. The `/1` variant was the sidebar path until the UNIFICATION removed it too: the sidebar now uses the single `EvoDashWeb.LiveHooks.NodeAware` loader (`fetch_active_tasks/1` + `partition_active_tasks/1` + `assign_active_tasks/1`, `@active_statuses`), and `Assigns.assign_running_and_pending_tasks/1` + `Assigns.show_review_button?/1` were deleted (assigns.ex keeps only `build_notified_task_ids/1` + `assign_form_defaults/1`).
...
- **ProjectsLive `:tasks` REMOVED (Tasks A-D)** — the dashboard no longer retains any unfiltered summary-map list (previously it was the FULL 16-key summary-map list incl. decoded `result` JSON — `Codec.decode_result` reconstructs the `{:ok, %{...}}` tuple with embedded `%Usage{}` and archive_records; see `store.ex:58` + `store.ex:987-1023` — re-fetched on EVERY task-broadcast burst). The only task data ProjectsLive retains is `:running_tasks`/`:pending_tasks` (statuses-filtered summaries) and the `notified_task_ids` MapSet (ids only; see the "ProjectsLive `@tasks` is GONE" bullet below).
