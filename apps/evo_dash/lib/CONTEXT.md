...
- **TasksLive remote-poll dirty-check**: TasksLive `:remote_poll` transfers ONLY the lightweight changed-since summaries (usually `[]`) per 3s tick and reloads the full page only when `EvoDashWeb.TasksLive.DirtyTracker` reports a change. The page renders FULL TaskInfo (expandable cards need archive_metadata/logs/usage — never degraded to summaries); a periodic full re-sync every 10 ticks (~30s) bounds deletion staleness (changed-since cannot detect deletions). Local nodes use the PubSub broadcast path. AgentsLive `:remote_poll` re-transfers agent summaries; the sidebar has NO poll (broadcast/debounce-driven).
...
- **Inline decode on the registry heap**: `list_tasks_summary*`/`list_tasks_changed_since` decode ALL rows inside the TaskRegistry GenServer (inline handler, task_registry.ex) — each call transiently materializes the full decoded list on the evo_git GenServer heap, then hands the maps to the LiveView which retains them. Both heaps scale with DB size.
...
- **TasksLive remote dirty-tracker baselines are minimal-projection**: both call sites — `seed_dirty_tracker/1` (`tasks_live.ex:767`) and the `:remote_poll` belt-and-braces reseed (`tasks_live.ex:439`) — use `EvoDash.NodeContext.list_task_ids(current_node)` (minimal id/status/updated_at projection via `EvoGit.RemoteNode.list_task_ids/2`, no result/opts JSON decode). A node switch does not trigger a full scan + full decode on the remote node.
