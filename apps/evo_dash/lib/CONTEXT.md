...
**Remaining candidates (blockers in parens):** (6) raw archive JSON export for `TaskExportController` (decoded then re-encoded; a raw-column read would serve the stored JSON directly); (8) dead dashboard `@tasks` machinery — `tasks` attr passed to `Layouts.app` (declared but unused), dead `view_full_result`/`view_full_options`/`toggle_task_details` events, unused `expanded_task_ids` render — the source of the only remaining UNFILTERED full-scan summary fetch on every broadcast burst (see Memory Profile: DashboardLive `@tasks` is DEAD; fix = remove the dead assign + minimal id/status query for `notified_task_ids`, NOT a statuses filter). `get_unique_paths` re-fetch and recent-projects sort are negligible.
...
- **DashboardLive `@tasks` is DEAD (biggest remaining avoidable full-scan, added by investigator T1-A2)**: Since the sidebar redesign (`208a0891`, "Redesign layout from horizontal navbar to collapsible left sidebar") NO UI renders the dashboard task list: the inline task-card markup was removed, `Layouts.app` declares `attr(:tasks, :list, default: [])` but NEVER uses it (`layouts.ex:40`; grep `@tasks` in layouts.ex = no matches), and NO heex file anywhere triggers the surviving event handlers `view_full_result` / `view_full_options` / `toggle_task_details` (dead handlers at `dashboard_live.ex:988-1017` + `ModalHelpers.view_full_result/2` injected via `use EvoDashWeb.ModalHelpers`; `expanded_task_ids` is assigned but never rendered). Yet the UNFILTERED fetch still runs on: mount (`notified_task_ids`, `dashboard_live.ex:479` — needs only id+status of terminal tasks), `handle_params` fallbacks (`560,583,595` — ALL rows when no project), `activate_project` (`930,1482` — by_path), task start/cancel/delete/clear (`930,969,1035,1022`), and every task-broadcast burst via `Assigns.current_tasks/1` (`assigns.ex:113-119` → unfiltered `list_tasks_summary()` / `list_tasks_summary_by_path(path)`; debounce → `dashboard_live.ex:1267`). Live consumers of `@tasks`: (a) `notified_task_ids` + notification detection (id+status of [:completed,:failed,:cancelled] only), (b) `Project.task_notification_content/1` (opts+result of terminal tasks, `project.ex:209-231`), (c) the dead modal handlers. **Fix direction**: drop `@tasks` from the dashboard (delete the dead events + the `tasks` attr), keep the sidebar on the statuses-filtered query, serve `notified_task_ids` from a minimal query (id+status; or `list_tasks_changed_since`). ⚠ A plain statuses filter CANNOT fix `current_tasks/1` (its needs = terminal-status notifications ∪ sidebar statuses = ALL statuses) — the fix is removing the dead assign, not filtering it.
- **Result-blob display audit (investigator T1-A2)**: the heavy per-row `result` decode in summary queries is displayed NOWHERE in dashboard UI. The sidebar uses only `result.branch_name` PRESENCE (`show_review_button?`, `assigns.ex:75` / `node_aware.ex:101` — pattern match, value not rendered); browser notifications use `result` (pr_title/error/exit) only for terminal tasks; the "full result" modal (`render_result_full`) is dead. TasksLive/ReviewLive consume full structs via `list_tasks_paginated`/`get_task`, NOT the summary query. So per-row `Codec.decode_result` (rebuilds embedded `%Usage{}` + archive_records — the largest blob in the DB) is essentially wasted work for the current dashboard UI; only branch_name presence + terminal-task pr_title/error text are ever read from it. (Dropping `result` from the summary projection would require re-sourcing `show_review_button?` and notification text — e.g. via review_status/branch_name columns or a targeted query.)
- **TasksLive remote `seed_dirty_tracker` calls UNFILTERED `list_tasks_summary(node)`** (`tasks_live.ex:767`) once per node switch (remote nodes only) — the tracker baseline needs only id + updated_at; an unfiltered call = full scan + full result/opts decode on the remote node. Could pass statuses or (better) a minimal id/updated_at projection. Minor (once per node switch).
- **Git history (design intent, investigator T1-A2)**: evo_git `ec48b124` (2026-07-31) introduced `select_tasks_summary` as a full scan with NO WHERE/LIMIT/ORDER BY and decoded `result` from day one (the commit message's "no heavy JSON fields" claim is inaccurate — result is the heaviest field). The statuses arg + SQL pushdown arrived in evo_git `c196be51`. evo_dash never filtered the dashboard main list by status and never paginated it; the dashboard evolution was: (a) server-side pagination added to the Tasks page (`f27b4d75` LIMIT/OFFSET), (b) active tasks moved into the sidebar (`07d201c4`/`2da36db0`), (c) dashboard task-card list REMOVED in `208a0891` while the unfiltered `@tasks` fetch was KEPT (re-carried through the summary-maps migration `751e399e`), (d) ~30MB binary retention mitigated earlier by post-fetch stripping (`6e8d2ff4`), (e) sidebar statuses-filtered only (`e03f5bc0`).
