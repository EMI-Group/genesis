...
- **task_submit opts contract** (`projects_live.ex:997-1115`): the handler pattern-matches `%{"prompt" => prompt, "mode" => combined_mode}` and builds the start_task opts as `[path: active_project_path, mode: <"new"|"existing"|"simple">]` (mode mapped from `genesis_new`/`genesis_existing`/`evolve_simple`), then conditionally adds: `:prompt` (genesis — trimmed textarea text) XOR `:objective` (evolve — same trimmed text), `:build_system` (genesis, `String.to_existing_atom`), `:node_path` + `:starting_commit` + `:resume_from` (evolve, non-blank only), `:foreign_repos` (from `@foreign_repos` assign when non-empty), `:archive` (true when checkbox), `:model_id` (from `@selected_model_id`). Dispatched as `NodeContext.start_task(node, task_type, opts)` when `@remote?` else `TaskRegistry.start_task(task_type, opts)` (lines 1077-1082); `NodeContext.start_task/3` (`node_context.ex:517-519`) is a thin delegate to `EvoGit.RemoteNode.start_task/3`. Other dashboard start_task call sites: `merge_check.ex:128` (auto-resolve: `:evolve` with hardcoded `:objective` + `:starting_commit`/`:merge_from`/`:merge_target`) and `review_live.ex:685` (`:extract_skills`). Objective display reads `opts[:prompt] || opts[:objective]` (`task_card_components.ex:94` and `:489`; `review_live.ex:821`).
