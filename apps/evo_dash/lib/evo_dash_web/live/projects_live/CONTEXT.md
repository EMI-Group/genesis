...
| Sidebar Active Tasks (NodeAware, unified) | `TaskRegistry.list_tasks_summary(@active_statuses)` (local) / `NodeContext.list_tasks_summary(node, @active_statuses)` (remote RPC) | YES — blocks on `GenServer.call` reply | **Unified in `EvoDashWeb.LiveHooks.NodeAware`** (`fetch_active_tasks/1` → `partition_active_tasks/1` → `assign_active_tasks/1`; `load_running_and_pending_tasks/1` delegating entry). The old local-only `Assigns.assign_running_and_pending_tasks/1` duplicate (assigns.ex:36-53) is DELETED. Decode is Task-delegated (task_registry.ex:416-427) but the LiveView blocks and receives the FULL decoded list. `result` is NOT read from summary maps — `show_review_button?/1` is column-based (`branch_name`), so the parallel evo_git change can drop `result` from the projection. NOT project-scoped — scans all projects' rows in those 4 statuses. |
...
2. **ProjectsLive.mount**: `list_recent_projects()` (SQLite) + `Config.resolve()` + `build_systems()` (module attr, pure) + `list_task_ids` #1 + deferred `:load_config_status`. **The old duplicate `Assigns.assign_running_and_pending_tasks/1` (summary #2) is REMOVED** — ProjectsLive has no sidebar loader of its own anymore; the unified NodeAware loader is the single path.
3. **handle_params**: `assign_node/2`'s summary reload is dedup-guarded by `:tasks_node_loaded` (node_aware.ex:175-183 — kills the mount double-fetch AND the dead-render re-fetch; ProjectsLive's fallback branches no longer call any summary loader — the redundant `Assigns.assign_running_and_pending_tasks` calls at 635/665/674 were deleted, keeping only the `list_task_ids` notified-set union); with a project → `activate_project` (File.dir? + detect_mode FS + **genesis.toml read** + `list_task_ids`; the redundant summary reload at the end of activate_project was deleted).
...
⇒ **A single full page load fires `list_tasks_summary` exactly ONCE** (the connected mount; the dead render skips it, ProjectsLive no longer duplicates it, and `assign_node`'s dedup guard suppresses the handle_params re-fetch). Each call blocks the LiveView while the single-connection Store GenServer runs the query and the Task process decodes rows. SPA-style push_patch navigation re-runs only handle_params (dedup-guarded: NO summary, just task_ids + recents + genesis.toml read when a project is active, unless the node context changed).

### Memory (~25MB/page)

Retained in the LiveView heap per tab (N tabs = N copies): `@running_tasks` + `@pending_tasks` — the statuses-filtered summary maps. **`result` is no longer decoded/read from summary maps** — `show_review_button?` is column-based (`branch_name`), so the parallel evo_git change drops `result` from the summary projection (killing the per-row `Codec.decode_result` that rebuilt `%Usage{}` + archive_records — the largest blob in the DB — for every completed row; this was the ~25MB retention driver). `pending_tasks` = every completed-unreviewed-with-branch task (full summary map). Small retainers: `@recent_projects`, `@notified_task_ids` (ids only), `@model_profiles`, `@project_config`/`@commands`/`@foreign_repos`, `@config_status`. LiveView processes live until the tab closes (no `live_disconnect_timeout` in LV 1.2.8).

### Wasteful patterns to fix (candidate list)

1. **Duplicate sidebar fetch — DONE**: NodeAware on_mount (connected mount only, dead-render skipped) is now the SINGLE summary query per page load; the `Assigns.assign_running_and_pending_tasks/1` duplicate and its 8 ProjectsLive call sites were removed/rerouted (mount:520, handle_params fallbacks 635/665/674, activate_project:1846 deleted as redundant with on_mount/assign_node dedup + broadcast-driven debounced reloads; task_submit/cancel/delete/clear rerouted through the unified `NodeAware.assign_active_tasks/1`).
2. **`result` decoded per summary row but barely used — DONE (evo_dash side)**: `show_review_button?/1` is re-sourced to the denormalized `branch_name` column (single public impl in NodeAware); the parallel evo_git change drops `result` from the summary projection. Notification text (`pr_title`/error/exit) still comes from FULL rows fetched per newly-terminal id (`TaskRegistry.get_task/1`) — unaffected.
