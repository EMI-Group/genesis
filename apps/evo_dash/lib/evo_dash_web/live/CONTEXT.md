...
## Notes for Agents — Session-memory messages carry NO timestamps

Investigated (2025): the agents-page chat history (`AgentsLive` → `messages_to_history_entries/1` → `message_to_history_entry/1`) consumes `%ReqLLM.Message{}` structs from `EvoGit.AgentScheduler.RemoteAPI.get_agent_history/1`, which returns the raw `AgentState.context.messages` list.

**There is NO per-message timestamp anywhere in the lifecycle** — verified at every candidate location:
- `%ReqLLM.Message{}` struct (defined in the `req_llm` dep, `deps/req_llm/lib/req_llm/message.ex:108-121`): exactly 7 fields — `role, content, name, tool_call_id, tool_calls, metadata, reasoning_details`. No `timestamp`/`time`/`created_at`/`inserted_at`.
- `:metadata` is a free-form map (Zoi.map), but the ONLY keys ever written are `:turn` (via `EvoGit.Agent.ContextBuilder.tag_message_turn/2`, `apps/evo_git/lib/evo_git/agent/context_builder.ex:109-111`) and `:tool_name`. ReqLLM's own `build_message_metadata/1` (deps/req_llm/.../response_builder.ex:376-398) adds only `:response_id`/`:phase`/`:phase_items` on assistant messages — no time.
- Message creation sites in `:evo_git` (runner.ex, tool_dispatch.ex, context_compression.ex) set no time fields. The only time-related hits in the agent dirs are agent-level (LLM-call duration via `System.monotonic_time`, `:evogit_started_at` process dict, archive `completed_at`) — never per-message.
- `AgentState` (agent_scheduler/agent_state.ex:40-62) has a `turn` integer but no per-message timestamps.

**Implication for a "show message timestamps" UI feature:** it requires adding a timestamp at the SOURCE in the `:evo_git` app (read-only for evo_dash agents — escalate). Feasible approach: mirror `tag_message_turn/2` with a `tag_message_time/2` (metadata is forward-compatible, no dep change). Caveats: (1) not every message passes through turn-tagging (nudge messages, compression summaries are untagged), so stamping at a single choke point like `Store.update_agent_context/2` (store.ex:325-345) is more complete but refreshes timestamps on every context sync unless guarded; (2) context compression REPLACES history with a summary (context_compression.ex:85-86), so old per-message timestamps are lost there.
