...
- `simple_live/` → Simple-mode LiveViews (`SimpleLive.Home` — the default `/` minimal task-input home)
...
|--------|-------|---------|
| `SimpleLive.Home` | `GET /` | **Simple-mode home (default landing page)** — step 2 of the minimal flow (`/welcome` API setup → `/` task input → `/tree` agent tree). Two centered mode tabs above the auto-growing 3-line input box: **开发新软件 (new, default)** — one 开发路径 field (existing dir opens; a bare name auto-creates under `~/GenesisProjects`); **重构已有软件 (refactor)** — two fields: 原软件路径 (must exist) + 新软件路径 (optional; empty → in-place evolve, given → genesis at the new dir with the original registered as a read-only foreign repo). Prompt placeholder is just "请输入开发需求". Top-left brand corner (EVOX logo + 天演·启元 / EvoX Genesis, white variant under dark themes), standalone start button on the right, recent-project chips, path fields with `Project.path_suggestions/1` completion. Launch → `TaskRegistry.start_task/2` → `/tree`. First-open redirects to `/welcome` (same dead-render `VersionState.onboarding_needed?/0` check as `DashboardLive`). Bare `Layouts.simple` shell + `Layouts.pro_corner`. |
...
| `WelcomeLive` | `GET /welcome` | Simple-mode onboarding (step 1): minimal single-column LLM setup in the bare `Layouts.simple` shell — search box, provider-grouped preset model grid, API key entry with merged one-click save (`save_welcome_setup`). Skip/Get-started go through `/welcome/complete` → `/`. |
