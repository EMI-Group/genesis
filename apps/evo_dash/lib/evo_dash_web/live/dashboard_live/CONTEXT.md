...
**JS DirectoryPicker fallback (parallel work — DONE, commit `3ca5871f`, in HEAD)**: File System Access API browser picks yield only `handle.name` (never a full path) — the hook now warns and routes to the manual-entry fallback instead of filling the input with a bare name; Tauri native picks (custom `pick_directory` command, then plugin-dialog fallback) return full absolute paths and are additionally guarded by an absolute-path regex in `applyTauriResult` before filling/auto-submitting. **Regression tests (DONE, commits `d9149aba`/`db5a0d34`)**: unit tests for `normalize_project_path/1` + LiveView tests for relative-input rejection live under `test/`.
