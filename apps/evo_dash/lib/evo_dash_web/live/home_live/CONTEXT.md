...
- **Real-flow streaming bug (`home_live.ex:395`)**: the `agent_updated` clause gates history refetch on `:message_count in changed_fields` — atom membership in a list. Real core payloads carry `changed_fields` as a KEYWORD LIST `[message_count: n, total_tokens: n, ...]` (`EvoGit.AgentScheduler.Store.put_agent_state` prepends `[message_count: n]` to the strip_heavy_fields diff), so the membership test is ALWAYS false in production — `agent_updated` never triggers `async_fetch_history`; streaming degrades to the `{:agents_updated, ...}` fallback and the terminal-task path (bubbles still fill, but via lookup/terminal fetches). The suite (`home_live_test.exs`) pumps fake atom lists `[:message_count]`, so the 23 tests pass while production differs. Fix shape: `Keyword.has_key?(changed_fields, :message_count)` — the pattern `AgentsLive` already uses (agents_live.ex:330).
- **Test-vs-real payload delta** (why the suite misses real-flow issues): the suite uses STRING agent ids (`"agent-1"`), atom-list `changed_fields`, and plain-map histories; the real core broadcasts INTEGER agent ids, keyword-list changed_fields, a 6-key registered summary (`%{status:, depth:, parent_id:, task_id:, task_number:, objective:}`), and `agent_updated` BEFORE `agent_registered` for a new agent (store.ex insert path); `RemoteAPI.get_agent_history/1` returns native `%ReqLLM.Message{}` struct lists (roles system/user/assistant/tool; content parts :text/:thinking; tool_calls/reasoning_details nil-or-list; metadata `%{turn, timestamp}` or nil). WIP repro files covering the real shapes (all PASS — no crash reproduced in the test env): `test/evo_dash_web/live/home_repro_test.exs`, `home_repro2_test.exs`, `home_repro3_test.exs` (the last seeds a real `:evogit_agent_state` row + integer-id `agent_registered` and runs HomeLive's REAL `get_agent_history` → `Messages.assistant_text` → `Transcript.put_streaming_text` path).
- **Reported post-send crash (NOT reproducible in the test env)**: every handler in the post-send flow is defensive against real payloads (integer ids, nil status, nil timestamps, real structs — all exercised by the repro files). The `partition_active_tasks` `Enum.sort_by(&(&1.finished_at || &1.started_at), {:desc, DateTime})` raise for two-nil-timestamp rows (node_aware.ex:186) is contained by the async rescue in `request_tasks_load` (kills only the supervised child; the sidebar silently skips that refresh). If the crash recurs live, capture the LiveView's stacktrace directly (`Process.info(pid, :current_stacktrace)` / `:sys.get_state` on the HomeLive pid, or run `mix phx.server` with the page open) — the test env cannot reproduce it.
...
- **Bubble styling (ChatGPT-style, hand-crafted Tailwind — zero DaisyUI component classes on this page)**: the empty state (`@transcript == []`) shows the brand logo mark (light/dark variants via `dark:hidden` / `hidden dark:block`), a "Start a conversation" kicker + "How can I help you today?" greeting, and 4 suggestion chips wired to the existing `send_message` event via `phx-value-message`; user entries are right-aligned `bg-primary text-primary-content` bubbles with a flattened bottom-right corner (`rounded-br-md`); assistant entries are full-width with NO bubble — a `hero-sparkles` avatar circle plus plain `text-base-content` text; streaming-empty renders a typing-indicator pill with three bouncing dots, streaming-with-text appends a blinking `.help-caret`; error entries use a soft red tint (`border-error/25 bg-error/10 text-error`) with an exclamation icon. The composer is a rounded-3xl `.help-composer` container (styles in the "Help Chat Page (GET /help)" section at the end of `assets/css/app.css`, theme-token color-mix only) with the circular Send button morphing into the Stop button while running — both buttons stay in the DOM (the inactive one `hidden` + disabled) so `button[phx-click="stop"]` remains assertable on idle; the header has only the ghost "New chat" button. All colors come from theme tokens (no hardcoded hex).
