...
1. **Send** (`send_chat/2` — the SINGLE path for BOTH typed submits and suggestion-chip sends, which share the `handle_event("send_message", %{"message" => text}, ...)` clause): build the objective (`Transcript.build_preamble(transcript)` → `"Previous conversation:\nUser: …\nAssistant: …\n\nNew message: <text>"`, or bare text on `:empty`) → append optimistic user bubble + empty streaming assistant bubble (pulsing dots) → `chat_status: :running` + `chat_node: current_node` → `EvoDash.NodeContext.start_task(node, :reflect, ModelSelect.task_opts(socket.assigns[:selected_model_id], mode: "reflect", objective: objective))` — NO `:path` key; `:model_id` + `:model_id_locked` thread ONLY when the header selector pinned a profile, nothing on Auto → `AgentStream.task_id_from_start/1` → set `chat_task_id` + `chat_task_status: :pending` → persist → `async_lookup_agent`. `{:error, reason}` / no-id → `finalize_streaming` error text + back to `:idle` + persist.
