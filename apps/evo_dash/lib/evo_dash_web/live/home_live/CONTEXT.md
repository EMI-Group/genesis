...
**Events**: `send_message` (form `phx-submit`, `"message"` param; a fallback clause uses `chat_draft`), `chat_input` (draft sync), `chat_keydown`/`chat_keyup` (Enter/Shift latch), `new_chat` (idle-only; `start_new_chat/1`), `stop` (graceful cancel).
...
- sparkles avatar (existing look) + card frame (`rounded-lg border bg-base-100 shadow-sm`, status-tinted border via `card_border/1`)
...
- streamed text body: pulsing three-dot pill while empty+streaming, blinking `.help-caret` while streaming with text, final text otherwise
...
- Total: every payload access is `Map.get`-guarded
...
- **New chat / node switch**: `start_new_chat/1` calls `ChatHistory.new_chat()` (fresh id; OLD chats KEPT — there is no chat-switching UI yet) + `ChatHistory.prune(10)` (cap: keep the newest 10 chats — bounded memory; pruning is never automatic) + resets all live chat assigns + persists the fresh empty state.
...
## Notes for Agents — Model selector (support modules DONE, LiveView wiring PENDING at parent level)

The per-chat LLM model selector feature is **partially implemented**: the support modules in THIS directory are done and committed (`ChatState.selected_model_id` field + public `ChatState.normalize_model_id/1` + `ModelSelect.load/1` + `ModelSelect.task_opts/2` above), but the `home_live.ex` wiring (one level up, in the parent `live/` node's directory) and the tests (`apps/evo_dash/test/...`) are **outside this node's write scope** — a higher-level agent (parent of `live/` or the `live/` node itself) must complete them. Minimal remaining recipe:
...
- `home_live.ex`:
  ...
  - `base_assigns/0`: seed `selected_model_id: nil, model_profiles: [], model_selection_enabled: false`; `reset_chat/0`: reset `selected_model_id: nil` (per-chat default Auto).
