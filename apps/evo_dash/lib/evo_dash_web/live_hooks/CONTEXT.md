...
**Contract-pending statuses API**: The statuses forms of `list_tasks_summary` land with parallel `:evo_git` work and are **NOT yet in this worktree** — `EvoGit.TaskRegistry.list_tasks_summary/1` and `EvoDash.NodeContext.list_tasks_summary/2` (via `EvoGit.RemoteNode.list_tasks_summary/2` → `EvoGit.AgentScheduler.RemoteAPI.list_tasks_summary/1`) currently raise `UndefinedFunctionError` until that work merges. Do NOT revert to the `/0`/`/1` forms to work around it. The compile warnings about the undefined functions are expected and disappear after the merge.
...
- **Contract-pending statuses API**: the statuses forms `EvoGit.TaskRegistry.list_tasks_summary/1` and `EvoDash.NodeContext.list_tasks_summary/2` (via `EvoGit.RemoteNode.list_tasks_summary/2` → `EvoGit.AgentScheduler.RemoteAPI.list_tasks_summary/1`) land with parallel `:evo_git` work and raise `UndefinedFunctionError` until merged — do NOT revert `load_running_and_pending_tasks/1` to the unfiltered `/0`/`/1` calls as a workaround.
