...
- **Per-LiveView `:node_aware_reload_tasks` handlers** (all six route `{:tasks_updated}` / `{:task_status, _, _}` broadcasts into `NodeAware.handle_task_info/2` first): TasksLive (tasks_live.ex:548-558) = page reload (`reload_current_page/1`) + sidebar + clear-pending (dirty tracker deliberately NOT advanced — next remote-poll tick self-corrects); ProjectsLive (projects_live.ex:1610-1662) = CUSTOM handler: project-settings refresh if the panel is open + local-only browser-notification diff (`list_task_ids([:completed, :failed, :cancelled])` + per-new-id `get_task/1`) + `load_running_and_pending_tasks/1` + `clear_task_reload_pending/1` (does NOT call `reload_tasks/1`); ReviewLive (review_live.ex:728-745) = sidebar always + broadcast-guarded review-data async reload ONLY when the stashed `:last_broadcast_task_id` (set by `{:task_status, task_id, _}`, reset to nil by id-less `{:tasks_updated}`) matches the reviewed task id; AgentsLive (agents_live.ex:220-223), SettingsLive (settings_live.ex:840-843), SystemLive (system_live.ex:1255-1258) = sidebar only.
