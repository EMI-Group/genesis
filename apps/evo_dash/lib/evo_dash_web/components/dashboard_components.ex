      ...
      <div class="p-6 md:p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div class="md:col-span-4 flex flex-col gap-6">
            <div>
              <label class="block text-sm font-semibold text-base-content mb-2">Task Mode</label>
              <div class="relative">
                <select
                  name="mode"
                  class="w-full appearance-none bg-base-100 border border-base-300 rounded-lg px-4 py-3 pr-10 text-base font-medium focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary shadow-sm cursor-pointer transition-all"
                >
                  <option value="genesis_new" selected={@mode == "genesis_new"}>New Codebase (Genesis)</option>
                  <option value="genesis_existing" selected={@mode == "genesis_existing"}>Existing Codebase (Genesis)</option>
                  <option value="evolve_simple" selected={@mode == "evolve_simple"}>Simple Evolution (Top-down)</option>
                  <option value="evolve_complex" selected={@mode == "evolve_complex"}>Complex Evolution (Bottom-up)</option>
                </select>
                <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-base-content/50">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                  </svg>
                </div>
              </div>
              <p class="mt-1.5 text-xs text-base-content/50">Choose how you want to process your codebase</p>
            </div>
