...
| `EvoDashWeb.LaunchpadComponents` | `launchpad_components.ex` | Launchpad variants (Flow/Console/Workspaces, `docs/launchpad-frontend-spec.md`): `top_bar/1` (brand, variant switcher, pro links, scheduler-paused dot, theme toggle — no sidebar), `composer/1` (project chip + prompt textarea + controls row; textarea is `phx-update="ignore"`, cleared client-side via the `lp:clear_prompt` push event + `ComposerFocus` hook), `project_chip/1` (FULL path mono, click-to-edit inline input with `PathAutocomplete` + recent one-tap chips; renders its own form so it sits OUTSIDE the composer form), `tune_popover/1` (progressive-disclosure params; inputs carry `form=` + debounced `lp_tune_change`), `task_card/1` (status dot, 2-line prompt, FULL path with start-ellipsis, mode/model chips, tokens, relative time, duration, Cancel/Review/Continue/Delete actions; summary-map safe), `stream/1`. Monochrome-first: one accent (primary) reserved for the launch action + running status. |
...
| `EvoDashWeb.Layouts` | `layouts.ex` | Layout function components — `flash_group/1` (renders `:info`, `:error`, `:warning` flashes), `theme_toggle/1`, `language_selector/1`. The sidebar-based `app/1` app shell is **DELETED** (retired with the classic dashboard): every page now renders the pad chrome directly — `PadComponents.pad_top_bar/1` (brand left; Tree / Review N / Settings / System + theme toggle fixed top-right) + a `<main>` wrapper + `flash_group/1`. |
