...
The dashboard launch panel is ONE card (`.input-card`) containing the objective textarea AND the controls row (`.input-controls`) as its last element — in normal document flow, never `position: fixed`. The layout is **server-seeded at render + client-updated while typing**: `EvoDashWeb.TaskFormComponents.layout_for/1` computes the initial `data-layout` on `.input-layout` from the `@prompt` attr — the LiveView passes NO new attr and does NOT set `data-layout`. There is NO per-keystroke server event for the objective textarea (no `phx-change`): while typing, the AdaptiveInput JS hook mirrors the same thresholds client-side and flips `data-layout` directly.
