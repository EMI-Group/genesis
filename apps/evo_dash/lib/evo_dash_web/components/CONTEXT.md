...
| `EvoDashWeb.AgentsComponents` | `agents_components.ex` | Agent tree visualization: recursive `path_tree/1` with connector lines and status indicator helpers (color/background/border/icon). **Superseded by the EvolutionGraph canvas on `/agents` — currently unused, retained for reference** |
