...
| `EvoDashWeb.AgentsComponents` | `agents_components.ex` | Agent tree visualization: recursive `path_tree/1` with connector lines and status indicator helpers (color/background/border/icon). **Used by the file-tree panel on `/agents`** — renders the real directory tree (scanned from disk) with agent cards overlaid; clicking a card selects the agent and centers the EvolutionGraph on it |
...
| `EvoDashWeb.Layouts` | `layouts.ex` | Layout function components — `app/1` (the **sidebar-based app shell**: fixed left `<aside id="sidebar">` with branding, nav links (**Tasks `/` · Agents `/agents` · Settings `/settings`** — the NodeSelector block and the System/Tasks entries were removed in the workflow decluttering), active-tasks section, language/theme/**art-style** selectors in the bottom bar; `phx-hook="SidebarCollapse"` for collapsible mode persisted to `localStorage`; mobile hamburger + overlay present but **not yet wired to JS**), `flash_group/1` (renders `:info`, `:error`, `:warning` flashes), `theme_toggle/1`, `theme_toggle_compact/1`, `language_selector/1`, `art_style_selector/1` (dropdown with 25 styles — `default` / `default-dark` / `constructivism` / `swiss` / `terminal` / `ink` / `bauhaus` / `memphis` / `brutalism` / `glass` / `neumorphism` / `cyberpunk` / `vaporwave` / `nord` / `gruvbox` / `solarized-light` / `solarized-dark` / `mono` / `blueprint` / `kraft` / `botanical` / `noir` / `pastel` / `ocean` / `desert` / `artdeco` — data-driven from the private `art_styles/0` list; persisted to `localStorage` as `phx:art`, applied by the inline script in `root.html.heex` which also drives `data-theme` (default-dark → dark, everything else → light; **no separate light/dark toggle** — `theme_toggle_compact` was removed). `?art=<style>` URL param also works; active option marked client-side via `.art-active`). **Art style system:** `assets/css/art_styles.css` scopes full visual identities (palette, radius, shadows, borders, typography, component geometry — not just colors) under `:root[data-art="…"]`; each style also recolors the evolution graph via root-level `--evo-*` variables (canvas, nodes, edges, HUD, and the tree-switcher tab bar all follow). **Node-aware (SSH Remote Development):** `app/1` accepts optional backward-compatible attrs (`current_node`, `current_node_id`, `current_node_name`, `remote_targets`, `connection_statuses` — all defaulting to safe local values). A private `with_node_param/2` helper appends `?node=<id>` to ALL nav links (when a remote node is selected). The `simple_nav` attr (default `false`) hides the nav links, active-tasks section, and bottom bar (used by WelcomeLive). NOTE: the old top **navbar was replaced by this sidebar** — no top-bar header remains in the app shell. |
