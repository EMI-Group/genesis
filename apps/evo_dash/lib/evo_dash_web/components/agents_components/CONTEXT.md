...
- **The sticky lane header never actually sticks** (`commit_graph_view.ex` `lane_header/1` + `.cg-scroll` in `assets/css/app.css`): `.cg-lane-header` is `position: sticky; top: 0` INSIDE `#cg-scroll-<dom>.cg-scroll.overflow-x-auto`. Because one overflow axis is `auto`, CSS computes the other (`overflow-y`) to `auto` too — `.cg-scroll` is therefore itself a scroll container in BOTH axes, and with auto height it never scrolls vertically. Sticky resolves against the NEAREST scroll container (`.cg-scroll`), not the real vertical scroller (`.agents-tree-panel .card-body` at xl+, the page on mobile), so the chips scroll away with the list. Horizontal sync (the design intent) works; the vertical stick does not. Fix direction: cap `.cg-scroll`'s height and make it the vertical scroller (`overflow: auto`) so the header sticks inside it.
- **The gutter nodes' `<title>` tooltips are unreachable** (`node_dot/1`): the gutter `<svg>` is `pointer-events: none` (the click contract lives on the rows), so a hover over a dot never shows the native tooltip. `.cg-row` carries NO `title` either, so the node tooltip content (message · sha · author · date · refs) is dead UI. Fix direction: move `node_title/1` onto the row `div` (and/or drop the SVG `<title>`).
- **No scroll-to-selection / row↔dot hover sync**: selecting an agent (row, lane chip, or the tree) re-renders the START/END rings but never scrolls them into view, and hovering a row does not highlight its gutter dot (both need the client `CommitGraph` hook, which today is enter-animations only).
- **Rows are click-only `div`s** (`commit_row/1`): no `role`/`tabindex`/keyboard activation — keyboard users can reach the lane chips (`<button>`) but not the rows.
- **Edge-jog collisions** (`Geometry.route/4`): the horizontal jog of every cross-lane route sits at the vertical midpoint of its span, so concurrent edges between the same lane pair (several children spawning from one fork; a child's spawn + merge_back) overdraw exactly. No staggering.
