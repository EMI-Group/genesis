...
- ~~**Edge-jog collisions** (`Geometry.route/4`): the horizontal jog of every cross-lane route sits at the vertical midpoint of its span, so concurrent edges between the same lane pair (several children spawning from one fork; a child's spawn + merge_back) overdraw exactly. No staggering.~~ **FIXED (geometry half)**: `Geometry.edge_geo/3` (the batch-aware variant) staggers concurrent same-lane-pair edges deterministically — see the geometry API bullets above. The renderer's `edge_path/1` still calls the canonical `edge_geo/2` per-edge (its `commit_list/1` computes `positions` once per repo and has the full edge list in scope — adopting `edge_geo/3` is a one-line wiring change for the renderer wave); until wired, rendered output is unchanged.
