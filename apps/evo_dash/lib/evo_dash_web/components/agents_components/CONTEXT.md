...
- `edge_path/1` → `path#commit-edge-<dom>-<from_sha>-<to_sha>.cg-edge[data-commit-graph-anim="edge"]` with `d` = an ORTHOGONAL (right-angle) route (`M fx fy L fx my L tx my L tx ty`, the corner at the vertical midpoint between the two rows) so same-column edges read as a straight vertical line (the horizontal segment is zero-length) and cross-column edges step across crisply; a zero-length edge is dropped (`:if={d}`).
...
- `node_dot/1` → `g#commit-node-<dom>-<sha>.cg-node[data-commit-graph-anim="node"][data-cg-sha][data-cg-agent-id={owner_id}]` containing a native `<title>` (`base` prefix for a base node, then message first line · short sha · author · date · refs; empties dropped) and a `rect.cg-node-dot` — a filled SQUARE centred on the grid point (`x = cx - @node_r`, `width = height = 2 * @node_r`); a `:base` node is a smaller HOLLOW square (`x = cx - @base_r`, side `2 * @base_r`); `rx`/`ry` = `0`, `stroke-width="1.5"`, inline `fill` / `fill-opacity` / `stroke`.
...
- Selection rings (an EXTRA `rect.cg-node-ring` child, visible without CSS): the START node (`selected_id ∈ node.start_ids`) gets a SOLID `var(--color-primary)` ring, the END node (`selected_id ∈ node.end_ids`) a DASHED one; the square ring's half-size = the dot's half-size + 3 (normal: `x = cx - 9`, side 18; base: `x = cx - 7`, side 14), `rx`/`ry` = `0`.
