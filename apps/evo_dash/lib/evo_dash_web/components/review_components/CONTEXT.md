...
- `file_tree_sidebar/1` + `tree_node/1` + `file_row/1` — sidebar file tree with per-directory aggregate add/delete counts. **Tree state is server-driven (no `<details>`)**: dir expansion comes from the `expanded_dirs` map attr (dir FULL path ⇒ boolean, collapsed by default; `toggle_dir` event), and a non-blank `file_filter` attr switches the sidebar to a FLAT case-insensitive full-path match (no dir nodes; `filter_files` event, 200ms debounce). `collapse_all_dirs`/`expand_all_dirs` ghost buttons act on the whole tree. Dir nodes carry a `path` key (accumulated joined path) so `phx-value-dir` is the full dir path; `build_file_tree`/`insert_into_tree` thread a `parent_path` accumulator. Sibling sort is explicitly case-insensitive (`String.downcase(name)`).
...
- **`split_diff_layout` / `commit_diff_layout`** (`diff_viewer.ex`): both render `file_tree_sidebar` + `diff_viewer` in the full-width two-pane layout (`flex flex-col lg:flex-row gap-3 items-start`; sidebar `w-full lg:w-72 shrink-0 lg:sticky lg:top-0 lg:max-h-[100dvh]`; diff column `flex-1 min-w-0 w-full space-y-3`). Attrs: `files`, `expanded_files`, `selected_file`, `file_context_levels` + `expanded_dirs`, `file_filter`; `split_diff_layout` additionally takes `repos` (list, default `[]`) + `active_repo_id` (default `"primary"`) and renders a repo toolbar above the panes when `length(@repos) > 1` — repo select (`switch_repo`, `name="repo_id"`, `select select-sm select-bordered rounded-lg max-w-56`, option label `"<repo_id> — <path truncated ~30 chars>"` via `repo_option_label/1`) + right-aligned summed `+adds`/`-dels` (`sum_files/2`) computed from `@files`. Per-`FileInfo` field reads: **`path`** (tree build, header, `file_path_to_id`), **`status`** (header + tree via `file_status_icon/color` — string atom-ish values "added"/"deleted"/"modified", catch-all else), **`additions`/`deletions`** (header, tree file node, dir aggregates, toolbar sums), **`language`** (→ `data-language` attr; from `EvoGit.Review.language_for_file/1`, stamped on every file, default "text"), **`diff`** (only in `render_diff_content`). **Drivers**: `expanded_files` map (`Map.get(@expanded_files, file.path, false)` — file body renders only when true); `file_context_levels` map (`Map.get(@file_context_levels, file.path, 3)` — default 3, `:all` disables expand bars); `selected_file` is declared on `diff_viewer/1` and passed through but **never referenced in its body** — it only drives the tree row highlight (`file_row`); `expanded_dirs`/`file_filter` drive the sidebar. **Lazy diff confirmed**: `render_diff_content` checks `file.diff` twice — `if file.diff, do: parse_diff_lines(file), else: []` and `is_nil(@file.diff)` → "Loading diff…" spinner; ReviewLive swaps in `file.diff` via `update_file_diff_in_socket/4` after the `load_file_diff` event. So the header/stats render from metadata alone; full diff text renders only for expanded files whose diff has been fetched.
...
- **`action_buttons`** (`actions.ex:20-180`): `branch_exists` (gates merge/reject/resume/divider, Create PR, Extract Skills; `not branch_exists` → warning box + resume), `has_pr`, `pr_url`, `loading` (disables all buttons), `is_no_changes` (warning vs info styling/message), `merge_targets` + `default_merge_target` (`selected`, confirm text), `repo_id` (default `"primary"` — the repo the buttons act on; threaded into the merge form as a hidden `<input type="hidden" name="repo_id" value={@repo_id}/>` and as `phx-value-repo_id` on the target-branch select and the bare `phx-click="merge"` button, so `merge`/`merge_target_change` events carry the acting repo), `merge_status` → `merge_status_block`. `merge_status_block` pattern-matches only `%{state: :checking}`, `%{state: :clean}`, `%{state: :conflict, files: files}` (`length(files)` and `conflict_files_summary(files)` = first 4 paths joined + "…"); **`merge_status.target` is NOT read** by the component (ReviewLive uses it for the merge call), and `state: :error` (or any other shape) hits the `_ ->` catch-all and renders nothing (old fallback behavior). `can_resume` gates the extra Resume button when the branch is gone.
- **`commit_detail_header`** (`diff_viewer.ex`): compact card (`rounded-xl border border-base-300 bg-base-100 p-4 mb-3`) reading `message` (title attr = full message), `sha` (sliced 0..7, mono badge chip), `author_name`, `date` (`relative_time`) of the `@commit` map (fed `@commit_header` = the matching `%CommitInfo{}` from `@commits`), plus `back_url` (required — `hero-arrow-left` ghost square back link, title/aria `gettext("Back to review")`) and `task_title` (default nil — muted truncated breadcrumb context on the meta row).
...
The review page's merge action can merge into a user-selectable branch instead of always the repo default. `action_buttons/1` takes `merge_targets` (list of local branch names), `default_merge_target` (resolved default branch name, or nil), and `repo_id` (default `"primary"`) — the repo the buttons act on. When `merge_targets != []`, the Merge button is wrapped in a `<form phx-submit="merge" phx-change="merge_target_change" class="contents">` alongside a compact `<select name="target_branch">` (DaisyUI `select-sm select-bordered rounded-lg`) pre-selecting `default_merge_target`; the form embeds a hidden `<input type="hidden" name="repo_id" value={@repo_id}/>`, the select carries `phx-value-repo_id={@repo_id}`, and the Merge button becomes `type="submit"` (keeping `phx-confirm`). When `merge_targets == []` (branches couldn't be listed), the plain `phx-click="merge"` button renders — also carrying `phx-value-repo_id={@repo_id}`. The event params' `"target_branch"` and `"repo_id"` land in the `merge` handler (`"repo_id"` also in `merge_target_change`).

**Per-repo merge/reject semantics**: the `merge` handler resolves the submitting repo from `params["repo_id"]` (falling back to the active repo, then `"primary"`, whitelisted against the known review-repo ids), builds a per-repo merge plan across `@review_repos` — the submitting repo merges into the form target (validated against ITS branch list, else its resolved default), every other repo merges into its own resolved default — and merges each repo into its own target on the viewed node. `reject` broadcasts the same way, deleting the agent branch in EVERY review repo. Outcomes are reported per-repo: full success sets the task status and navigates away; partial success/failure assigns `:merge_outcomes` (rendered by `merge_outcomes_panel` in the parent `review_components.ex`, one row per repo with its `repo_id`) and STAYS on the page — never dismisses a partially applied merge/reject silently. Data sources (all in `EvoGit.Review`, called from `ReviewLive.load_task_data/2` with plain `case` on the tuple returns — no try/rescue): `list_branches/1` (names filtered to non-blank binaries), `default_merge_target/1`, and `merge_branch/3` (`merge_branch/2` is the default-resolving path — do not remove it).
