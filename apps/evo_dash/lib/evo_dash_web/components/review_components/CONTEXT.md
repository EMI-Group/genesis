...
Gate: private `show_merge_all?/1` — `length(repos) >= 2` AND at least two repos that both HAVE CHANGES (`repo_has_changes?/1`) AND are still unresolved (`resolution == nil`) — else `false`. A single-repo task, fewer than 2 unresolved repos, or fewer than 2 CHANGE-BEARING unresolved repos renders NOTHING (a no-change repo needs no action, so it neither makes the button appear nor counts). Reuses the public `repo_has_changes?/1` + `resolution_state/1`, so a nil or absent `:resolution` counts as unresolved.

#### `repo_has_changes?/1` — public helper

`repo_has_changes?(repo)` is the SINGLE source of the "does this repo have changes?" test: true iff the repo's `branch_name` (via the defensive `field/3`, atom → string → default) is a non-blank binary; nil/`""`/whitespace → false; non-map → false. Public (like `Actions.conflict_files_summary/1`) because the hosting `ReviewLive` shares the EXACT predicate — the `merge_all` fold and the per-repo `merge`/`reject` guards act only on change-bearing repos, and `completion_status/2` treats a no-change repo as needing no action (it never blocks aggregate completion, and a task with no changes in ANY repo is dismissed via "Mark as read", never reported as `:rejected`).
...
Attrs: `can_resume` (`:boolean`, default `false`), `loading` (`:boolean`, default `false`), `branch_exists` (`:boolean`, default `true`), `has_pr` (`:boolean`, default `false`), `pr_url` (`:string`, default `nil`), `show_export` (`:boolean`, default `false`), `export_url` (`:string`, default `nil`), `no_changes` (`:boolean`, default `false`).

Normal state (`no_changes: false`): one `rounded-xl border border-base-300 bg-base-100 p-4 flex flex-wrap items-center gap-3` row holding the **Continue task** button — only when `@can_resume` — and the "…" overflow menu (which carries `ml-auto` and is therefore pinned to the row's right edge as its last child).

**No-change state** (`no_changes: true` — a task with NO changes in ANY repo, threaded from `ReviewLive`'s `@is_no_changes`): the dismissal is PROMOTED to the primary action as **"Mark as read"** (`mark_as_read_button/1` — `btn btn-primary btn-sm rounded-lg gap-1.5`, `hero-check`, `phx-click="ignore"` — the SAME event the overflow menu's Ignore fires — with a `phx-confirm`). It renders FIRST, then the Continue task button (when `@can_resume`), then — only when `@show_export` — the overflow menu with `show_ignore={false}` (the now-redundant Ignore item suppressed; the menu is omitted entirely when nothing is left to show). There are no merge/reject actions when nothing has a branch.
