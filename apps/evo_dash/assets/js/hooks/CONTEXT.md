...
- **`ClipboardCopy` (inline in `../app.js` ~line 520) has NO fallback and silently no-ops — known copy-button failure.** It reads `this.el.dataset.content`, and only copies when BOTH `content` is truthy AND `navigator.clipboard` exists (`if (content && navigator.clipboard)`). `navigator.clipboard` is only defined in SECURE contexts (https, `localhost`/`127.0.0.1`, `file:`). The dashboard is served over plain HTTP in several supported configurations — non-desktop prod binds `0.0.0.0` with `scheme: "http"` (`config/runtime.exs:257-271`), desktop can be exposed to LAN via `PHX_IP` over http, and dev binds loopback http — so any access via a LAN IP / non-localhost hostname yields `navigator.clipboard === undefined` → the click does NOTHING: no copy, no `pushEvent("copied")`, no flash, no error. There is also NO `document.execCommand('copy')` / hidden-textarea fallback anywhere in the repo (grep `execCommand` = zero matches). Additionally, when `navigator.clipboard` exists but `writeText` REJECTS (webview restrictions — e.g. webkit2gtk on Linux desktop webviews, permissions policy, document unfocused), `.catch(() => {})` swallows the rejection silently. On the success path it does `this.pushEvent("copied", {})` (server flashes "Copied to clipboard"; handlers exist in tasks_live.ex:739, review_live.ex:338, welcome_complete_live.ex, projects_live.ex) and toggles a 2s `text-success` class on the button's first `svg`. If copy buttons "do nothing", check the serving context first: secure-context browser tab + working `navigator.clipboard` is required; a real fix needs a fallback (execCommand via hidden textarea, and/or a Tauri clipboard capability — no clipboard-manager plugin is installed, `desktop/` has zero clipboard code).
