...
const EvolutionGraph = {
    ...
    for (const raw of list) {
      ...
      if (!n) {
        ...
        if (n.parentId !== parentId) {
          structureChanged = true;
          // 换父/变根：摘掉旧连线并维护新旧父节点的 children 列表
          if (n.edgeEl) { n.edgeEl.remove(); n.edgeEl = null; }
          const oldP = n.parentId && this.nodes.get(n.parentId);
          if (oldP) oldP.children = oldP.children.filter(c => c !== n);
          const newP = parentId && this.nodes.get(parentId);
          if (newP && !newP.children.includes(n)) newP.children.push(n);
        }
        n.parentId = parentId;
  ...
  removeNode(n) {
    // 从父节点 children 摘除，子节点变根（同时清掉指向被删节点的连线）
    if (n.parentId && this.nodes.has(n.parentId)) {
    ...
    for (const c of [...this.nodes.values()]) {
      if (c.parentId === n.id) {
        c.parentId = null;
        if (c.edgeEl) { c.edgeEl.remove(); c.edgeEl = null; }
      }
    }
    if (n.el) {
  ...
  bindPointer() {
    ...
    this.svg.addEventListener('pointermove', e => {
      ...
      if (g && g.__data__) {
        ...
        const n = g.__data__;
        // 用 textContent 组装提示（label 来自任务目标文本，innerHTML 会注入标签）
        ...
        this.tipEl.replaceChildren(
          ...
          document.createTextNode(
            (STATUS_CN[n.status] || n.status) + (n.agent ? ' · ' + n.agent : '')
          )
        );
    ...
    this.svg.addEventListener('click', e => {
      ...
      if (downNode && downNode.__data__) {
        ...
        if (!this.demo) {
          this.pushEvent('select_agent', { id: n.id });
          this.markSelected(n.id);
        }
      } else {
        if (!this.demo) {
          this.pushEvent('close_details', {});
