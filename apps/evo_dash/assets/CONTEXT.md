...
- **DirectoryPicker auto-confirm (project picker)**: server-side wx picks (via `EvoDash.DirectoryPicker` → `wxDirDialog`) return a full absolute path, so after filling the input the `data-picker-id="project"` browse button submits the enclosing `open_project` form directly via `form.requestSubmit()` — the project opens immediately with no extra "Open" click. (The Tauri command/plugin and File System Access API `showDirectoryPicker` branches were REMOVED — the FSA branch never yielded a full path, only `handle.name`; see the "DirectoryPicker — server-side wx protocol" bullet below.) The other two pickers (`data-picker-id="new-project"` and `"foreign-repo"`) also only fill their input: new-project still needs a name and foreign-repo is a settings form field. The server-side `open_project` handler (`ProjectFlow.open_project/2`) validates the path (`Path.expand` + `File.dir?`), so auto-submitting is safe even for a stale/bad selection.
- **Windows "path replaced by app dir" symptom (diagnosis)**: A user picking `D:\Test` saw `Directory does not exist: c:/Users/ranch/AppData/Local/genesis-desktop/Test` (basename survives, prefix replaced). Verified by investigation: the dashboard NEVER splits/joins the picked path — `fillInput` (`js/app.js:312-319`) writes the picker result VERBATIM into the `path` input, the form submits it verbatim, and the server flash (`DashboardLive.ProjectFlow.open_project/select_project`, `project_flow.ex:118/145`) prints the RAW (unexpanded) `path` param. So the string in the flash IS the input value, already normalized (lowercase drive + forward slashes = `Path.expand`/`Path.join` output format). The only in-dashboard mechanisms that can alter/replace the value are: (1) the `PathAutocomplete` hook's auto-complete (`js/app.js:83-108`) — when exactly ONE datalist suggestion starts with the current input, the input is REPLACED by that suggestion (datalist options are server-rendered, always normalized); (2) `Path.expand/1` resolving a RELATIVE input against the BEAM cwd (`project_flow.ex:98/130/171`) — the BEAM cwd is inherited from the Tauri process (`desktop/src-tauri/src/sidecar.rs` spawns `bin/genesis_desktop` with no `current_dir`), so a bare/relative value like `Test` becomes `<app-cwd>/Test`. The historical source of the bare value — the FSA `showDirectoryPicker` fallback filling `handle.name` — is GONE (the FSA branch was deleted when the hook moved to the server-side wx protocol; `EvoDash.DirectoryPicker` only yields real absolute paths). This diagnosis applies to pre-fix builds; on current builds the server also rejects any relative input via `ProjectFlow.normalize_project_path/1`; (3) legacy v0.9.3 `create_project` with an EMPTY `location` (`Path.expand("")` = cwd → `Path.join(cwd, name)` → created + stored in recents). The Rust `pick_directory` command was REMOVED along with the Tauri picker (see `desktop/` — outside this app's scope) — no join there either way. Debug leads for this class of bug: inspect the actual form param (flash shows it raw), the BEAM cwd, and the recents list in `tasks.sqlite`.
- **DirectoryPicker — server-side wx protocol (DONE)**: The hook (in `js/app.js`, ~line 113) is now a thin LiveView client — no Tauri, no File System Access API. Click → `this.pushEvent("directory_pick", {picker_id})` → `DashboardLive` (local node only) → `EvoDash.DirectoryPicker` GenServer runs the native `wxDirDialog` in a Task → the server pushes `picker_result:<picker_id>` with `{path, cancelled, unavailable}`. The hook handles: `path` → absolute-path regex guard → `fillInput` → auto-submit for `project`/`new-project` (`form.requestSubmit()`; server validates via `Path.expand` + `File.dir?`) / fill-only for `foreign-repo`; `cancelled` → no-op; `unavailable` → `markManualFallback` (`data-picker-error="manual"`, styled in `css/app.css` — `[data-picker-error="manual"] .picker-container::after` hint text + warning-tinted input border via `color-mix` + `--color-warning`/`--color-base-100` per Known Issue (i); cleared by `fillInput` on success). `handleEvent` is registered in `mounted()` (survives reconnects); NO JS-side timeout — the native dialog may legitimately stay open minutes. wx degrades to `unavailable` when: remote/headless node, picker disabled (`config :evo_dash, :directory_picker, enabled: false`), wx not compiled (`:code.which(:wx) == :non_existing` — dev/test `mix` runs prune wx from the code path since it is NOT an umbrella app dep; only the `genesis`/`genesis_desktop` releases load it via `wx: :load` in the root `mix.exs`), picker busy, or GenServer not running. Re-entrancy guard `this._picking` prevents stacked dialogs (re-armed by `picker_result` and `reconnected()`). `data-isLocal` was removed (it existed only for Tauri detection).
