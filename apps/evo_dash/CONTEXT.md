...
- `EvoDash.TaskStore` — SQLite (xqlite) backed persistent store (single GenServer holding one connection, namespaced keys `{:task, id}` / `{:project, path}`) for task history and recent projects; started under supervision before TaskRegistry. Values are stored as `:erlang.term_to_binary/1` BLOBs. Provides crash-safe read/recovery helpers (`safe_get/2`, `safe_select_all/1`, `safe_size/1`, `integrity_check/1`) that survive corrupt (un-deserializable) entries — `integrity_check/1` runs a SQLite `PRAGMA integrity_check` and scans all rows for undecodable blobs, removing corrupt rows. `integrity_check` is called by TaskRegistry on init.
