...
- **Data-safety architecture**: The TaskRegistry GenServer must NEVER terminate from a SQLite read/write error. All store-touching `handle_*` callbacks are wrapped in `try/rescue` (bodies extracted to `do_*` privates); write-failure rescues log at `Logger.error` (data loss is visible), read/cleanup rescues at `Logger.warning`. `select_all_tasks`/`select_all_projects` use `EvoDash.TaskStore.safe_select_all/1` which skips rows whose blobs fail to decode (per-row rescue, never raises). `EvoDash.TaskStore.integrity_check/1` runs on TaskRegistry init to heal a corrupted DB (runs `PRAGMA integrity_check` + scans for undecodable blobs, removes corrupt rows). Multi-key deletions go through `EvoDash.TaskStore.delete_multi/2`. `cleanup_expired_tasks` runs only on task-completion transitions, explicit `clear_finished_tasks`, and `init` — not on every mutation — and uses a single-pass scan. The completed task's `EvoDash.TaskStore.put` in `update_status` runs BEFORE `cleanup_expired_tasks`, so a crash mid-cleanup never loses the completed task.
