...
- **Data-safety architecture**: The TaskRegistry GenServer must NEVER terminate from a CubDB read/write error. All store-touching `handle_*` callbacks are wrapped in `try/rescue` (bodies extracted to `do_*` privates). `select_all_tasks`/`select_all_projects` use `EvoDash.TaskStore.safe_select_all/1` which survives corrupt entries via Agent-based partial-result salvage + forward/reverse reads. `EvoDash.TaskStore.integrity_check/1` runs on TaskRegistry init to heal a corrupted DB (salvage readable entries → `CubDB.clear` → rebuild). The completed task's `CubDB.put` in `update_status` runs BEFORE `cleanup_expired_tasks`, so a crash mid-cleanup never loses the completed task.
