...
- **Per-task model selection** (`DashboardLive` + `DashboardComponents.task_form/1`): The model dropdown and task mode selector are merged into a **single shared card** with a responsive `grid grid-cols-1 md:grid-cols-2 gap-4` layout (side-by-side on desktop, stacked on mobile). The model dropdown is populated from `Schema.model_profiles/1` (defaults to the first profile). The selected `:model_id` is threaded through `task_submit` → `build_common_runtime_opts/3` → runtime opts → core scheduler. Event: `select_model`.
