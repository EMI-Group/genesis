...
- `SettingsLive` — Configuration File tab (GUI editor for config.toml with collapsible sections for LLM, User, Scheduler, Sandbox, Evolution, Truncation, Task History). The LLM category includes a **Model Profiles List Editor** for the `[[llm.models]]` array-of-tables format — users can add/edit/delete model profiles (each with id, model, concurrency, and generation params: temperature, reasoning_effort, max_tokens, top_p, top_k, frequency_penalty, presence_penalty). The editor is rendered between the Quick Setup panel and the flat `[llm]` schema cards. Event handlers: `add_model_profile` (in-memory + enter edit mode), `edit_model_profile` (toggle), `cancel_edit_model_profile`, `save_model_profile` (validates + persists, duplicate-id rejection), `delete_model_profile`. The `select_llm_model_shortcut` and `save_custom_model` handlers ADD a new profile (with backward-compat mirroring to `[:llm, :model]`) instead of replacing the single flat model. Runtime updates push `:model_profiles` via `Schema.model_profiles/1` (not `:llm_model`/`:llm_generation_params`).
