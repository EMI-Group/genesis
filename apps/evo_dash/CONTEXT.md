...
- **SettingsLive / SystemLive / DashboardLive / TasksLive node-aware handling (FIX 3)**: These LiveViews branch on `current_node != node()`:
  ...
  - **SystemLive**: When remote, scheduler pause/resume controls **work via RPC** — the `toggle_pause` handler calls `EvoDash.NodeContext.call_remote(node, EvoGit.AgentScheduler, :pause/:resume, [])` for remote nodes. The pause/resume button is always visible (not hidden). Restart/stop VM buttons remain `disabled` (local-VM-only actions). The system check's config row reads the remote config status via `get_remote_config_status/1`. The scheduler paused state is read from `NodeContext.paused?/1`.
  ...
  - **TasksLive**: When remote (`@current_node != node()`), the entire task list UI is hidden and replaced with an info message: "Task history is only available when viewing the local node. Remote agents can be viewed on the Agents page." The remote daemon has no EvoDash.Store, so task history is meaningless there.
