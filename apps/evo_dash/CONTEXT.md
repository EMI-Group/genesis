...
- `WelcomeLive` — First-run onboarding page (`/welcome`). **Inline quick-LLM-setup redesign**: a flat grid of ALL preset models across providers (provider display name + model display name together, e.g. "Google Gemini 3.5 Flash"), avoiding the confusing two-level provider→model flow. Variants (Alibaba Global/CN, Z.ai Normal/Coding Plan) are expanded as separate entries with their own credential_key. When a model profile already exists, shows a "You're All Set!" state. Event handlers (`select_welcome_model`, `save_quick_setup`, `save_api_key`) adapt the SettingsLive logic but delegate to the same persistence layer (`ConfigIO`, `ModelProfileHelpers`, `EvoGit.Config`). **Version-state tracking**: on mount calls `EvoGit.Config.VersionState.upgraded?()` (gracefully degrades via `Code.ensure_loaded?/1` if the sibling module isn't compiled) and shows a "Welcome to the new version!" note once per upgrade; current version displayed in footer via `Application.spec(:evo_git, :vsn)`. Skip/Get-Started buttons redirect to `/welcome/complete`.
