...
- **Recommended shape** (when implemented): tiered — (1) zero-dep `:zip`+regex DOCX extractor; (2) pure-BEAM PDF via `ex_pdf` (default, works in all 3 releases incl. `genesis_remote`); (3) optional `pdftotext` upgrade path via `EvoGit.Executable.resolve("pdftotext")` (system-first, vendored fallback — extend `bundled_path/1` beyond git/rg) for higher-quality layout when available.
