...
- **Async merge check**: on review page load (and when the merge-target selector changes), `ReviewLive` spawns a supervised Task (`EvoDash.TaskSupervisor`, same self-message pattern as SettingsLive's LLM test) calling `EvoDash.NodeContext.check_merge/4` → `EvoGit.RemoteNode.check_merge/4` (node-aware, `:erpc` on remote) → `EvoGit.Review.check_merge/3` — a **non-mutating dry-run**: resolves both refs, adds a detached temp worktree at the target under `<repo>/.genesis/merge_check_<hex>/`, runs `git merge --no-commit`, collects conflict files via `git diff --name-only --diff-filter=U`, and always removes the worktree afterwards (`EvoGit.Adapters.Git.remove_worktree/2`, rm_rf + prune fallback, never raises). Returns `{:ok, :clean} | {:ok, {:conflict, files}} | {:error, reason}`; identical SHAs → `:clean` (already merged). Main repo index/HEAD untouched.
