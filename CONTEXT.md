...
- **Boot-time sizing** (`config/runtime.exs:23-66`): pool `count = max(total_concurrency + 2, 8)` where `total_concurrency` = sum of per-profile `concurrency` from `EvoGit.Config.resolve()` (each defaults to 3); falls back to `scheduler.default_llm_max_concurrency` when no `[[llm.models]]` profiles exist. Configured via the full `finch:` override form — `finch: [name: ReqLLM.Finch, pools: %{default: [protocols: [:http1], size: 1, count: stream_pool_count, start_pool_metrics?: true]}]` — with `stream_pool_timeout: 120_000` kept top-level (read at CALL time). The `start_pool_metrics?: true` flag is REQUIRED for `Finch.get_pool_status(ReqLLM.Finch, :default)` to enumerate origins (see runtime reconciliation below). Evaluated before `:req_llm` starts, in ALL releases (genesis, genesis_desktop, genesis_remote — releases run runtime.exs at boot; the desktop/remote flags in runtime.exs never touch the pool; NO desktop-specific pool override exists anywhere in `desktop/`, `rel/`, `config/`, or app code).
