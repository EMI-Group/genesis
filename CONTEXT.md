...
- **Force kill** — `EvoGit.TaskRegistry.force_kill_task/1` (the OLD brutal `cancel_task`, renamed; same for RemoteAPI/RemoteNode/NodeContext): kills all agents + the wrapper, persists `:cancelled` with result nil'd. Works from `:running` and `:cancelling`.
