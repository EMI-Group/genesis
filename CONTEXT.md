...
- **Linux**: Builds x86_64 (`ubuntu-24.04`) and ARM64 (`ubuntu-24.04-arm`) → `.deb` / `.rpm` / AppImage / `.tar.gz` portable archive (AppImage excluded on ARM64 — `appimagetool`/`linuxdeploy` are x86_64-only). The ubuntu-24.04 glibc requirement is accepted; a musl build is a planned future improvement. Flatpak is not built — Tauri v2 has no native Flatpak bundle target (documented in the workflow).
...
Research (Aug 2026) for feeding `.pdf`/`.docx` file contents into LLM objectives. **Status: plain-text files and `.docx` are IMPLEMENTED** via `EvoGit.PromptFile` (`apps/evo_git/lib/evo_git/prompt_file.ex`, wired into CLI `get_input/2` at `cli.ex:136-154`): `.txt`/any text read verbatim, `.docx` extracted with OTP `:zip` + regex (zero deps, works in all releases incl. `genesis_remote`), `.pdf` returns a clear "not supported yet" error. **`.pdf` support is the open decision** — options below. The objective stays a plain `String.t()` end-to-end (AgentSpec/AgentState/TaskInfo `opts`); conversion happens at the CLI boundary. The dashboard (evo_dash) has NO file-input path yet (would need LiveView upload plumbing). Findings:
...
- **Recommended shape** (PDF decision when taken): tiered — (1) zero-dep `:zip`+regex DOCX extractor — **DONE** (`EvoGit.PromptFile`); (2) pure-BEAM PDF via `ex_pdf` (default, works in all 3 releases incl. `genesis_remote`); (3) optional `pdftotext` upgrade path via `EvoGit.Executable.resolve("pdftotext")` (system-first, vendored fallback — extend `bundled_path/1` beyond git/rg) for higher-quality layout when available; or (4) ReqLLM-native PDF attachment for file-capable providers with a text-extraction fallback for the rest.
