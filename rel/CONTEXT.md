...
- **EPMD-less distribution via custom `-epmd_module`**: EPMD-less distribution uses a custom `-epmd_module` callback module (`EvoGit.EpmdDist`) that implements the `erl_epmd` behaviour without contacting any external process. The remote daemon registers itself in an ETS table at boot; the local dashboard registers remote-node port mappings (discovered at `Node.connect` time from the SSH tunnel's local port) in the same ETS table. This makes `port_please/2` return the correct port directly — no EPMD process needed, no connection refused errors. Do NOT use `-erl_epmd_port N` instead: it sets the EPMD listening port but does NOT prevent `erl_epmd` from trying to connect to a real `epmd` process on port 4369 (causing `econnrefused` in the absence of a running EPMD).
