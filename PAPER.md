...
EvoX Genesis是一个去中心化的演化式软件开发框架，它结合了大语言模型（LLM）驱动的智能代理与严谨的架构模型，实现自主软件创建与修改。该系统引入了**双维度架构**，将提供语义结构感知的*空间维度*——层次化上下文树——与追踪演化历史的*时间维度*——不可变Git提交的系统发育有向无环图（DAG）——相交融合。在此框架中，**无状态代理**作为纯变换函数运行于 `{commit, node_path} × objective → new_commit`，沿上下文树递归分解任务，从而在无上下文窗口耗尽的情况下实现无界可扩展性。系统支持两个执行阶段——*创世*（引导）和*演化*（迭代修改），其中演化进一步分为*简单*（自顶向下规划）和*复杂*（基于新颖性搜索和质量多样性的自底向上）两种模式。本报告呈现Genesis系统的设计哲学、架构原理、代理分类学、运行时机制，并将其置于演化计算和自主编程代理的更广阔背景中。

---

## 1. Introduction / 引言

### 1.1 Problem Context / 问题背景
...
基于LLM的编程代理的出现已经改变了软件开发实践。然而，现有方法面临根本性的限制。传统编程代理（如Claude Code、GitHub Copilot Workspace）作为单体式、有状态的进程运行，必须将整个代码库上下文加载到单一对话窗口中。这造成了硬性可扩展性上限：随着代码库增长超出上下文窗口的容量，代理的理解能力下降，其进行架构一致性修改的能力崩溃。

与此同时，演化计算领域提供了强大的优化范式——遗传算法、新颖性搜索、质量多样性——但这些传统上作用于低层表示（位串、实值向量），而非真实软件的语义化、架构化结构。

EvoX Genesis通过引入一个框架来弥合这些差距，在该框架中，LLM驱动的代理作为演化算子在具有结构感知的层次化空间中运行，以Git的DAG作为时间基底。
...
1. **结构感知优于扁平文件：** 代码库不是文件的扁平集合，而是语义节点的层次化树，每个节点携带明确的架构契约。代理在空间上导航这棵树，始终在明确定义的范围内操作。

2. **无状态代理优于有状态会话：** 代理是瞬态的、无状态的函数。所有持久化知识驻留在空间维度（上下文树）或时间维度（Git历史）中。这消除了状态损坏，实现了平凡的并行化，并允许任何代理在代码库演化历史的任何点被实例化。

3. **演化进步优于绿色构建：** 系统接受部分进步——实现了多一个功能或通过了多一个测试的版本是有价值的，即使其他部分仍然损坏。这镜像了自然选择的渐进式、方向性改进，并实现了对解空间的快速探索。

---

## 2. The Dual-Dimension Architecture / 双维度架构
...
/ Genesis的核心创新是两个正交维度——空间和时间——的交集，它们共同为代理在代码库生命周期的任何点提供完整的状态规范。

### 2.1 The Spatial Dimension: The Context Tree / 空间维度：上下文树
...
2. **路由表** — 将责任区域映射到子目录，使父代理能够将工作委托给正确的子节点，而无需调查子树的内部。

**上下文继承**自顶向下流动。被分配到`src/auth/oauth/`的代理通过聚合从根 → `src/` → `src/auth/` → `src/auth/oauth/`的显式上下文来构建其世界观。这种继承的上下文提供了架构一致性，而不需要每个代理重新推导整个系统结构。

正式的上下文层次结构下至**目录级别**。一旦代理针对特定文件，它就依赖LLM的自然代码理解能力来导航内部逻辑（文档字符串、注释、代码结构）。这一设计决策反映了现代LLM在文件级理解方面的天然优势，使得显式的子文件建模变得不必要。

### 2.2 The Temporal Dimension: The Phylogenetic Graph / 时间维度：系统发育图
...
空间和时间维度的交集是强大的：代理的完整状态由`{node_path, base_commit, current_commit, objective}`指定——这个元组唯一标识了代理看到什么、从哪里开始、当前在哪里以及必须实现什么。

---

## 3. The Stateless Agent Model / 无状态代理模型
...
代理接收其空间分配（`node_path`）、时间锚点（`base_commit`——它从中分支的提交）、当前时间位置（`current_commit`——它迄今为止产生的结果）以及自然语言`objective`。它返回一个新状态，通常通过一次或多次Git提交推进`current_commit`。

关键的是，代理维护**没有长期记忆**。它们完全依赖：
- 上下文树（继承的空间上下文）用于架构感知
- Git历史（时间维度）用于理解先前的工作和决策
- 短期会话记忆（与LLM的当前对话），在完成时被丢弃

这种无状态性具有深远的影响：
- 任何代理都可以在代码库演化的任何点被实例化
- 代理可以被平凡地并行化——没有共享的可变状态
- 状态回滚就像签出历史提交一样简单
- 代理崩溃是可恢复的——调度器从最后提交的状态重新分发

### 3.2 Agent Taxonomy / 代理分类学
...
Genesis采用专门的代理分类学，其中每种代理类型具有不同的角色、工具集和委托配置。代理按其与上下文树的交互类型进行分类：

**读写代理**（可以修改代码）：
| 代理 | 角色 | 描述 |
|-------|------|-------------|
| **Manager** | 编排者 | 规划、委托、验证——不直接实现。通过子代理协调工作，解决冲突并确保质量。 |
| **Generalist** | 全栈工程师 | 多面手代理，能自主调查、规划和实现。可以委托给调查者和执行者。 |
| **Executor** | 实现专家 | 接收明确定义的目标并执行精确、有针对性的代码修改。严格专注于分配范围。 |
| **CodebaseArchitect** | 绿地设计师 | 从零开始创建项目骨架。分三阶段工作：骨架设计 → 实现 → 审查。 |
| **SkillExtractor** | 知识蒸馏器 | 分析已完成的工作，将可重用的知识蒸馏为技能，供未来代理使用。 |

**只读代理**（可以读取和更新CONTEXT.md，但不能修改代码）：
| 代理 | 角色 | 描述 |
|-------|------|-------------|
...
### 3.3 Recursive SubAgent Delegation / 递归子代理委托
...
实现无界可扩展性的关键机制是**递归子代理委托**。当代理遇到属于上下文树子节点的工作时，它不会尝试直接处理。相反，它：
...
关键属性是**子代理的上下文足迹不计入父代理的会话限制**。父代理只接收压缩的结果（文本摘要、差异统计、提交SHA）——而不是子代理的完整对话历史。这意味着：

- 根节点的父代理可以委托给子代理，子代理再委托给孙代理，依此类推——到任意深度——而不会耗尽任何单个代理的上下文窗口。
- 递归结构镜像了上下文树的层次结构，确保每个代理仅在其空间范围内操作。

这与传统编程代理根本不同，在传统代理中，所有工作都在单一对话上下文中进行，该上下文随工作范围线性增长。

### 3.4 Enforcing the Spatial Contract / 空间契约的强制执行
...
1. **只读代理只能生成只读子代理。** 写权限不能通过委托升级——如果父代理仅被授权读取，其子代理不能执行未授权的写入。

2. **读写代理可以生成两种类型，但读写子代理必须在父代理分配节点的相同或子节点内操作。** 写范围不能升级到超出父代理的权限。

3. **文件操作是节点范围的。** 被分配到`src/foo/`的代理只能修改`src/foo/`及其后代目录内的文件。

这种层次化权限系统确保代理尊重架构边界，不能对代码库的其他部分进行未授权的修改。它将上下文树从被动的文档结构转变为主动的强制执行机制。

---

## 4. Runtime Execution Phases / 运行时执行阶段

### 4.1 Phase 1: Genesis (Bootstrapping) / 第一阶段：创世（引导）
...
**模式A — 现有代码库（上下文提取）：**
系统在仓库根目录生成一个`ContextExtractor`代理。该代理递归下降到目录结构中，为每个子目录生成子代理来分析代码、提取API、记录意图并建立上下文树。该过程使用**不动点收敛**：初始提取后，父代理审查聚合的上下文以发现差异，并可能生成纠正子代理。这个循环重复直到上下文稳定（达到不动点）。

**收敛断路器**防止无限循环：代理*仅*基于功能性API表面修改（而非措辞或风格偏好）评估上下文变更，硬性迭代限制保证数学上的终止性。

**模式B — 新代码库（架构与实现）：**
`CodebaseArchitect`代理解释用户的提示并在根节点草拟初始架构计划。然后它分三阶段运行：
...
**模式A — 简单演化（自顶向下）：**
用于清晰的、明确定义的任务，其中到解决方案的路径是已知的。目标节点处的`Manager`代理：
1. 通过读取上下文树路由表映射空间上下文
2. 分析目标以识别所需步骤
3. 对于复杂目标，可选地生成`TaskScheduler`来产生结构化执行序列
4. 将每个步骤委托给相应子节点中的`Executor`子代理
5. 验证结果，解决冲突，并迭代直到目标达成

分解遵循上下文树：代理仅编辑其分配节点级别内的文件，对任何子节点工作委托给子代理。这确保了**递归实现**——实现结构镜像架构结构。

不动点收敛适用：如果子代理完成后目标未达成，父代理生成新的子代理继续迭代，直到完成或达到会话限制。
...
5. **解合成：** 演化收敛（或达到最大代数）后，系统收集最新颖和最多样化的片段，并要求LLM合成一个连贯的解决方案。该解决方案通过Manager代理应用于代码库。

这种自底向上的模式体现了**算法意外发现**——系统被设计为通过利用LLM的语义理解来桥接不相交的领域，从而发现意想不到的解决方案。

---

## 5. The Agent Scheduler and Git Isolation / 代理调度器与Git隔离

### 5.1 Worktree-Based Isolation / 基于工作树的隔离
...
Genesis通过类似于操作系统进程调度的**代理调度器**管理执行。受约束的资源是固定数量的Git工作树槽位——仓库的隔离工作副本。此设计提供了几个关键属性：

1. **主签出的不可变性：** 用户的主要工作副本从不会被任何代理直接修改。所有变更都发生在隔离的工作树中。

2. **执行生命周期：** 代理从`waiting`状态开始。当工作树槽位可用时，调度器将其分配给等待中的代理，签出代理的精确`current_commit`，并开始执行。这确保恢复的代理不会覆盖先前的进展。

3. **协作式多任务（让出）：** 工作树不能被空闲锁定。当代理需要生成子代理时，它必须让出：提交待处理变更，转换为`waiting`状态，并释放工作树。调度器随后可以将工作树分配给其他排队的代理（包括新生成的子代理）。子代理完成后，父代理重新排队等待工作树以恢复。

4. **持久化工作树：** 工作树在重试之间为每个代理持久化——在初始分发时创建，在重试时重用，仅在最终完成时清理。这在崩溃之间保留了部分工作。

### 5.2 Slot Management and Rate Limiting / 槽位管理与速率限制
...
## 6. Genesis vs. Traditional Evolutionary Algorithms / Genesis与传统演化算法的比较

### 6.1 Fundamental Differences / 根本差异
...
**1. 树状演化 vs. 扁平种群：**
传统EA演化扁平的个体种群，每个个体是表示为固定长度编码的完整候选解。Genesis相反，演化一棵*代理树*，递归地分解问题。每个代理独立地演化其局部子树，父代理协调集成。这镜像了复杂有机体如何发育：不是作为扁平基因组，而是作为层次化分化过程，其中每个细胞根据其在发育树中的位置而特化。

**2. 递归展开 vs. 世代迭代：**
传统EA在离散世代上迭代：评估所有个体 → 选择 → 变异 → 替换。Genesis使用**递归展开**：根节点处的代理将其目标分解为子目标，每个子代进一步分解，以此类推直到到达叶级任务。这更接近递归函数求值，而非基于种群的优化。"世代"概念被递归委托的深度所取代。

**3. 不动点收敛 vs. 适应度优化：**
传统EA在种群的平均适应度稳定于最优附近时收敛。Genesis在上下文树达到**不动点**时收敛——即进一步代理迭代不产生功能性API表面变更的状态。这类似于指称语义中的不动点语义或迭代方程求解器的收敛。收敛断路器（仅基于功能变更的评估、硬性迭代限制）保证此过程终止。
...
### 6.2 Conceptual Positioning / 概念定位
...
- **发育过程**是递归代理委托树——每个代理根据其节点位置和目标"分化"。
- **表型**是最终的代码库。

这种发育视角解释了为什么Genesis能够产生架构一致的结果：上下文树提供位置信息（类似于生物学中的形态发生素梯度）来引导每个代理的特化，而空间契约确保每个代理的贡献与整体协调地集成。

---

## 7. Genesis vs. Traditional Coding Agents / Genesis与传统编程代理的比较

### 7.1 Architectural Differences / 架构差异
...
基于LLM的编程代理领域快速扩展，Claude Code、GitHub Copilot Workspace、Cursor和Aider等工具获得了广泛采用。Genesis在几个根本方面与这些不同：

**1. 无状态代理 vs. 有状态会话：**

传统编程代理（如Claude Code）作为单一的有状态对话运行。代理加载上下文、进行修改并迭代——所有这些都在一个连续会话中。代理的"记忆"是对话历史，单调增长。

Genesis代理是**无状态函数**。代理的状态完全由`{node_path, base_commit, current_commit}`捕获。调用之间不携带对话历史——只有空间上下文（上下文树）和时间上下文（Git历史）持久化。这意味着：
- 代理可以被平凡地并行化（无共享可变状态）
- 崩溃完全可恢复（从最后提交重新分发）
- 上下文不会累积超过单个代理的会话
...
传统代理面临硬性可扩展性上限：整个相关代码库上下文必须适合LLM的上下文窗口。随着项目增长，代理要么遗漏重要上下文（质量下降），要么超出窗口（失败）。

Genesis通过**具有上下文隔离的递归分解**解决此问题。每个代理仅在其节点范围内操作，只看到：
- 继承的上下文树路径（轻量级、结构化）
- 分配节点内的文件
- 已完成子代理的结果（压缩摘要，而非完整对话）

子代理的上下文足迹不计入父代理的限制。这使得能够处理**任意大的代码库**——递归委托的深度随项目的架构深度扩展，而非随其总大小。

**3. 空间契约强制执行 vs. 无约束编辑：**

传统代理对整个代码库有不受限制的访问权。被要求修复`src/auth/`中bug的代理如果认为必要，也可能修改`src/db/`或`src/api/`。这可能导致架构违规和意外的副作用。

Genesis强制执行**空间契约**：被分配到`src/auth/`的代理只能修改`src/auth/`及其后代内的文件。写权限是层次化范围的，不能升级。这将代码库从无差别的blob转变为具有明确边界的结构化空间。

**4. 系统发育演化 vs. 线性编辑：**

传统代理线性工作：进行修改、用户审查、重复过程。没有分支、并行探索或选择性合并的概念。

Genesis使用**系统发育DAG**：代理从特定提交分支、独立探索、成功分支被选择性合并。这实现了：
- 多种方法的**并行探索**
- **部分进步接受**——修复一个bug的分支即使其他bug仍然存在也是有价值的
- **时间回滚**——任何代理可以检查或从任何历史提交分支
- **二分式调试**——在不同提交处生成代理以识别回归何时引入

**5. 架构感知 vs. 扁平文件感知：**

传统代理将代码库感知为扁平的文件集合。它们通过探索（搜索、读取文件）发现结构，这消耗上下文窗口容量且本质上不完整。

Genesis通过上下文树提供**继承的架构上下文**。`src/auth/oauth/`处的代理自动知道：
- `src/auth/`做什么（从其CONTEXT.md）
- 整个系统做什么（从根CONTEXT.md）
- 在哪里委托工作（从路由表）
- 适用什么约束（从每一级的约束）

这种架构感知是**结构化的，非发现的**——它作为代理初始化的一部分被继承，而非通过昂贵的探索推导。

### 7.2 Summary Comparison Table / 总结比较表
...
## 8. Additional Design Features / 其他设计特性

### 8.1 Dynamic Skills System / 动态技能系统
...
Skills are **globally defined** but **hierarchically enabled** per Context Tree node — each CONTEXT.md may specify which skills are active at that level, and skills inherit downward through the tree. This spatial enablement ensures that agents only see skills relevant to their assigned domain.
...
技能是**全局定义**但按上下文树节点**层次化启用**的——每个CONTEXT.md可以指定在该级别激活哪些技能，技能在树中向下继承。这种空间启用确保代理只看到与其分配领域相关的技能。

**SkillExtractor**代理自动分析已完成的工作，将可重用的知识蒸馏为新技能，创建一个自我改进的系统，其中成功的策略被捕获以供未来代理使用。这镜像了演化系统中*遗传记忆*的概念——成功的适应性被编码并由后代继承。

### 8.2 Multi-Repository Support / 多仓库支持
...
外部仓库在阶段初始化时注册并按代理跟踪。为维护安全性，外部仓库中只允许只读调查——有写能力的代理不能修改外部代码库。

### 8.3 Multi-Platform Sandboxing / 多平台沙箱化
...
这确保即使LLM生成恶意或有缺陷的代码，爆炸半径也仅限于代理的工作树，不能影响主机系统。

### 8.4 Context Compression / 上下文压缩
...
当代理的总令牌数超过可配置阈值时，框架自动压缩聊天历史。这种压缩保留基本信息（做出的决策、修改的文件、关键发现），同时丢弃冗长的中间输出。这允许单个代理在其节点内处理复杂任务而不会达到硬性上下文限制，同时仍然维护无状态原则——压缩的上下文是瞬态的，在代理完成时被丢弃。

### 8.5 Delegation Hinting / 委托提示
...
当代理重复编辑子目录（低于其分配节点）中的文件时，框架跟踪这些写操作。超过阈值（默认：对同一子目录5次写入）后，工具输出中会附加一个温和的提示，建议代理为该子树生成子代理。这鼓励正确的层次化分解而不强制执行——在子目录中进行快速、一次性编辑的代理不会被中断，但实际上"居住"在子目录中的代理会被引导走向正确的委托。

---

## 9. Implementation Technology / 实现技术
...
- **Actor模型并发：** Elixir的轻量级进程和消息传递模型自然地镜像无状态代理设计。每个代理作为隔离进程运行，调度器通过消息传递协调它们。
- **容错性：** OTP的监督树提供崩溃恢复和重试逻辑——这对于LLM API调用可能失败和代理可能崩溃的系统至关重要。
- **模式匹配与不可变性：** Elixir的函数式范式强制不可变性，防止了困扰面向对象代理实现的共享状态错误。
...
1. **双维度架构** ——将空间上下文树与时间系统发育DAG相交，为任何演化点上的任何代理提供完整的状态规范。

2. **无状态代理模型** ——代理作为`{commit, node_path} × objective`上的纯函数，通过具有上下文隔离的递归分解实现无界可扩展性。
