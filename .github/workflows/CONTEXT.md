...
- **VERSION never enters the workflows**: build-desktop.yml has zero references to the root `VERSION` file or the release tag (except `release_tag` for the publish step); versions are baked at build time from tauri.conf.json (synced by `mix bump.version`, `apps/evo_git/lib/mix/tasks/bump.version.ex`) and the mix releases. An updater manifest would need a new step reading `github.event.release.tag_name` or VERSION.
