...
- **Publish step**: `publish-release` job (build-desktop.yml:789-878) uses `softprops/action-gh-release@c12583777ecdfd3be55c69cf75464299dc01057e # v3` — that SHA is the **annotated-tag object of v3.0.2** (the REST API cannot resolve it as a commit; `git cat-file` on a clone resolves it to `3d0d988 release 3.0.2 (#818)`). v3.0.2 defaults: `overwrite_files: true` (re-runs for the same tag overwrite same-named assets → re-releases are idempotent), `make_latest` unset → GitHub API default (true for non-prerelease), `fail_on_unmatched_files: true` set explicitly. No `body`/`name`/`draft`/`prerelease`/`generate_release_notes` inputs — on the release-event path the action updates the existing release and uploads assets; on `workflow_dispatch`+`publish_release` it creates the release for `inputs.tag`. `concurrency.cancel-in-progress: true` (lines 29-34) means re-publishing the same tag cancels the previous run mid-flight — a partially-updated release is possible until the new run completes (end state converges; no stale-asset cleanup: removed asset names stay attached). The `Generate updater manifest (latest.json)` step (build-desktop.yml:817-871) runs between artifact download and the upload step.
