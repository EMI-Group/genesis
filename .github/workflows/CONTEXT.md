...
- **`mix release` hangs on cached `_build`**: the Mix build cache (`_build/`) is keyed by OTP/Elixir/mix.lock, not the project version. A restored cache contains an already-built release for the current version, and `mix release` blocks on the interactive `Release X already exists. Overwrite? [Yn]` prompt — which hangs forever in non-TTY CI. All three platform jobs therefore pass `--overwrite` to `mix release` (both `genesis_desktop` and `genesis_remote`). Never remove `--overwrite`, and keep it in sync across all three jobs.
