...
- **Updater machinery (CI side, active pre-key)**: the three desktop build jobs pass `TAURI_SIGNING_PRIVATE_KEY` + `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` as env to `tauri build`; with `bundle.createUpdaterArtifacts: true` in tauri.conf.json, tauri 2.11.4 auto-emits signed updater payloads. Without the key, tauri fails ("A public key has been found, but no private key" — tauri#14581), so each build step falls back to `--config '{"bundle":{"createUpdaterArtifacts":false}}'` (RFC 7396 JSON merge) via a `UPDATER_ARGS` bash array appended to the command line. Updater payloads are staged under stable names: macOS re-tars the now-stapled app and re-signs it with `tauri signer sign`; Linux x64 stages `*.AppImage.tar.gz` + `.sig` (arm64 has no AppImage → skipped); Windows stages `*.exe.sig` beside the NSIS installer (the installer IS the payload — `bundle/msi/*.msi.sig` is deliberately NOT staged, MSI is excluded from the feed). `publish-release` then generates `dist/latest.json` — only for platforms with BOTH payload and `.sig` present; with no signed payloads the file is simply not written (no empty manifest) and nothing extra is uploaded. The unversioned installers still rely on GitHub's `releases/latest/download/` redirect (README.md:164-177); `latest.json` URLs are versioned (`/releases/download/<tag>/`). No release-asset checksums are generated (sha256 checks in the workflow verify only vendored ripgrep/MinGit downloads).
...
**User handoff — activating auto-update** (CI is ready; only these three steps remain):
1. Generate a minisign keypair: `tauri signer generate -w ~/.tauri/genesis.key` (writes `~/.tauri/genesis.key` + `~/.tauri/genesis.key.pub`).
2. Put the FULL contents of `genesis.key.pub` into `desktop/src-tauri/tauri.conf.json` → `plugins.updater.pubkey`, replacing the placeholder. The placeholder MUST be replaced before (or before-or-with) adding the secrets, so the embedded pubkey matches the signing key.
3. Add `TAURI_SIGNING_PRIVATE_KEY` (the key file content) + `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` as repository secrets.
