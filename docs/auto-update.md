...
3. **Prerequisites before any of this works:** a minisign (Ed25519) keypair for the update feed, Windows Authenticode certificate, macOS `.app.zip` stapling (currently only the DMG is stapled), and a manifest-generation step in `publish-release`.
...
- The Cloudflare worker behind `https://genesis.evox.group/dl/` (serving `genesis_remote_*` with mainland-China proxying; source **not in this repo**) is the natural host for the update manifest; nothing exists there yet beyond the remote tarballs.
...
1. **GitHub Releases (zero new infra):** `publish-release` gains a step that generates `latest.json` (per-platform entries, signatures from the build) and uploads it; the app points at `https://github.com/<owner>/<repo>/releases/latest/download/latest.json`. Unversioned names are already permanent (`releases/latest/download/<name>`); versioned URLs (`releases/download/<tag>/<name>`) are also permanent and support pinning.
2. **genesis.evox.group (China-friendly):** extend the existing Cloudflare worker (source is external to this repo — needs a separate repo/ownership decision) to serve the manifest + proxied installers, reusing the mainland-China auto-detection the remote tarballs already use.
...
1. **Windows certificate:** who owns the Authenticode cert (cost ~$100–400/yr) — mandatory for a trustworthy auto-update on Windows?
2. **Linux repos:** host official apt/dnf repos, or ship installers + notify and let distros package? (Repos are the "platform-recommended" answer but add infra.)
3. **Update cadence & auto-download policy:** check on startup only, or also every N hours? Auto-download in background, or download only on user confirmation?
4. **Staged rollout:** is a beta/stable channel split wanted from day one?
5. **macOS:** official updater plugin (one code path, custom UI) vs Sparkle (native UX, third-party plugin)? Recommendation: official plugin first; Sparkle only if native UX becomes a requirement.
6. **Remote daemons:** include the remote-update flow (§6) in this effort or defer?
7. **`:interrupted` status vs `:cancelled` + reason metadata:** status is the cleaner contract but touches ~10 hardcoded lists; metadata is cheaper. Preference?
