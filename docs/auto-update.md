...
- **`publish-release` generates the updater manifest:** a `Generate updater manifest (latest.json)` step writes `dist/latest.json` (per-platform `{url, signature}` entries whose `url` points at `https://genesis.evox.group/dl/<payload>`, plus `version`/`notes`/`pub_date`; a platform enters only when both its payload and its `.sig` exist, and when none qualify the file is not written at all), which is then uploaded with the rest of the artifacts via `files: dist/*`. No checksum files are generated. The upload uses `softprops/action-gh-release@v3.0.2` (idempotent overwrite, `fail_on_unmatched_files`). Artifact names are deliberately **unversioned** so `releases/latest/download/<name>` is permanent — a versioned feed would use `releases/download/<tag>/<name>` (also permanent).
- The Cloudflare worker behind `https://genesis.evox.group/dl/` (mainland-China proxying; source **not in this repo** — it lives in the external `genesis-doc` repository) is the app-facing host for updates: the updater feed endpoint and the per-platform manifest payload URLs in `tauri.conf.json` / the `publish-release` manifest now point at `/dl/` (this upgrade path has been executed — see §5.2). The worker serves the updater feed (`/dl/latest.json`) and the desktop updater payload names (`genesis_desktop_darwin_arm64.app.tar.gz`, `genesis_desktop_windows_x64.exe`, …) alongside the `genesis_remote_*` tarballs. **Accepted URL forms**: `/dl/<asset>` and `/dl/latest/<asset>` (the latter aliasing GitHub's latest release) proxy the asset from `releases/latest/download/<asset>`; `/dl/<tag>/<asset>`, e.g. `/dl/v0.12.6/genesis_desktop_linux_x64.AppImage`, proxies it from that exact tag (`releases/download/<tag>/<asset>` — exactly GitHub's own shape); `/dl` and `/dl/` redirect to the docs site's `/#download`. The asset name must be in the worker's fixed allowlist and the tag well-formed, else 404; `?mirror=1` forces the edge-streamed path on any form. The tag-scoped form ships only in the worker source and is live only after a manual `pnpm deploy` in `genesis-doc`, so shipped artifacts deliberately keep the unversioned `/dl/<asset>` form until then.
...
1. **GitHub Releases (zero new infra):** `publish-release` generates `latest.json` (per-platform entries, signatures from the build) and uploads it to the release. Unversioned names are permanent (`releases/latest/download/<name>`); versioned URLs (`releases/download/<tag>/<name>`) are also permanent and support pinning. **Adopted as the upstream/fallback baseline:** GitHub release assets remain the upstream source the Cloudflare worker proxies, and the GitHub feed URL (`https://github.com/<owner>/<repo>/releases/latest/download/latest.json`) is kept as a secondary fallback endpoint in `tauri.conf.json` (tried after the proxy primary). The worker mirrors both conventions: its `/dl/<asset>` and `/dl/latest/<asset>` forms map to the unversioned shape, its `/dl/<tag>/<asset>` form to the versioned one (§5.2).
2. **genesis.evox.group (China-friendly) — CHOSEN production path:** the existing Cloudflare worker (source is external to this repo — needs a separate repo/ownership decision) serves the manifest + proxied installers, reusing the mainland-China auto-detection the remote tarballs already use. Executed: the app feed endpoint (`plugins.updater.endpoints` in `tauri.conf.json`) points at `https://genesis.evox.group/dl/latest.json`, and the `publish-release` manifest's per-platform payload `url`s point at `https://genesis.evox.group/dl/<filename>` (matching the `/dl/` smart-download convention of `EvoGit.RemoteBootstrap.download_url/1`). The worker serves both: `/dl/latest.json` resolves (streamed from the CF edge for covered countries / `?mirror=1`, otherwise a 302 to the GitHub release manifest) and the desktop updater payload names are served alongside the `genesis_remote_*` tarballs. The worker's accepted forms are `/dl/<asset>` and `/dl/latest/<asset>` (both → the latest GitHub release), `/dl/<tag>/<asset>` (→ that exact tag, GitHub's `releases/download/<tag>/<asset>` shape), and `/dl`/`/dl/` → the docs site's `/#download`; the tag-scoped form ships in the worker source only and is live only after a manual `pnpm deploy` in `genesis-doc`, so the app feed and the CI manifest payload `url`s deliberately stay on the unversioned `/dl/<asset>` form (an already-installed app resolves its manifest payload URL at update-check time, so adopting the tag-scoped form before that deploy would 404 every user's payload download). One gap remains: no Linux updater payload is published yet — the published `latest.json` carries only the `darwin-aarch64` and `windows-x86_64` platform entries.

**"Push" semantics:** desktop updaters are pull-based by nature. "Push-like" behavior = periodic checks (startup + interval) plus a lightweight version check for the dashboard notification banner. The worker serves no separate version-only endpoint — a request for `GET https://genesis.evox.group/dl/latest-version.json` 404s as an unknown asset; the banner instead derives the latest version from the full updater feed at `https://genesis.evox.group/dl/latest.json` (the manifest also reachable via the `latest` alias `/dl/latest/latest.json`), which is what the dashboard actually fetches. A true push channel (WebSocket/APNs/WNS) is unnecessary complexity — the periodic check with a visible notification satisfies the requirement.

### 5.3 What CI must add (`build-desktop.yml`)
1. **Configure the signing secrets.** Every staged desktop artifact is minisign-signed (`<artifact>.sig`), the per-platform updater payloads are emitted, and `latest.json` is built from those `.sig` files — all only when the **`TAURI_SIGNING_PRIVATE_KEY`** (+ `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`) repository secrets are set; with the key absent `tauri build` falls back to `--config '{"bundle":{"createUpdaterArtifacts":false}}'`, no `.sig` is staged, and the manifest carries no entry for that platform.
...
- New step between the artifact download (L766-771, `actions/download-artifact`) and the upload step (L784-792, `softprops/action-gh-release` with `files: dist/*`, creating a draft that `gh release edit --draft=false` flips public at L794-801):
  ...
  - Collect the updater payloads present in `dist/` — `darwin-aarch64` → `.app.tar.gz`, `linux-x86_64` → AppImage `.tar.gz` (x64 only), `windows-x86_64` → NSIS exe. For each: `url` = `https://genesis.evox.group/dl/<filename>` — the unversioned `/dl/` smart-download URL the Cloudflare worker resolves against the latest GitHub release (the GitHub `releases/download/<tag>/<filename>` remains the upstream the worker proxies). CI deliberately keeps this unversioned single-segment form: the worker also accepts the tag-scoped `/dl/<tag>/<filename>` form (GitHub's `releases/download/<tag>/<filename>` shape), but that form is live only after the external worker deploy, and a manifest payload `url` is fetched at update-check time by already-installed apps — switching to it early would 404 every user's payload download. `signature` = **full contents of the `.sig` file** (including the `untrusted comment:` header lines — the plugin requires the whole file content, not a URL).
