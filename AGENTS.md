...
* **The Spatial Contract (Directory):** Every `Directory Node` *must* contain a `CONTEXT.md` file. This file is not treated as a normal code file within the Git repository. It is conceptually bound to the directory as an intrinsic attribute (similar to an `xattr`) acting as the directory's schema to define its **Intent**, **API Surface**, **Constraints**, and **Routing Table**.
  * **Routing Table:** A critical section that maps areas of concern, functionality domains, or module categories to specific child node paths. This serves as a "next-level navigation map" so that a parent agent can quickly determine *where* to spawn subagents for specific tasks. For example, a root `CONTEXT.md` might specify: "Core runtime concerns → `apps/evo_git/`", "Web dashboard → `apps/evo_dash/`", "Configuration → `config/`". Each child directory's `CONTEXT.md` further refines this mapping recursively, creating a complete navigable hierarchy of responsibilities.
