...
- **`req_llm`** Finch streaming pool in `runtime.exs` (lines 23–66) — **dynamically sized** from the **total LLM concurrency across all model profiles** (`[[llm.models]]` → `concurrency` per profile). When no profiles are configured (fresh install / legacy flat `[llm]` config), falls back to `[scheduler] default_llm_max_concurrency`. Configured via the full `finch:` override form (NOT the `stream_pool_*` shorthand — those keys are only consumed by ReqLLM's `get_default_pools/0`, which is bypassed when `finch.pools` is set):
  - `finch.pools.default.count`: `max(sum_of_profile_concurrencies + 2, 8)` — number of pool processes (each holds 1 connection, `size: 1`, so capacity = `count` concurrent HTTP/1 streams) in ReqLLM's Finch pool. For single-model config with default `concurrency=3`, this is `max(3+2, 8) = 8` (ReqLLM's own default). For multi-model configs, the pool grows to accommodate all models running concurrently.
  - `finch.pools.default.size`: 1 — connections per pool process
  ...
  - `stream_pool_timeout`: 120_000 ms (2 min) — top-level key, read at CALL time (`streaming/finch_client.ex:300`), NOT part of the pool config — time to wait for a free pool connection
  - Runs at boot *before* `:req_llm` starts its Finch pool, sizing it correctly. The `+2` buffer covers auxiliary non-slot-gated LLM calls — the only such calls today are the LLM self-check (`system_check.ex`) and PR-title generation (`pull_request.ex`); context compression IS slot-gated.
