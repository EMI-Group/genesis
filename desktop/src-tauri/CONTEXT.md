...
- **Pubkey handoff — DONE (Aug 16 2026)**: `tauri.conf.json` → `plugins.updater.pubkey` holds a REAL minisign public key since commit `4281ae8aa` ("Add tauri auto update public key") — the placeholder `"REPLACE_WITH_TAURI_SIGNER_GENERATED_PUBLIC_KEY"` is gone. `check_update`/`download_update` therefore skip the app-level `not_configured` gate (`updater_pubkey_is_placeholder/1` in `main.rs:71-79` now only fires for a genuinely missing/empty/whitespace key — e.g. binaries BUILT BEFORE that commit, since the check reads the build-time `app.config()`) and proceed to the real plugin check. Remaining handoff items: the `TAURI_SIGNING_PRIVATE_KEY` / `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` CI secrets (root `.github/workflows/build-desktop.yml`) and actually publishing a signed per-platform payload + `latest.json` entry for EVERY platform. Pubkey is read from `app.config().plugins.0.get("updater").get("pubkey")`; `plugins` is a `HashMap<String, JsonValue>` — hence the `.0` tuple access.
