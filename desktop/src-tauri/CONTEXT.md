...
- **Fix (workflow — root scope, NOT this subtree)**: install wxWidgets 3.2 (`ppa:wxformbuilder/wxwidgets3.2` on jammy) + `libglu1-mesa` on the Linux **x64** runner before `tauri build` (arm64 job bundles no AppImage). This both fixes the build and makes the AppImage self-contained for the wx picker (linuxdeploy bundles the wxWidgets closure into `usr/lib/`). The exact snippet is recorded in the root agent's final report for the v0.9.5 AppImage incident; verify the PPA package name (`-t64` vs not) with `apt-cache search libwxgtk3.2` on the runner.
