...
- **Placeholder pubkey handoff**: `tauri.conf.json` → `plugins.updater.pubkey` is the literal `"REPLACE_WITH_TAURI_SIGNER_GENERATED_PUBLIC_KEY"`. The user must run `tauri signer generate`, paste the generated public key over the placeholder, and add the `TAURI_SIGNING_PRIVATE_KEY` / `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` secrets to CI (`.github/workflows/build-desktop.yml`, root scope). Until then every update command reports `not_configured` (pubkey read from `app.config().plugins.0.get("updater").get("pubkey")`; `plugins` is a `HashMap<String, JsonValue>` — hence the `.0` tuple access).
