...
| `Cargo.toml` | Rust dependencies (tauri v2 with `devtools` + `tray-icon` features, tauri-plugin-shell, tauri-plugin-single-instance, reqwest, rfd) |
...
- **Tray menu "Quit Genesis"** → shows a **native confirmation dialog** (`rfd::MessageDialog`, Yes/No, Warning level — warns that running tasks will be interrupted) before quitting; only on confirmation, `BackendManager::kill_for_quit()` (sets the `intentional_shutdown` flag BEFORE taking and killing the child, so the backend watchdog never restarts after a quit began — double-kill guard), then `app.exit(0)`. The `rfd` crate (Rusty File Dialogs) is a pure Rust dependency — NOT a Tauri plugin, so no capabilities/permissions change is needed (deliberately avoids the unreliable `tauri-plugin-dialog`).
