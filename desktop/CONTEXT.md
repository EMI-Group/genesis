...
3. The WebView window opens, pointing to `http://localhost:<PORT>` (the window is created in Rust with `WebviewUrl::External`; the config `window` and `devUrl` were removed from tauri.conf.json). The window's initial load races the backend boot and typically fails with connection refused; once the readiness poll (step 2) succeeds, the setup re-navigates the webview to the dashboard via a bounded retry (~20 × 250ms, breaking early on quit/update intent — `navigate_after_ready` in `src-tauri/src/main.rs`). This re-navigation is required for a healthy boot: without it the webview would sit on the failed-load page for the whole session and the dashboard's `quit-requested` listener would never load, wedging the tray Quit flow (the watchdog only navigates on the crash-recovery path)
...
1. **Tray "Quit Genesis"** → Rust shows+focuses the main window (unminimize → show → set_focus, same as the "Show Window" arm and the single-instance callback), then probes backend health (`sidecar::probe_http`).
   - **Backend healthy** → Rust emits the Tauri event `quit-requested` (payload `()`) and does NOTHING else — no kill, no exit. **Known gap**: because the webview is loaded from an EXTERNAL URL (`http://127.0.0.1:<port>`), the page is a Remote origin to Tauri's ACL and the dashboard's `event.listen("quit-requested")` (plugin command `plugin:event|listen`) is silently rejected unless a `remote` capability context is added to `capabilities/default.json` — see `desktop/src-tauri/CONTEXT.md` → Known Issues. Until fixed, emitted events reach no JS listeners and the 9s tray-Quit fallback kills the app.
