...
- **Headless `genesis-desktop --headless` / backend COOKIE in the Nix store — FIXED**: the mix release launcher reads `releases/COOKIE` (or `RELEASE_COOKIE` env) and the read-only Nix store cannot regenerate it — previously the backend failed to boot with `cat: …/releases/COOKIE: No such file or directory`. **Fixed** in `genesis.nix` by setting `removeCookie = false` on the `beamPackages.mixRelease` call (nixpkgs deletes the cookie in postFixup by default) and baking a deterministic cookie into `$out/releases/COOKIE` in `postInstall`. Validated: the nix-built desktop backend now boots and serves HTTP 200 at localhost:9999.
