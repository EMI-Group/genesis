...
No auto-update machinery exists anywhere in this subtree (current state): no `tauri-plugin-updater` in `src-tauri/Cargo.toml` or `Cargo.lock` (direct or transitive), no `plugins.updater`/`updater` section in `src-tauri/tauri.conf.json`, no updater permissions in `src-tauri/capabilities/default.json` (only `core:default`, `shell:allow-execute`, `shell:allow-spawn`), and no update-related code/comments in the Rust sources. App version surfaces as literals in `tauri.conf.json` `version` (drives installer and would-be updater version comparison) and `Cargo.toml` `version`, both synced by `mix bump.version`; the Rust code never reads its own version. A **local** `tauri build` with `bundle.targets: "all"` produces updater-compatible artifacts, but CI (`build-desktop.yml`) passes explicit `--bundles` flags, so the **published** artifacts are: macOS → `.dmg` + a `ditto -c -k --keepParent` `.app.zip` (L556, L594-595) — **NO `.app.tar.gz`, the artifact tauri-plugin-updater's macOS installer expects**; Windows → all targets (MSI + NSIS `.exe`, staged L711-712); Linux x64 → `deb,rpm,appimage` + portable `.tar.gz`; Linux arm64 → `deb,rpm` only (matrix L156-168). Updater feed candidates today: the NSIS `.exe` (Windows), the x64 AppImage (Linux); macOS requires a CI change to also produce/stage the `.app.tar.gz`. Integration-relevant facts: the bundled backend resource `resources/genesis-backend` (gitignored; populated only before `cargo tauri build`) is a standard mix release whose launcher `bin/genesis_desktop` resolves the active version via `releases/start_erl.data` — the launcher path is version-independent, so replacing the resource dir (backend-only update) needs no Rust spawn-logic changes; the `begin_quit` flag → backend `System.stop()` → watchdog `app.exit(0)` flow is the natural pre-install hook for a download-and-install flow. CI artifact publishing/manifest signing lives in the root `.github/workflows/` (out of this subtree).
