...
- **`--headless` mode is NOT covered**: `run_headless()` bypasses the Tauri builder entirely (no plugin, no window, no watchdog). The old port-collision risk is gone — each headless instance picks its own free port — but the parent-death monitor does apply: `headless_sidecar_env` sets `EVOGIT_DESKTOP=1` + `EVOGIT_PARENT_PID`, so a headless backend self-exits ~2s after its parent shell dies (including SIGKILL orphans). This is a dev/debug utility and remains deliberately out of scope.
...
- **Orphaned backend on abnormal shell death (fixed)**: when the shell is killed abnormally (macOS Force Quit, crash/panic, `kill -9`) the backend child used to survive and keep port 9999 bound, so the next launch crashed with "port 9999 is already in use" (the new backend failed to bind; the watchdog entered the infinite error-page retry). Two-part fix: (a) **dynamic ports** — the shell picks a free ephemeral port at startup (`TcpListener::bind("127.0.0.1:0")`), so a fresh launch never reuses a zombie's port (the OS won't assign a bound port to `bind(0)`); (b) **parent-death monitor** — the shell passes its own pid via `EVOGIT_PARENT_PID` in the sidecar env, and the new `EvoGit.DesktopParentMonitor` in apps/evo_git checks it every ~2s and calls `System.stop(0)` when the pid is gone (gated on `EVOGIT_DESKTOP=1`), so the backend self-exits within ~2s of the shell dying. Side effect: the single-instance macOS simultaneous-launch race is now harmless (both instances' backends get different ports). Reference `desktop/src-tauri/CONTEXT.md` for the Rust-side details.
