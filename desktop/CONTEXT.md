...
| File | Purpose |
|------|---------|
| `src-tauri/src/main.rs` | Rust entry point — initializes Tauri, builds system tray (Show Window · separator · Quit Genesis menu), spawns backend, opens window, intercepts close-to-tray; registers the `begin_quit` command; the shared `request_quit` function shows+focuses the window and emits `quit-requested` to the webview for the web-page confirm flow (reached from BOTH the tray Quit item AND a user-initiated macOS Cmd+Q via `RunEvent::ExitRequested`); owns `BACKEND_HOST`/`backend_url` and the deterministic `dashboard_loaded` boot gate (`navigate_until_loaded`) |
| `src-tauri/src/shell_log.rs` | Shell-side file logger — `init()` (truncates `<data_dir>/logs/shell.log` once per process + installs a panic hook recording payload/location/backtrace), `log()` (RFC 3339 timestamped append that also keeps the `[desktop] …` console output), `data_dir()`/`log_path()` (mirror `EvoGit.Platform.data_dir/0`) |
...
- **Logging**: the sidecar writes no log file; the Elixir backend's stdout/stderr are drained and re-printed by `sidecar.rs` to the desktop process's own stdout/stderr with a `[backend] ` prefix (piped → `println!`/`eprintln!`); in `--headless` mode they are inherited directly (`Stdio::inherit`). No `--log` flags and no logging env vars are passed to the backend (sidecar env is only PORT/PHX_IP/PHX_SERVER/SECRET_KEY_BASE/RELEASE_DISTRIBUTION/EVOGIT_DESKTOP/EVOGIT_LIFETIME_PORT). **The Rust shell DOES write a log file**: `<data_dir>/logs/shell.log` (`src-tauri/src/shell_log.rs`) — `init()` creates+truncates it once at shell startup and installs a panic hook (payload/location/backtrace), and `log()` appends RFC 3339 timestamped lines alongside the `[desktop] …` console output, covering sidecar spawn/exit, watchdog restart, webview navigation results, boot-gate outcome, quit/update intents, and `RunEvent::Exit`. `data_dir()` mirrors `EvoGit.Platform.data_dir/0` (macOS `~/Library/Application Support/genesis/`, Linux `$XDG_DATA_HOME/genesis/`, Windows `%APPDATA%\genesis\`). Accepted limitation: a `config.toml [data] dir` override is invisible to the shell (it does not parse the user TOML), so the shell log stays at the platform default.
...
- **Configurable binding address**: the backend binds to `127.0.0.1` (localhost) by default. Set `EVOGIT_BIND=0.0.0.0` before launching for remote access. The backend port is chosen by the shell at startup: an explicitly-set `PORT` env var is honored only when that port is free; otherwise a random free ephemeral port is picked (`TcpListener::bind("127.0.0.1:0")`). The chosen port is passed to the backend via `PORT`, and the WebView always connects via the **IPv4 loopback literal `127.0.0.1`** on that same resolved port (`BACKEND_HOST`/`backend_url(port)` in `src-tauri/src/main.rs`) regardless of bind address — `localhost` is a macOS trap because it also resolves to `::1` and WKWebView (CFNetwork) does not reliably fall back to IPv4, while the Phoenix endpoint binds IPv4 loopback only.
...
2. Tauri polls `http://127.0.0.1:<PORT>` until the backend responds (up to 30s)
3. The WebView window opens, pointing to `http://127.0.0.1:<PORT>` (the window is created in Rust with `WebviewUrl::External`; there is no config-declared window). The window's initial load races the backend boot and typically fails with connection refused; once the readiness poll (step 2) succeeds, the setup re-navigates the webview to the dashboard via a **deterministic** bounded gate (`navigate_until_loaded` in `src-tauri/src/main.rs`). The window's `on_page_load` callback latches a `dashboard_loaded` flag ONLY on `PageLoadEvent::Finished` for the backend URL, and the gate polls that latch (bounded attempts, non-busy, aborting on quit/update intent) — because a `navigate`/`load_url` call reports Ok as soon as the request is ACCEPTED, not when the page loaded, so a naive "retry until accepted" would stop at the first attempt and leave a silently-failed load stuck on the failed-load page forever. On budget exhaustion the shell navigates to the existing watchdog error page (with its Retry button) instead of leaving a blank page. This re-navigation is required for a healthy boot: without it the webview would sit on the failed-load page for the whole session, the dashboard's `quit-requested` listener would never load, and the tray Quit confirmation would wedge (the watchdog only navigates on the crash-recovery path)
4. Closing the window hides it to the system tray (backend keeps running); the "Quit Genesis" tray menu item (below a separator) — or a user-initiated macOS Cmd+Q — shows+focuses the window and emits `quit-requested` so the dashboard renders a web-page confirm dialog (backend-down fallback: immediate `kill_for_quit()` + exit — see "Quit Flow (Web Confirmation)")
...
1. **Tray "Quit Genesis" — or a user-initiated macOS Cmd+Q (see the "Cmd+Q" paragraph below)** → Rust shows+focuses the main window (unminimize → show → set_focus, same as the "Show Window" arm and the single-instance callback), then probes backend health (`sidecar::probe_http`).
...
**Cmd+Q (macOS)**: `run_gui` drives Tauri with an explicit `app.run(|_, event| …)` loop instead of a bare `.run(...)`. A `RunEvent::ExitRequested` with `code == Some(_)` (one of our own `app.exit()` calls) is passed through untouched, while a user-initiated one (`code == None`, i.e. Cmd+Q) is intercepted with `api.prevent_exit()` and routed through the **same `request_quit` function** as the tray item — so Cmd+Q asks for confirmation like every other quit path instead of hard-terminating the app while its backend keeps running (previously Cmd+Q had no handler and killed the whole app without the confirm flow). **Accepted caveat:** `prevent_exit()` cannot distinguish Cmd+Q from an OS logout/shutdown, so a logout also waits for the user to confirm the dashboard dialog — consistent with the no-force-quit-with-a-healthy-backend policy.
