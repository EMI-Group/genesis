...
- **Simultaneous-launch race**: two instances launched at (nearly) the same instant can both pass the check on macOS (socket not yet bound) and, in theory, Linux (D-Bus name acquisition is atomic so Linux is safe; Windows mutex is atomic too). The macOS race window is small and inherent to the plugin — not mitigated.
