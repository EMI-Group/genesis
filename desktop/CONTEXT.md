...
- **Simultaneous-launch race**: two instances launched at (nearly) the same instant can both pass the check on macOS (socket not yet bound) and, in theory, Linux (D-Bus name acquisition is atomic so Linux is safe; Windows mutex is atomic too). The macOS race window is small and inherent to the plugin — not mitigated, but now harmless: with dynamic ports each instance's backend gets a different port, so there is no port-conflict crash (both windows simply appear).
...
**Gotcha**: both SVGs have a large viewBox (`16002.59 x 12975.69`) with the artwork filling it edge-to-edge — render high-res, then trim/fit to ~86% of a square 1024x1024 transparent canvas, then:
...
- **Orphaned backend on abnormal shell death (fixed)**: when the shell is killed abnormally (macOS Force Quit, crash/panic, `kill -9`) the backend child used to survive and keep port 9999 bound, so the next launch crashed with "port 9999 is already in use" (the new backend failed to bind; the watchdog entered the infinite error-page retry). Two-part fix: (a) **dynamic ports** — the shell picks a free ephemeral port at startup (`TcpListener::bind("127.0.0.1:0")`), so a fresh launch never reuses a zombie's port (the OS won't assign a bound port to `bind(0)`); (b) **TCP lifetime pipe (zero polling)** — the shell starts a `127.0.0.1:0` lifetime listener (`sidecar::start_lifetime_listener`, detached accept thread holding one connection per backend spawn) and passes its port via `EVOGIT_LIFETIME_PORT` in the sidecar env; the Elixir backend (`EvoGit.DesktopParentMonitor`, apps/evo_git) connects to the pipe and blocks on recv — the shell never writes, so any close/error means the shell is dead and the backend calls `System.stop(0)` immediately (gated on `EVOGIT_DESKTOP=1`; var omitted → monitor off). No polling, immediate detection. Listener bind failure is non-fatal (defense-in-depth — the dynamic port already prevents the crash). Side effect: the single-instance macOS simultaneous-launch race is now harmless (both instances' backends get different ports). Reference `desktop/src-tauri/CONTEXT.md` for the Rust-side details.

## Auto-Update Status

The Tauri-side updater is **IMPLEMENTED** (official `tauri-plugin-updater` v2.10.1): `src-tauri/Cargo.toml` has `tauri-plugin-updater = "2"`, `src-tauri/tauri.conf.json` has `bundle.createUpdaterArtifacts: true` and a top-level `plugins.updater` block (`endpoints` → GitHub `releases/latest/download/latest.json`, real minisign `pubkey` — see handoff below, `windows.installMode: "passive"`), and the Rust sources expose three update commands (`check_update` / `download_update` / `begin_update`, JSON contracts pinned with the dashboard workstream) plus a watchdog update-intent install flow — full details in `desktop/src-tauri/CONTEXT.md` → "Auto-Update (tauri-plugin-updater v2 — IMPLEMENTED)". `src-tauri/capabilities/default.json` is unchanged (`core:default`, `shell:allow-execute`, `shell:allow-spawn` — app-owned commands need no permissions; the JS npm package is NOT used, the dashboard invokes the commands via `window.__TAURI__.core.invoke`).

**Handoff / blockers (CI workstream — root scope)**: the pubkey in `tauri.conf.json` is now a **real minisign public key** (generated via `tauri signer generate` and verified by the src-tauri maintainers) — the old placeholder is gone, so the Rust-side placeholder gate no longer triggers. The remaining blocker is CI-side: the `TAURI_SIGNING_PRIVATE_KEY` / `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` repo secrets don't exist yet, so `tauri build` with `createUpdaterArtifacts: true` still FAILS at the signing step (`A public key has been found, but no private key. Make sure to set \`TAURI_SIGNING_PRIVATE_KEY\` environment variable.`; `--no-sign` skips signing; `tauri build --config '{"bundle":{"createUpdaterArtifacts":false}}'` is a valid JSON-fragment override so CI stays green pre-key) and CI publishes no signed updater payloads / `latest.json` (the feed endpoint returns nothing usable until `publish-release` runs with signing). App version surfaces as literals in `tauri.conf.json` `version` (drives installer and updater version comparison) and `Cargo.toml` `version`, both synced by `mix bump.version`; the Rust code reads its own version via `app.package_info().version`. **CI artifacts (root `.github/workflows/build-desktop.yml`, out of this subtree)**: CI passes explicit `--bundles` flags, so the **published** artifacts are: macOS → `.dmg` + a `ditto -c -k --keepParent` `.app.zip` (L556, L594-595) — **NO `.app.tar.gz`, the artifact tauri-plugin-updater's macOS installer expects**; Windows → all targets (MSI + NSIS `.exe`, staged L711-712); Linux x64 → `deb,rpm,appimage` + portable `.tar.gz`; Linux arm64 → `deb,rpm` only (matrix L156-168). Updater feed candidates today: the NSIS `.exe` (Windows), the x64 AppImage (Linux); macOS requires a CI change to also produce/stage the `.app.tar.gz`. Integration-relevant facts: the bundled backend resource `resources/genesis-backend` (gitignored; populated only before `cargo tauri build`) is a standard mix release whose launcher `bin/genesis_desktop` resolves the active version via `releases/start_erl.data` — the launcher path is version-independent, so replacing the resource dir (backend-only update) needs no Rust spawn-logic changes; the `begin_update` flag → backend `System.stop()` → watchdog install-and-relaunch flow is the implemented apply path (the backend must be fully stopped before the installer runs — on Windows the BEAM child maps NIF DLLs from inside the bundle, locking them against NSIS replacement). CI artifact publishing/manifest signing lives in the root `.github/workflows/` (out of this subtree).
