defmodule EvoGit.Runtime.Genesis do
  ...
  def run(root_prompt) do
    ...
    # 1. Initialize Root
    {:ok, head_sha} = PhyloGraphNode.current_head()

    # Create Root Context
    initial_state = %{commit_sha: head_sha, node_path: "."}
    objective = "Create the top-level CONTEXT.md defining the architecture based on: #{root_prompt}. Create only the CONTEXT.md file and directory structure (empty dirs are fine)."

    case Agent.mutate(initial_state, objective) do
      {:ok, root_state} ->
        Logger.info("Genesis: Root context created at #{String.slice(root_state.commit_sha, 0, 7)}")
  ...
  defp expand(current_sha, visited) do
    # Find all directories in current SHA using PhyloGraphNode abstraction
    case PhyloGraphNode.list_directories(current_sha) do
      {:ok, all_dirs} ->
        ...
        if candidates == [] do
          ...
          objective = "Implement the context and structure for this directory based on the parent context. Create CONTEXT.md if it is a structural directory, or source files if it is a leaf."
